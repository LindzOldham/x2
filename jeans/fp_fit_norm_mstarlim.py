...
re_norm, vd_norm, mu_norm = np.loadtxt('/data/ljo31b/EELs/phys_models/FP_normals_100_3re_Mstarlim.dat').T
re_nugg, vd_nugg, mu_nugg = np.loadtxt('/data/ljo31b/EELs/phys_models/FP_nuggets_100_3re_Mstarlim.dat').T
...
dxx,dyy,dzz = 2.*dvd_norm/vd_norm, dmu_norm,dre_norm/re_norm/np.log(10.)
...
np.savetxt('/data/ljo31b/EELs/phys_models/FP_normals_100_scattered_3re_Mstarlim_1.dat',[xx,yy,zz,dxx,dyy,dzz])
...
S.sample(4000)
outFile = '/data/ljo31b/EELs/inference/FP/jeansmodels_norm_3re_Mstarlim_1'
f = open(outFile,'wb')
cPickle.dump(S.result(),f,2)
f.close()
result = S.result()
result = np.load('/data/ljo31b/EELs/inference/FP/jeansmodels_norm_3re_Mstarlim_1')
lp,trace,dic,_ = result
a1,a2 = np.unravel_index(lp.argmax(),lp.shape)
for i in range(len(pars)):
    pars[i].value = trace[a1,a2,i]#np.median(trace[1000:,:,i])
