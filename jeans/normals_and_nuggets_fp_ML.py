...
def run(Mstar,n,ML):
    ...
    sigma_dm_norm = veldisp(r,sb_norm,Mdm,ap=2.*gal_norm.re)
    sigma_dm_nugg = veldisp(r,sb_nugg,Mdm,ap=2.*gal_nugg.re)
    sigma_star_norm = veldisp(r,sb_norm,Mlum_norm,ap=2.*gal_norm.re)      
    sigma_star_nugg = veldisp(r,sb_nugg,Mlum_nugg,ap=2.*gal_nugg.re)  
    vd_norm = (sigma_dm_norm + sigma_star_norm)**0.5
    vd_nugg = (sigma_dm_nugg + sigma_star_nugg)**0.5

    # m/l = 1
    mag_sol = solarmag.getmag('F606W_ACS',0.5)
    mag = -2.5*Mstar + mag_sol + 2.5*np.log10(ML)
...
Mstars = np.random.uniform(low=10.5,high=11.8,size=100)
ML_ratios = np.random.uniform(low=0.1,high=10.,size=100) # stellar mass-to-light ratios
...
for i in range(Mstars.size):
    run(Mstars[i],n=sersic_indices[i],ML=ML_ratios[i])

np.savetxt('/data/ljo31b/EELs/phys_models/FP_normals_100_2re_ML.dat',cat_norm)
np.savetxt('/data/ljo31b/EELs/phys_models/FP_nuggets_100_2re_ML.dat',cat_nugg)
