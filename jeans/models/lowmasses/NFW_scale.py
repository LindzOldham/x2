...
def run(Mstar,result,name):
    ...
    sigma_star = veldisp(r,sb,Mlum,ap=1.5*RE)
    sigma_dm = veldisp(r,sb,Mdm,ap=1.5*RE)
    #vd = (sigma_dm + sigma_star)**0.5
    vd1,vd2 = sigma_star**0.5, sigma_dm**0.5

    # SB
    cat_nugg.append([vd1,vd2])
    print  name, '%.2f'%vd1,'%.2f'%vd2, Mhalo
...
np.savetxt('/data/ljo31b/EELs/phys_models/models/NFW_12_scale_LO_M.dat',cat_nugg)
