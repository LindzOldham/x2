...
def run(Mstar,result,name):
    ...
    # halo properties
    Mhalomu = highz_halo_table(Mstar,0.5)
    Mhalo = np.random.normal(Mhalomu,0.24)
    ...
    sigma_star = veldisp(r,sb,Mlum+Mdm,ap=1.5*RE)
...
np.savetxt('/data/ljo31b/EELs/phys_models/models/NFW_normal_AMscatter.dat',cat_nugg)


np.savetxt('/data/ljo31/Lens/pylathon/FP/data4.dat',VD)
re,vdd,vdl = np.loadtxt('/data/ljo31/Lens/pylathon/FP/data4.dat').T
