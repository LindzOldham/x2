...
re_norm, vd_norm, mu_norm = np.loadtxt('/data/ljo31b/EELs/phys_models/FP_normals_100_2renorm_Mstarlim.dat').T
re_nugg, vd_nugg, mu_nugg = np.loadtxt('/data/ljo31b/EELs/phys_models/FP_nuggets_100_2renorm_Mstarlim.dat').T
...
pl.figure()
pl.scatter(1.2*np.log10(vd_norm)+0.3*mu_norm, np.log10(re_norm),color='SteelBlue',s=40,label='normals')
pl.scatter(1.2*np.log10(vd_nugg)+0.3*mu_nugg, np.log10(re_nugg),color='SteelBlue',s=40,label='nuggets')
pl.xlabel('$1.2 \log\sigma + 0.3 \mu$')
pl.ylabel('$\log r_e$')
pl.legend(loc='lower right')
...
np.savetxt('/data/ljo31b/EELs/phys_models/FP_nuggets_100_scattered_2renorm_mstarlim_1.dat',[xx,yy,zz,dxx,dyy,dzz])
...
S.sample(3000)
outFile = '/data/ljo31b/EELs/inference/FP/jeansmodels_nugg_2renorm_mstarlim_1'
f = open(outFile,'wb')
...
for i in range(len(pars)):
    pars[i].value = np.median(trace[1000:,:,i])
