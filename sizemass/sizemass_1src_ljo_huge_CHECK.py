...
masses = np.load('/data/ljo31b/EELs/inference/new/huge/NEWFORFPNEW_masses_212.npy')
...
dlogRe = dRe/Re
rho = np.load('/data/ljo31/Lens/LensParams/ReMass_covariances.npy')[:,0] # remake???
#np.save('/data/ljo31/Lens/LensParams/ReMass_1src_huge_ljo',np.column_stack((logRe,logM,dlogRe,dlogM,rho)))
...
S.sample(4000)
outFile = '/data/ljo31/Lens/Analysis/huge/sizemass_1src_ljo'
f = open(outFile,'wb')
...
pl.legend(loc='upper left',fontsize=15,scatterpoints=1)
pl.xlim([10.5,12])
pl.ylim([-0.4,1.9])
pl.show()
...
np.save('/data/ljo31/Lens/Analysis/huge/sizemass_1src_fit_ljo_new',np.column_stack((xfit,yfit,lo,up)))
