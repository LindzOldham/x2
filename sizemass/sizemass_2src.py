...
model_vi,masses,model_vk,ages = np.load('/data/ljo31/Lens/Analysis/InferredAges_2src_all_new.npy')
vi,vk,dvi,dvk,v,i,k,Re,dRe = np.load('/data/ljo31/Lens/LensParams/colours_2src.npy')
...
np.save('/data/ljo31/Lens/LensParams/ReMass_2src_new',np.column_stack((logRe,logM,dlogRe,dlogM,rho)))
...
S.sample(4000)
outFile = '/data/ljo31/Lens/Analysis/sizemass_2src_inferredage_wideprior_new'
f = open(outFile,'wb')
...
np.save('/data/ljo31/Lens/Analysis/sizemass_2src_fit_new',np.column_stack((xfit,yfit,lo,up)))
