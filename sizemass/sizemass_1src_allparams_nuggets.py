...
logRe,logM,dlogRe,dlogM,rho = logRe[nuggets],logM[nuggets],dlogRe[nuggets],dlogM[nuggets],rho[nuggets]
name = py.open('/data/ljo31/Lens/LensParams/Phot_1src.fits')[1].data['name'][nuggets]


np.save('/data/ljo31/Lens/LensParams/ReMass_1src_nuggets_allparams',np.column_stack((logRe,logM,dlogRe,dlogM,rho)))
'''
logRe,logM,dlogRe,dlogM,rho = np.load('/data/ljo31/Lens/LensParams/ReMass_1src_nuggets_allparams.npy').T
logRe1,logM1,dlogRe1,dlogM1,rho1 = np.load('/data/ljo31/Lens/LensParams/ReMass_1src_lensgals_nuggets_allparams.npy').T
cols = [logRe,logM,dlogRe,dlogM,rho]
col1s = [logRe1,logM1,dlogRe1,dlogM1,rho1]
for i in range(len(cols)):
    cols[i] = np.concatenate((cols[i],col1s[i]))

logRe,logM,dlogRe,dlogM,rho = cols
print len(logRe)
...
S.sample(4000)
outFile = '/data/ljo31/Lens/Analysis/sizemass_1src_inferredage_allparams_nuggets'
f = open(outFile,'wb')
...
pl.figure()
pl.scatter(x[:-3],y[:-3],color='SteelBlue')
pl.plot(xfit,xfit*beta + alpha,'SteelBlue')#,label='EELs')
plot_ellipses(x[:-3],y[:-3],sxx[:-3],syy[:-3],rho[:-3],'SteelBlue')
pl.scatter(x[-3:],y[-3:],color='Crimson')
pl.plot(xfit,xfit*beta + alpha,'Crimson')#,label='EELs')
plot_ellipses(x[-3:],y[-3:],sxx[-3:],syy[-3:],rho[-3:],'Crimson')
...
np.save('/data/ljo31/Lens/Analysis/sizemass_1src_fit_allparams_nuggets',np.column_stack((xfit,yfit,lo,up)))
