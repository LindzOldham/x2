...
model_vi,masses,model_vk,ages = np.load('/data/ljo31/Lens/Analysis/LensgalInferredAges_2src_all_new.npy')
# update j1125 based on its special fit:
visrc,age,Msrc,Mlens,vilens,vksrc,vklens = np.load('/data/ljo31/Lens/Analysis/j1125_InferredAge.npy').T[0].T
model_vi[:,3],model_vk[:,3] = vilens,vklens
masses[:,3] = Mlens
ages[:,3] = age
# check order there!!!
vi,vk,dvi,dvk,v,i,k,Re,dRe = np.load('/data/ljo31/Lens/LensParams/colours_lens_2src.npy')
...
np.save('/data/ljo31/Lens/LensParams/ReMass_lens_2src_new',np.column_stack((logRe,logM,dlogRe,dlogM,rho)))
...
for i in range(len(name)):
    print name[i], '& $', '%.2f'%Re[i], r'\pm', '%.2f'%dRe[i], r'$ & $','%.2f'%masses[1,i], r'\pm', '%.2f'%(masses[2,i]-masses[1,i]), '$ & $', '%.2f'%ages[1,i], r'\pm', '%.2f'%(ages[2,i]-ages[1,i]), '$ & $', '%.2f'%vi[i], r'\pm', '%.2f'%dvi[i],'$ & $','%.2f'%model_vi[1,i], r'\pm', '%.2f'%(model_vi[2,i]-model_vi[1,i]),'$ & $', '%.2f'%vk[i], r'\pm', '%.2f'%dvk[i],'$ & $', '%.2f'%model_vk[1,i], r'\pm', '%.2f'%(model_vk[2,i]-model_vk[1,i]),r'$ \\'
...
S.sample(4000)
outFile = '/data/ljo31/Lens/Analysis/sizemass_2src_inferredage_lensgals_new'
f = open(outFile,'wb')
