...
def CotSomponents(components,col):
    ...
    pl.suptitle(col)
    pl.subplot(325)
    pl.imshow(components[4],interpolation='nearest',origin='lower',cmap='afmhot',aspect='auto',vmax=np.amax(components[4])*0.5)
    pl.colorbar()
    pl.title('source 2 ')
...
sig2 = py.open('/data/ljo31/Lens/J1248/sig2_maskdust3.fits')[0].data.copy()[10:-10,20:-25]

result = np.load('/data/ljo31/Lens/J1248/emcee22')
lp= result[0]
a2=0.
...
gal = img2 - comps[0] - comps[1] - comps[2]
srcmod = comps[3]

pl.figure()
pl.imshow(gal, interpolation='nearest',origin='lower',vmin=0,vmax=np.amax(gal)*0.5)
pl.colorbar()
pl.title('galaxy subtracted')
pl.figure()
pl.imshow(srcmod, interpolation='nearest',origin='lower',vmin=0,vmax=np.amax(srcmod)*0.5)
pl.colorbar()
pl.title('source model')
