...
X=31 - gui_11_2src3, offsets are now on the V-band, using the gross image in case my image is still too small??? 2/1/2, ovrs=1. THis is on V-I, but using an I-band guifile!
X=32 - exactly the same as 31, but dividing the noisemaps bby half. To see if this helps it to find solutions!
X=33 - X=31, with prior on re source = 0.05. Because it's hitting the 0.2 prior edge atm. Weird source. Still using ovrs=1 (but should probably change this?)
X=34 - as 33, but with ovrs=2. It would really be nice if the V-band could look better as well...but beggars can't be choosers
'''
...
import time
start=time.time()
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=8,nwalkers=80,ntemps=3)
S.sample(1000)
...
jj=0
for jj in range(20):
    S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=8,nwalkers=80,ntemps=3,initialPars=trace[-1])
    S.sample(1000)
