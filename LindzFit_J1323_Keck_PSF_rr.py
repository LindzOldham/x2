...
X=0
X=1 # continuing X=0
X=2 # continuing X=1, better prior on sig2!
X=3 # trying to select only the good walkers and start it off from where they are...
print X
...
pars.append(pymc.TruncatedNormal('pa 3',mu=pa3,tau=15.,value=pa3,a=-180,b=180))
...
'''row = klp[-1,0]
ktrace=ktrace[:,0]
ktrace2=ktrace[:,row>-35300000]
print ktrace2.shape
Ktrace = np.zeros((40,len(pars)))
Ktrace[:31] = ktrace2[-1]
Ktrace[31:] = ktrace2[-1,:9]

print np.where(Ktrace==0)'''
