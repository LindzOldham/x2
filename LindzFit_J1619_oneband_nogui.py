...
X=32 - putting best I band on V band and fitting positions only. - so based on X=11.
'''
...
print '1'
gals.append(SBModels.Sersic('Galaxy 1',{'x':pars[-2],'y':pars[-1],'q':0.92,'pa':-69.65,'re':21.64,'n':1.19}))
cov += [0.1,0.1]
### galaxy 2
pars.append(pymc.Uniform('Galaxy 2 x',40.,50.,value=45.))
pars.append(pymc.Uniform('Galaxy 2 y',45.,55.,value=49.))
print '2'
gals.append(SBModels.Sersic('Galaxy 2',{'x':pars[-2],'y':pars[-1],'q':0.99,'pa':-26.82,'re':3.28,'n':1.89}))
cov += [0.1,0.1]
# lens 1
pars.append(pymc.Uniform('Lens 1 x',40.,50.,value=45.))
pars.append(pymc.Uniform('Lens 1 y',45.,55.,value=49.))
print '3'
lenses.append(MassModels.PowerLaw('Lens 1',{'x':pars[-2],'y':pars[-1],'q':0.95,'pa':0.,'b':9.91,'eta':1.29}))
cov += [0.1,0.1]
# lens 2
lenses.append(MassModels.ExtShear('Shear',{'x':lenses[0].pars['x'],'y':lenses[0].pars['y'],'pa':-26.33,'b':-0.0592}))
# source 1
pars.append(pymc.Uniform('Source 1 x',42.,52.,value=47.))
pars.append(pymc.Uniform('Source 1 y',47.,57.,value=52.))
srcs.append(SBModels.Sersic('Source 1',{'x':pars[-2],'y':pars[-1],'q':0.67,'pa':95.25,'re':33.98,'n':7.51}))
cov += [0.1,0.1]
...
import time
start=time.time()
S = myEmcee.Emcee(pars+[likelihood],cov=optCov,nthreads=4,nwalkers=40)
S.sample(1000)
...
jj=0
for jj in range(20):
    S = myEmcee.Emcee(pars+[likelihood],cov=optCov,nthreads=4,nwalkers=40,initialPars=trace[-1])
    S.sample(1000)
