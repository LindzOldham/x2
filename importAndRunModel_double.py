...
imgName = '/data/ljo31/Lens/SDSSJ1606+2235_F606W_sci_cutout_2.fits'
sigName = '/data/ljo31/Lens/SDSSJ1606+2235_F606W_noise_cutout_2.fits'
...
guiFile = '/data/ljo31/Lens/ModelFit13'
...
# for a double-source lens, define a second lens + source system which depends on the first...
pars_2 = []
srcs_2 = []
cov_2 = []
for name in S.keys():
    ...
    for key in 'x','y','q','pa','re','n':
        if s[key]['type']=='constant':
            ...
            cov.append(s[key]['sdev'])
    srcs.append(SBModels.Sersic(name,p))
    ### get an approximate position for this one
...
for name in L.keys():
    ...
    for key in 'x','y','q','pa','b','eta':
        if s[key]['type']=='constant':
            ...
            pars.append(pymc.Uniform('%s %s'%(name,key),lo,hi,value=val))
            p[key] = pars[-1]
            cov.append(8) # idea for standard deviation
    key = 'b'
    lo,hi,val = 10,35,20 # ideas for the Einstein radius of this second source
    lenses_2.append(MassModels.PowerLaw(name,p))
...
pyfits.PrimaryHDU(model).writeto('%s_components_2.fits'%pref,clobber=True)
pyfits.PrimaryHDU(im).writeto('%s_model_2.fits'%pref,clobber=True)
pyfits.PrimaryHDU(image-im).writeto('%s_resid_2.fits'%pref,clobber=True)
