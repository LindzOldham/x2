...
X=7 # think we need ovrs=5 really. This one has ovrs=1 and multiplying model.
print X
    ...
    if name == 'Source 2':
        ...
        for key in 'q','re','n','pa':
            if key == 're':
                ...
                p[key] = dic[name+' '+key][a1,a2,a3]
        for key in 'x','y': # subtract lens potition - to be added back on later in each likelihood iteration!
            val = dic['Source 2 '+key][a1,a2,a3]*5.
            lo,hi=val-20,val+20
            pars.append(pymc.Uniform('%s %s'%(name,key),lo ,hi,value=val ))
            p[key] = pars[-1] + lenses[0].pars[key] 
            cov +=[5]
...
jj=0
for jj in range(10):
    S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=18,nwalkers=44,ntemps=3,initialPars=trace[-1])
    S.sample(500)
