...
result = np.load('/data/ljo31/Lens/J1125/emcee3419')
lp= result[0]
trace = result[1]
det = result[2]
print 'here'
...
for k,j in itertools.product(range(thin.shape[0]),range(thin.shape[1])):
    ...
    srcs,gals,lenses = [],[],[]
    srcs.append(SBModels.Sersic('Source 1', {'x':det['Source 1 x'][k,j]+det['Lens 1 x'][k,j],'y':det['Source 1 y'][k,j]+det['Lens 1 y'][k,j],'q':det['Source 1 q'][k,j],'pa':det['Source 1 pa'][k,j],'re':det['Source 1 re'][k,j],'n':det['Source 1 n'][k,j]}))
    srcs.append(SBModels.Sersic('Source 2', {'x':det['Source 2 x'][k,j]+det['Lens 1 x'][k,j],'y':det['Source 2 y'][k,j]+det['Lens 1 y'][k,j],'q':det['Source 2 q'][k,j],'pa':det['Source 2 pa'][k,j],'re':det['Source 2 re'][k,j],'n':det['Source 2 n'][k,j]}))
    ...
    gals.append(SBModels.Sersic('Galaxy 2', {'x':det['Galaxy 1 x'][k,j],'y':det['Galaxy 1 y'][k,j],'q':det['Galaxy 2 q'][k,j],'pa':det['Galaxy 2 pa'][k,j],'re':det['Galaxy 2 re'][k,j],'n':det['Galaxy 2 n'][k,j]}))
...
print 'F606W & $', '%.2f'%ms[ii][0], '_{-', '%.2f'%lower5[0], '}^{+', '%.2f'%upper5[0], '}$ & $', '%.2f'%mi[ii][0], '_{-', '%.2f'%lower5[1], '}^{+', '%.2f'%upper5[1], '}$ & $', '%.2f'%mu[ii][0], '_{-', '%.2f'%lower5[2], '}^{+', '%.2f'%upper5[2], '}$ & $', '%.2f'%reff[ii][0], '_{-', '%.2f'%lower5[3], '}^{+', '%.2f'%upper5[3], r'}$ \\' 
...
np.save('/data/ljo31/Lens/J1125/phot_FINAL_uncertainties_nonconcentric_606', grandarray_555)
np.save('/data/ljo31/Lens/J1125/phot_FINAL_uncertainties_nonconcentric_814', grandarray_814)
