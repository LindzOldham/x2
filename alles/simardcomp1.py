...
eeels = 1-q
pl.figure()
pl.hist(e,25,normed=1,histtype='stepfilled',label='Simard+11')
for ii in range(len(eeels)-1):
    pl.axvline(eeels[ii],ls='--',lw=3,color='k')
pl.axvline(eeels[-1],ls='--',lw=3,color='k',label='EELs')
pl.xlim([0,0.8])
pl.xlabel('$e$ (X models)')
pl.legend(loc='upper right')
pl.savefig('/data/ljo31/Lens/TeXstuff/paper/nhistellipt.pdf')
