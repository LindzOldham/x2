...
bands = np.load('/data/ljo31/Lens/LensParams/HSTBands.npy')[()]
sz = np.load('/data/ljo31/Lens/LensParams/LensRedshifts.npy')[()]
table_k = np.load('/data/ljo31/Lens/LensParams/KeckPhot_lensgals_1src_new_dict.npy')[()]
table = py.open('/data/ljo31/Lens/LensParams/Phot_1src_lensgals_new.fits')[1].data
...
v=vk*0
name = 'J1619'
Res,mags = np.load('/data/ljo31/Lens/Analysis/ReMagPDFs_lensgals_'+str(name)+'.npy')
rev,rei = Res.T
magv,magi =  mags.T
...
for jj in range(200):
    ...
    np.save('/data/ljo31/Lens/Analysis/ReMassTrace_200_asigo_j1619_lensgals',masses)
