...
# plot things
def NotPlicely(image,im,sigma,V):
    ...
    pl.suptitle(str(V))
    pl.savefig('/data/ljo31/public_html/Lens/J0901/plotrun'+str(X)+str(V)+'.png')
...
guiFile = '/data/ljo31/Lens/J0901/model23'

print guiFile
X=11
result = np.load('/data/ljo31/Lens/J0901/emcee16')
    ...
    if name == 'Source 2':
        ...
        for key in 'q','re','n','pa':
           ...
           if key == 're':
               ...
               pars.append(pymc.Uniform('%s %s'%(name,key),lo-0.5,hi,value=val))
           p[key] = pars[-1]
           if key == 'pa':
        ...
        for key in 'x','y': # subtract lens potition - to be added back on later in each likelihood iteration!
            ...
            val = dic[name+' '+key][a1,a2,a3]
            lo,hi = lo - lenses[0].pars[key].value.item(), hi - lenses[0].pars[key].value.item()
            pars.append(pymc.Uniform('%s %s'%(name,key),lo ,hi+2,value=val ))
...
for i in range(len(imgs)):
    ...
    if comps == True:
        for kk in range(len(gals)+len(srcs)):
            ...
            pl.colorbar()
            pl.savefig('/data/ljo31/public_html/Lens/J0901/grrr/component'+str(kk)+'.png')
    fits.append(fit)
...
x5,y5,re5,n5,pa5,q5 = dic['Source 2 x'][a1,a2,a3], dic['Source 2 y'][a1,a2,a3],dic['Source 2 re'][a1,a2,a3], dic['Source 2 n'][a1,a2,a3], dic['Source 2 pa'][a1,a2,a3], dic['Source 2 q'][a1,a2,a3]
