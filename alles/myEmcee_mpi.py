...
class Emcee:
    def __init__(self,pars,cov=None,nwalkers=None,initialPars=None):
        ...
        if len(hackdets)==0:
            self.sampler = emcee.EnsembleSampler(self.nwalkers,self.ndim,optFunc,pool=pool)
        else:
            self.sampler = emcee.EnsembleSampler(self.nwalkers,self.ndim,optFunc_blobs,pool=pool)
        self.p0 = initialPars
...
class PTEmcee:
    def __init__(self,pars,cov=None,nwalkers=None,ntemps=None,initialPars=None):
...
#            self.nwalkers = self.nwalkers+1
        ...
        self.pool = MPIPool(loadbalance=True)
        if not self.pool.is_master():
            self.pool.wait()
            sys.exit(0)

        self.sampler = emcee.PTSampler(self.ntemps,self.nwalkers,self.ndim,loglikelihood,logprior,pool=self.pool)
        self.p0 = initialPars
