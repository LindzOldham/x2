...
class PixelizedModel:
    ...
    def setCentroid(self,weight=True):
    ...
    def pixeval(self,x,y,scale=1.,order=3):
        X = (x-self.x)*scale+self.x0
        Y = (y-self.y)*scale+self.y0
        psf = ndimage.map_coordinates(self.model,[Y,X],prefilter=False,order=order)
        psf /= psf.sum()
        return self.amp*psf
