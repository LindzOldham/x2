...
pars = []
cov = []
pars.append(pymc.Uniform('log age lens',8,10.1,value=9.5))
pars.append(pymc.Uniform('log age src',8,10.1,value=9.5))
...
sigma = np.array([0.5*dvi,0.5*dvis,0.5*dvi,0.5*dvis,0.5*dvk,0.5*dvks,dg,dr,di,dz])
mu = [1,9]
@pymc.deterministic
def logP(value=0.,p=pars):
    ...
    logT,M = [pars[0].value,pars[1].value],[pars[2].value,pars[3].value]
    for kk in range(2):
        model_v[kk],model_i[kk] = splev(logT[kk],interps_V[kk]).item()-2.5*M[kk],splev(logT[kk],interps_I[kk]).item()-2.5*M[kk]
        model_k[kk] = splev(logT[kk],interps_K[kk]).item()-2.5*M[kk]
        ##
        Fu += 10**M[kk]*10**(-0.4*splev(logT[kk],U[kk]).item()) * mu[kk]
        Fg += 10**M[kk]*10**(-0.4*splev(logT[kk],G[kk]).item()) * mu[kk]
        Fr += 10**M[kk]*10**(-0.4*splev(logT[kk],R[kk]).item()) * mu[kk]
        Fi += 10**M[kk]*10**(-0.4*splev(logT[kk],I[kk]).item()) * mu[kk]
        Fz += 10**M[kk]*10**(-0.4*splev(logT[kk],Z[kk]).item()) * mu[kk]
    mod_u,mod_g,mod_r = -2.5*np.log10(Fu),-2.5*np.log10(Fg),-2.5*np.log10(Fr)
    mod_i,mod_z = -2.5*np.log10(Fi),-2.5*np.log10(Fz)
    #gr, iz,gi = mod_g-mod_r, mod_i-mod_z,mod_g-mod_i
    ...
    model = np.array([model_v[0],model_v[1],model_i[0],model_i[1],model_k[0],model_k[1],mod_g,mod_r,mod_i,mod_z])
