...
'''
X = 54 - exponent regularisation
X = 55 - multiplicative regularisation
'''
...
def NotPlicely(image,im,sigma):
    ...
    pl.title('data-model')
    pl.subplot(224)
    pl.imshow((image-im)/sigma,origin='lower',interpolation='nearest',extent=ext,vmin=-0.5,vmax=0.5,cmap='afmhot',aspect='auto')
...
poisson1 = py.open('/data/ljo31/Lens/J0901/F606W_noisemap_poisson.fits')[0].data.copy()
noise1 = py.open('/data/ljo31/Lens/J0901/F606W_noisemap_sigma.fits')[0].data.copy()
...
poisson2 = py.open('/data/ljo31/Lens/J0901/F814W_noisemap_poisson.fits')[0].data.copy()
noise2 = py.open('/data/ljo31/Lens/J0901/F814W_noisemap_sigma.fits')[0].data.copy()
...
guiFile = '/data/ljo31/Lens/J0901/model12'
...
result = np.load('/data/ljo31/Lens/J0901/emcee38')
lp= result[0]
...
## sig = (poisson + sigma**X)**0.5
pars.append(pymc.Uniform('noise coefficient',0.5,10,value=1.))
cov.append(1.0)
...
# add X to trace
addtrace = np.random.normal(1,0.3,trace[0,:,:,0].shape)
usetrace = np.zeros((trace[a1].shape[0], trace[a1].shape[1],trace[a1].shape[2]+1))
usetrace[:,:,:28] = trace[a1]
usetrace[:,:,28] = addtrace

S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=10,nwalkers=74,ntemps=4,initialPars=usetrace)
print 'set up'
S.sample(1000)
print 'sampled'
outFile = '/data/ljo31/Lens/J0901/emcee_noisemap'+str(X)
f = open(outFile,'wb')
