...
# check order there!!!
vi,vk,dvi,dvk,v,i,k = np.load('/data/ljo31/Lens/LensParams/colours_1src.npy')

logRe = np.log10(Re)
logM = masses[1]
dlogM = masses[2]-masses[1]
dlogRe = dRe/Re
rho = np.load('/data/ljo31/Lens/LensParams/ReMass_covariances_new.npy')[:,0]
...
np.save('/data/ljo31/Lens/Analysis/sizemass_1src_fit',np.column_stack((xfit,yfit,lo,up)))
'''
