...
#sig2 = py.open('/data/ljo31/Lens/J1248/sig2_maskdust4plus.fits')[0].data.copy()#[10:-10,20:-25]

result = np.load('/data/ljo31/Lens/J1248/emcee28')
lp= result[0]
a2=0.
...
gals = []
for name in ['Galaxy 1','Galaxy 2','Galaxy 3']:
    p = {}
    if name == 'Galaxy 1':
...
dustabs = dic['dust absorption'][a1,a2,a3]
colours = ['F606W', 'F814W']
models = []
fits = []
for i in range(len(imgs)):
    ...
    for src in srcs:
        ...
        print tmp.shape
        b,std,xmid,ymid = lenses[0].pars['b'], lenses[0].pars['b']*0.5,lenses[0].pars['x'],lenses[0].pars['y']
        r = np.sqrt((xin-xmid)**2. + (yin-ymid)**2.)
        mask3 = np.where((galaxy>=0.05*np.amax(galaxy))   & (r>b-std) & (r<b+std))# & (galaxy>=0.2*np.amax(galaxy)))
        pl.figure()
        pl.imshow(tmp,origin='lower')
        pl.colorbar()
        one = tmp.copy()
        tmp.ravel()[mask3] -= dustabs#*galaxy[mask3]
        print gal.pixeval(xin,yin)[mask3]
        two = tmp.copy()
        pl.figure()
        pl.imshow(tmp,origin='lower')
        pl.colorbar()
        tmp = convolve.convolve(tmp,psf,False)[0]
        print tmp.shape
        print 'e', galaxy.shape
        model[n] = tmp.ravel()
        print galaxy[mask3]
        pl.figure()
        pl.imshow(galaxy.reshape(image.shape))
        n +=1
    ...
    pl.figure()
    pl.imshow(one-two,origin='lower',vmin=0,vmax=10)
    pl.colorbar()
...
re5,n5,pa5,q5,x5,y5 = dic['Galaxy 3 re'][a1,a2,a3], dic['Galaxy 3 n'][a1,a2,a3], dic['Galaxy 3 pa'][a1,a2,a3], dic['Galaxy 3 q'][a1,a2,a3],dic['Galaxy 3 x'][a1,a2,a3], dic['Galaxy 3 y'][a1,a2,a3],
...
print 'galaxy 3 ', '&', '%.2f'%x5, '&',  '%.2f'%y5, '&', '%.2f'%n5, '&', '%.2f'%re5, '&', '%.2f'%q5, '&','%.2f'%pa5,  r'\\'
