...
def NotPlicely(image,im,sigma):
    ...
    pl.figure()
    pl.subplot(221)
    pl.imshow(image,origin='lower',interpolation='nearest',extent=ext,vmin=0,vmax=10) #,vmin=vmin,vmax=vmax)
    pl.colorbar()
    pl.title('data')
    pl.subplot(222)
    pl.imshow(im,origin='lower',interpolation='nearest',extent=ext,vmax=10) #,vmin=vmin,vmax=vmax)
    pl.colorbar()
    pl.title('model')
    pl.subplot(223)
...
#print g1,g2,s1,s2,l1

## try some way of fitting!
#pars = [xoffset,yoffset,sig]
pars = []
cov = []
pars.append(pymc.Uniform('xoffset',52.,82.,value=67))
pars.append(pymc.Uniform('yoffset',48,77,value=62.5))
cov += [20,20] # think about this!
pars.append(pymc.Uniform('sigma',0,8,value=4))
cov += [5]
pars.append(pymc.Uniform('q',0,1,value=0.6))
cov += [0.5]
pars.append(pymc.Uniform('pa',-180,180,value=100.0))
cov += [50]
...
def logP(value=0,p=pars):
    ...
    q = pars[3].value
    pa = pars[4].value
    psfObj = SBObjects.Gauss('psf',{'x':0,'y':0,'sigma':sig,'q':q,'pa':pa,'amp':1})
    ...
    g1a['x'] = g1a['x']+x0
    g1a['y'] = g1a['y']+y0
    g2a['x'] = g1a['x'] + 5*(g2a['x']-(g1a['x']-x0))
    g2a['y'] = g1a['y'] + 5*(g2a['y']-(g1a['y']-y0))
...
model = lensModel.lensFit(coeff,image,sigma,gals,lenses,srcs,xc,yc,OVRS,noResid=True,psf=psf,verbose=True,getModel=True,showAmps=True)
