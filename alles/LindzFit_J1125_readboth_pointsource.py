...
result = np.load('/data/ljo31/Lens/J1125/emcee29a')
lp= result[0]
...
for name in S.keys():
    ...
    if name == 'Source 2':
        ...
        ()
print p
srcs.append(SBModels.Sersic('Source 2',p))

print 'here'
xx,yy = srcs[0].pars['x'], srcs[0].pars['y']
print srcs
...
def logP(value=0.,p=pars):
    ...
    for i in range(len(imgs)):
        ...
        for src in srcs:
            ...
            n +=1
        tmp = xc*0
        ii = np.where(((x0-xx)**2 + (y0-yy)**2)**0.5<0.5)
        tmp = iT.resamp(tmp, OVRS, True)
        tmp = convolve.convolve(tmp,psf,False)[0]
        model[n] = tmp[mask].ravel()
        n +=1
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=12,nwalkers=60,ntemps=4,initialPars=trace2[a1])
S.sample(100)
#S = myEmcee.Emcee(pars+[likelihood],cov=optCov,nwalkers=100,nthreads=6) # should have 100 walkers.
#S.sample(1000)

outFile = '/data/ljo31/Lens/J1125/emcee_pointsource_'+str(X)
f = open(outFile,'wb')
...
for i in range(len(imgs)):
    ...
    for src in srcs:
        ...
        n +=1
    tmp = xc*0
    ii = np.where((abs(np.sqrt((x0-xx)**2 + (y0-yy)**2))<0.5))
    tmp = iT.resamp(tmp, OVRS, True)
    tmp = convolve.convolve(tmp,psf,False)[0]
    model[n] = tmp.ravel()
    n +=1
