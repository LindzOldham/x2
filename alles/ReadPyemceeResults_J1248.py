...
result = np.load('/data/ljo31/Lens/J1248/emcee96')
lp= result[0]
a2=0.
...
for i in range(len(imgs)):
    ...
    for gal in gals:
        ...
        tmp = xc*0.
        if gal.pars['q']<0.3:
            tmp = gal.boxypixeval(xp,yp,1./OVRS,csub=11,c=cc) # evaulate on the oversampled grid. OVRS = number of new pixels per old pixel.
        else:
