...
X=18 - gui_10. Put x=10 back into gui and running. 2/1/1
X = 19 - model17 on a bigger image
X = 20 - as 18, but on biggerigger image!!!
X=21 - as 19, but fitting for a background term in both bands. Lensoff.
X = 22 - as 21, but with lens on. This will take a lot longer, but will be a good check. Compare 21 and 22: does turning the lens off make the fit significantly worse?
X = 23 - as 16, on bigger image and fitting for bg. lensoff.
X = 24 - as 17, but on bigger image and fitting for bg. Lensoff nochmal.
X=26 - a final run of X=23, now allowing everything to vary and everything to be offset.
X=27 - X=21 run again, lenson initially then lensoff.
X=28 - run of X=24 (when it's done), but with the lens on (at least initially) TO RUN
'''
...
guiFile = '/data/ljo31/Lens/J1144/model17'
...
import time
start=time.time()
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=10,nwalkers=72,ntemps=4)
S.sample(1500)
print time.time()-start

outFile = '/data/ljo31/Lens/J1144/emcee_lenson'+str(X)
f = open(outFile,'wb')
...
## now fix lens
newpars = pars[:12]+pars[-6:]
newcov = cov[:12]+cov[-6:]
newoptCov = np.array(newcov)

starter=np.zeros((4,72,18))
starter[:,:,:12] = trace[-1,:,:,:12]
starter[:,:,12:] = trace[-1,:,:,:-6]

print starter.shape, len(newpars)

jj=0
for jj in range(5):
    S = myEmcee.PTEmcee(newpars+[likelihood],cov=newoptCov,nthreads=10,nwalkers=72,ntemps=4,initialPars=starter)
    S.sample(1000)

    outFile = '/data/ljo31/Lens/J1144/emcee_lensoff'+str(X)
    f = open(outFile,'wb')
    cPickle.dump(S.result(),f,2)
    f.close()
...
cPickle.dump([G,L,S,offsets,[guishear,True]],open('/data/ljo31/Lens/J1144/UpdatedGuiFile_'+str(X),'wb'))
