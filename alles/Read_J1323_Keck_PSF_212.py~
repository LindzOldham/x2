...
kresult = np.load('/data/ljo31/Lens/J1323/KeckPSF_3')
klp= kresult[0]
ka2=0
...
for i in range(len(imgs)):
    ...
    for gal in gals:
        gal.setPars()
        tmp = xc*0.
        tmp[mask2] = gal.pixeval(xin,yin,1,csub=11) # evaulate on the oversampled grid. OVRS = number of new pixels per old pixel.
    ...
    for src in srcs:
        src.setPars()
        tmp = xc*0.
        tmp[mask2] = src.pixeval(x0,y0,1,csub=11)
