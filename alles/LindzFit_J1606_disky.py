...
X=2 - gui_0c but running Lind\Fit_J1606_disky - so using the exponential disk rather than the sersic for the source's disk! TO RUN NEXT
...
guiFile = '/data/ljo31/Lens/J1606/gui_0c'
...
for name in S.keys():
    ...
    p = {}
    if name == 'Source 2':
        pars.append(pymc.Uniform('Source 2 x',s['x']['lower'],s['x']['upper'],value=s['x']['value']))
        pars.append(pymc.Uniform('Source 2 y',s['y']['lower'],s['y']['upper'],value=s['y']['value']))
        pars.append(pymc.Uniform('Source 2 pa',-180,180.,value=-50))
        pars.append(pymc.Uniform('Source 2 x0',0,50,value=20))
        pars.append(pymc.Uniform('Source 2 y0',0.5,5,value=2))
        cov += [1.,1.,5.,5.,5.]
        srcs.append(SBBModels.Disk('Source 2',{'x':pars[-5],'y':pars[-4],'pa':pars[-3],'x0':pars[-2],'y0':pars[-1]}))
    elif name == 'Source 1':
        print name
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=15,nwalkers=72,ntemps=3)
S.sample(1000)
...
jj=0
for jj in range(16):
    S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=15,nwalkers=72,ntemps=3,initialPars=trace[a1])
    S.sample(1000)
