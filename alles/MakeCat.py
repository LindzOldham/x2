...
mags.append(mag)
mus.append(mu)
restmus.append(restmu)
restmags.append(restmag)
Res.append(Re)
Lums.append(l)
cats_m.append(med.flatten())
cats_l.append(lo.flatten())
cats_h.append(hi.flatten())
...
print 'J0901'
mag,mu,restmu,restmag,Re,l = MakeTab(model)
print mag,mu,restmu,restmag,Re
key=model.name
#mags.append((key,mag))
mags.append(mag)
mus.append(mu)
...
Lums.append(l)
print Lums
cat = np.column_stack((mags,restmags, mus, restmus, Res,Lums))
#np.savetxt('/data/ljo31/Lens/LensParams/PhotCat_1src.txt',cat)
#np.save('/data/ljo31/Lens/LensParams/PhotCat_1src',cat)
'''
...
data=np.array(zip(names,cats_m[0],cats_m[1],cats_m[:,0],cats_m[:,1],cats_m[:,2],cats_m[:,3],cats_m[:,4],cats_m[:,5],cats_m[:,6],cats_m[:,7],cats_m[:,8],cats_m[:,9],cats_m[:,10],cats_m[:,11],cats_m[:,12],cats_m[:,13],cats_l[0],cats_l[1],cats_l[:,0],cats_l[:,1],cats_l[:,2],cats_l[:,3],cats_l[:,4],cats_l[:,5],cats_l[:,6],cats_l[:,7],cats_l[:,8],cats_l[:,9],cats_l[:,10],cats_l[:,11],cats_l[:,12],cats_l[:,13],cats_h[0],cats_h[1],cats_h[:,0],cats_h[:,1],cats_h[:,2],cats_h[:,3],cats_h[:,4],cats_h[:,5],cats_h[:,6],cats_h[:,7],cats_h[:,8],cats_h[:,9],cats_h[:,10],cats_h[:,11],cats_h[:,12],cats_h[:,13]),dtype=types)
...
#data = np.array(zip(names,cat).T,dtype=types)
np.save('/data/ljo31/Lens/LensParams/PhotCat_1src',data)
np.savetxt('/data/ljo31/Lens/LensParams/PhotCat_1src.txt',data,fmt=["%s"] +["%.3f",]*14)
...
cat = np.column_stack((mags,restmags, mus, restmus, Res,Lums))
#np.savetxt('/data/ljo31/Lens/LensParams/PhotCat_2src.txt',cat)
#np.save('/data/ljo31/Lens/LensParams/PhotCat_2src',cat)



types = [('s1','S16'),('mag v',float),('mag i',float),('rest mag v',float),('rest mag i',float),('rest ,mag b',float),('mu v',float),('mu i',float),('rest mu v',float),('rest mu i',float),('Re v',float),('Re i',float),('lum v',float),('lum i',float),('lum b',float)]

names = ['J0913','J1125','J1144','J1323','J1347','J1446','J1605','J1606','J2228']

data=np.array(zip(names,cat[:,0],cat[:,1],cat[:,2],cat[:,3],cat[:,4],cat[:,5],cat[:,6],cat[:,7],cat[:,8],cat[:,9],cat[:,10],cat[:,11],cat[:,12],cat[:,13]),dtype=types)


#data = np.array(zip(names,cat).T,dtype=types)
np.save('/data/ljo31/Lens/LensParams/PhotCat_2src',data)
np.savetxt('/data/ljo31/Lens/LensParams/PhotCat_2src.txt',data,fmt=["%s"] +["%.3f"]*14)
'''
'''
magv,magi,rmagv,rmagi,rmagb,muv,mui,rmuv,rmui,rv,ri,lumv,lumi,lumb = np.loadtxt('/data/ljo31/Lens/LensParams/PhotCat_1src.txt',unpack=True,usecols=[1]).T
ii=np.isfinite(lumi)
