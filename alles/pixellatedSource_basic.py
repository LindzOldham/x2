...
#mg = pyfits.open('/data/ljo31/Lens/J1605/SDSSJ1605+3811_F814W_sci_cutout2.fits')[0].data.copy()
...
img = pyfits.open('/data/ljo31/Lens/J1605/img.fits')[0].data.copy()
...
for name in L.keys():
    ...
    for key in 'x','y':
        print key, model[key]['value']
        if model[key]['type']=='constant':
            lpars[key] = model[key]['value']
        else:
            lo,hi,val = model[key]['lower'],model[key]['upper'],model[key]['value']
            pars.append(pymc.Uniform('%s:%s'%(name,key),lo+15,hi+15,value=val+15))
            lpars[key] = pars[-1]
            cov.append(model[key]['sdev'])
