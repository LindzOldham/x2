...
X=7 - restarting 5 with galaxies chained together to stop disk from wandering off...
X=8 - with a cut-off disk radius!
X=9 - 2/1/2 - gui_2src_3_uniforms, ovrs=2.
X=12 - as 9, but forcing galaxy 2 not to wander off like last time.
'''
...
guiFile = '/data/ljo31/Lens/J1446/gui_2src_3_uniforms'
...
pars.append(pymc.Uniform('Galaxy 2 xmax',0,100,value=15))
#pars.append(pymc.Uniform('Galaxy 2 x',dic['Galaxy 2 x'][a1,a2,a3]-5,dic['Galaxy 2 x'][a1,a2,a3]+5,value=dic['Galaxy 2 x'][a1,a2,a3]+1))
#pars.append(pymc.Uniform('Galaxy 2 y',dic['Galaxy 2 y'][a1,a2,a3]-5,dic['Galaxy 2 y'][a1,a2,a3]+5,value=dic['Galaxy 2 y'][a1,a2,a3]-1))
pars.append(pymc.Uniform('Galaxy 2 pa',-180,180.,value=14))
pars.append(pymc.Uniform('Galaxy 2 x0',0,50,value=5))
pars.append(pymc.Uniform('Galaxy 2 y0',1,5,value=1.5))
cov += [0.5,5.,10.,0.5]
gals.append(SBBModels.Disk('Galaxy 2',{'x':gals[0].pars['x'],'y':gals[0].pars['y'],'pa':pars[-3],'x0':pars[-2],'y0':pars[-1],'xmax':pars[-4]}))
print gals[1].pars
...
for name in L.keys():
    ...
    for key in 'x','y','q','pa','b','eta':
        lo,hi,val = s[key]['lower'],s[key]['upper'],s[key]['value']#ddic[name+' '+key][a1,a2,a3]
        pars.append(pymc.Uniform('%s %s'%(name,key),lo,hi,value=val))
        if key=='pa':
...
p['x'] = lenses[0].pars['x']
p['y'] = lenses[0].pars['y']
pars.append(pymc.Uniform('extShear',-0.3,0.3,value=shear[0]['b']['value']))#dic['extShear'][a1,a2,a3]))
cov.append(0.05)
p['b'] = pars[-1]
pars.append(pymc.Uniform('extShear PA',-180.,180,value=shear[0]['pa']['value']))#value=dic['extShear PA'][a1,a2,a3]))
cov.append(100.)
p['pa'] = pars[-1]
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=15,nwalkers=70,ntemps=3)
S.sample(1000)
...
jj=0
for jj in range(20):
    S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=15,nwalkers=70,ntemps=3,initialPars=trace[-1])
    S.sample(1000)
