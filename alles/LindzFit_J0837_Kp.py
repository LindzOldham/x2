...
guiFile = '/data/ljo31/Lens/J0837/model0_Kp'
...
for name in S.keys():
    ...
    if name == 'Source 1':
        ...
        for key in 'x','y': # subtract lens potition - to be added back on later in each likelihood iteration!
            ...
            print (val-lo)*6, (hi-val)*6, val
            pars.append(pymc.Uniform('%s %s'%(name,key),val-(val-lo)*6. ,val+(hi-val)*6.,value=val ))   # the parameter is the offset between the source centre and the lens (in source plane obvs)
...
S.sample(1000)
outFile = '/data/ljo31/Lens/J0837/emcee_Kp'+str(X)
f = open(outFile,'wb')
