...
result = np.load('/data/ljo31/Lens/J1323/emcee_1src4')
lp= result[0]
a2=0.
...
for i in range(len(imgs)):
    ...
    for gal in gals:
        gal.setPars()
        tmp = xc*0.
        tmp = gal.boxypixeval(xp,yp,1./OVRS,csub=23) # evaulate on the oversampled grid. OVRS = number of new pixels per old pixel.
    ...
    for src in srcs:
        src.setPars()
        tmp = xc*0.
        tmp = src.boxypixeval(x0,y0,1./OVRS,csub=23)
