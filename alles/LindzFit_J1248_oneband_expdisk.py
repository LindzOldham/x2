...
X=6 - Iband_go
X=7 - as 6, but easing prior on galaxy 2 n
X=8 - with an exponential disk for the galaxy light
X=9 - with expsechdisk, but starting from gui values rather than dic values. Just to see if that does anything.'''
...
result = np.load('/data/ljo31/Lens/J1248/emcee6')
lp= result[0]
a2=0.
a1,a3 = numpy.unravel_index(lp[:,0].argmax(),lp[:,0].shape)
trace = result[1]
dic = result[2]
guiFile = '/data/ljo31/Lens/J1248/Iband_go'
print guiFile
...
for name in G.keys():
    ...
    if name == 'Galaxy 1':
        ...
        pars.append(pymc.Uniform('Galaxy 2 xmax',0,300,value=55))
        ...
        pars.append(pymc.Uniform('Galaxy 2 pa',-180,180.,value=129))
        pars.append(pymc.Uniform('Galaxy 2 x0',0,50,value=20))
        pars.append(pymc.Uniform('Galaxy 2 y0',1.,8,value=3.5))
        cov += [10,1,1,10,5,5]
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=22,nwalkers=60,ntemps=3)
S.sample(1000)
...
jj=0
for jj in range(20):
    S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=22,nwalkers=60,ntemps=3,initialPars=trace[-1])
    S.sample(1000)
