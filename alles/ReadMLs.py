...
pars = np.zeros(12)
result = np.load('/data/ljo31/Lens/Analysis/1src_VKIage_lensgals_wideprior')
lp,trace,dic,_ = result
a1,a2 = np.unravel_index(lp.argmax(),lp.shape)
ftrace=trace.reshape((trace.shape[0]*trace.shape[1],trace.shape[2]))
for i in range(len(pars)):
    pars[i] = np.percentile(ftrace[:,i],50,axis=0)
...
for m in range(12):
    print name[m],' & ', '%.2f'%pars[m],  ' & ','%.2f'%masses[m], ' & ', '%.2f'%mlvs[m], ' & ', '%.2f'%vi[m],' & ', '%.2f'%vk[m], ' & ', '%.2f'%model_vi[m],' & ', '%.2f'%model_vk[m], r'\\'
print r'\end{tabular}'
