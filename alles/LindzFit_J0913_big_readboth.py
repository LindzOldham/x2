...
''' 
X = 0 - model3 - with an edited F814W noisemap. 
X = 1 - model4 - with an edited F814W noisemap.
X = 2 - model5
X = 3 - model6
X = 4 - model7
...
X = 10 - gui_emcee6_both_2sg_b READY TO RUN ASAP!
X = 11 - big1. Made images bigger to catch all flux and stop psf fringing
X = 10b - continuing 10, with more walkers!
X = 10c - the end of 10b, but allowing the sources to separate to see what that does to les residuells
'''
...
for name in S.keys():
    ...
    if name == 'Source 2':
        ...
        for key in 'q','re','n','pa':
           ...
           val = dic[name+' '+key][a1,a2,a3]
           if key == 'n':
               pars.append(pymc.Uniform('%s %s'%(name,key),lo,12,value=val))
           else:
               pars.append(pymc.Uniform('%s %s'%(name,key),lo,hi,value=val))
           p[key] = pars[-1]
           if key == 'pa':
...
t = np.zeros((4,100,34))
t[:,:,:32] = trace[a1]
t[:,:,32]=dic['Source 2 x'][-1]
t[:,:,33]=dic['Source 2 y'][-1]
#t = np.zeros((4,100,32))
#t[:,:72,:]=trace[a1]
#t[:,72:,:]=trace[-1,:,:28,:]

S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=10,nwalkers=100,ntemps=4,initialPars=t) # too complicated!,initialPars=trace[a1])
S.sample(1000)
...
jj=0
for jj in range(12):
    S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=10,nwalkers=100,ntemps=4,initialPars=trace[a1]) #too complicated!
    print 'sampling'
    S.sample(1000)
