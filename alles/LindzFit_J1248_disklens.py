...
''' X=0 - modelX6. 2/1/1. Residuals on I band are pretty bad...
X=1 - model5 - 2/2/1
X=3 - gui_0_2lenses_uniforms 2/2/1. X=1 just seemed to get completely lost.
X=4 - gui_0_2srcs_8_uniforms
'''
...
def NotPlicely(image,im,sigma):
    ...
    pl.figure()
    pl.subplot(221)
    pl.imshow(image,origin='lower',interpolation='nearest',extent=ext,cmap='afmhot',aspect='auto',vmin=0,vmax=np.amax(image)*0.8) #,vmin=vmin,vmax=vmax)
    pl.colorbar()
    pl.title('data')
    pl.subplot(222)
    pl.imshow(im,origin='lower',interpolation='nearest',extent=ext,cmap='afmhot',aspect='auto',vmin=0,vmax=np.amax(image)*0.8) #,vmin=vmin,vmax=vmax)
    pl.colorbar()
    pl.title('model')
    pl.subplot(223)
...
guiFile = '/data/ljo31/Lens/J1248/gui_0_2lenses_8_uniforms'
#guiFile = '/data/ljo31/Lens/J1248/gui_0_2lenses_uniforms'
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=24,nwalkers=70,ntemps=3)
S.sample(1000)
...
jj=0
for jj in range(40):
    S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=24,nwalkers=70,ntemps=3,initialPars=trace[-1])
    S.sample(1000)
