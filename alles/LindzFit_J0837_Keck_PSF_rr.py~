...
for name in ['Source 2', 'Source 1']:
    ...
    if name == 'Source 2':
        print name
        key = 'q'
        pars.append(pymc.Uniform('%s %s'%(name,key),0.05,1,value=dic[name+' '+key][a1,a2,a3] ))
        p[key] = pars[-1]
        key = 'n'
        pars.append(pymc.Uniform('%s %s'%(name,key),0.1,8,value=dic[name+' '+key][a1,a2,a3] ))
        p[key] = pars[-1]
        key = 're'
        pars.append(pymc.Uniform('%s %s'%(name,key),0.1,100,value=dic[name+' '+key][a1,a2,a3] ))
        p[key] = pars[-1]
        key = 'pa'
        pars.append(pymc.Uniform('%s %s'%(name,key),-180,180,value=dic[name+' '+key][a1,a2,a3] ))
        p[key] = pars[-1]
        cov += [0.1,0.1,1,1]
