...
X=14 - 2/1/2, but all Sersices (these seem to be better at finding solutions maybe? Weiss nicht)
X=17 - continuing x14
X=18 - modelX3
'''
...
for name in L.keys():
    ...
    for key in 'x','y','q','pa','b','eta':
        lo,hi,val = s[key]['lower'],s[key]['upper'],s[key]['value']
        if key == 'eta':
            pars.append(pymc.Uniform('%s %s'%(name,key),lo,1.75,value=val))
        else:
            pars.append(pymc.Uniform('%s %s'%(name,key),lo,hi,value=val))
        if key=='pa':
...
cov.append(0.05)
p['b'] = pars[-1]
pars.append(pymc.Uniform('extShear PA',-180.,180,value=shear[0]['b']['value']))#dic['extShear PA'][a1,a2,a3]))
cov.append(100.)
p['pa'] = pars[-1]
...
jj=0
for jj in range(20):
    S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=20,nwalkers=70,ntemps=3,initialPars=trace[-1])
    S.sample(1000)
