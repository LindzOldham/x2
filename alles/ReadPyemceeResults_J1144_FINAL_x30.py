...
''' emcee version of X22 - ie. 2/1/1 model '''
...
result = np.load('/data/ljo31/Lens/J1144/emcee_ONESRC_FINAL_30')
...
cov.append(0.1)
p['b'] = pars[-1]
if dic['extShear PA'][a1,a2] > 0:
    pars.append(pymc.Uniform('extShear PA',0.,180.,value=dic['extShear PA'][a1,a2]))
elif dic['extShear PA'][a1,a2] < 0:
    pars.append(pymc.Uniform('extShear PA',-180.,0,value=dic['extShear PA'][a1,a2]))
cov.append(100.)
p['pa'] = pars[-1]
...
#re5,n5,pa5,q5,x5,y5 = dic['Source 2 re'][a1,a2], dic['Source 2 n'][a1,a2], dic['Source 2 pa'][a1,a2], dic['Source 2 q'][a1,a2],dic['Source 2 x'][a1,a2], dic['Source 2 y'][a1,a2]
...
re3,n3,pa3,q3,x3,y3 = dic['Galaxy 2 re'][a1,a2], dic['Galaxy 2 n'][a1,a2], dic['Galaxy 2 pa'][a1,a2], dic['Galaxy 2 q'][a1,a2], dic['Galaxy 1 x'][a1,a2], dic['Galaxy 1 y'][a1,a2]
#if trace.shape[-1] == 26:
...
for name in G.keys():
    ...
    if name == 'Galaxy 1':
        for key in 'x','y','q','pa','re','n':
            s[key]['value'] = np.array(dic[name+' '+key][a1,a2])
    elif name == 'Galaxy 2':
        for key in 'q','pa','re','n':
            s[key]['value'] = np.array(dic[name+' '+key][a1,a2])
        for key in 'x','y':
            s[key]['value'] = np.array(dic['Galaxy 1 '+key][a1,a2])
...
for name in guishear.keys():
    guishear[name]['value'] = np.array(dic['Lens 1 '+key][a1,a2])

guishear['b']['value'] = np.array(dic['extShear'][a1,a2])
guishear['pa']['value'] = np.array(dic['extShear PA'][a1,a2])
...
for name in S.keys():
    ...
    if name == 'Source 1':
        for key in 'q','re','n','pa':
            s[key]['value'] = np.array(dic[name+' '+key][a1,a2])
        for key in 'x','y':
            s[key]['value'] = np.array(dic[name+' '+key][a1,a2] + dic['Lens 1 '+key][a1,a2]) 
    
print guishear
offsets[0][3] = np.array(dic['xoffset'][a1,a2])
offsets[1][3] = np.array(dic['yoffset'][a1,a2])
cPickle.dump([G,L,S,offsets,[guishear,True]],open('/data/ljo31/Lens/J1144/FINAL_1src_30','wb'))
