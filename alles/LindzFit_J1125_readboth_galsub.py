...
#lp = np.load('/data/ljo31/Lens/J1125/emcee29_lpcut.npy')
#trace = np.load('/data/ljo31/Lens/J1125/emcee29_tracecut.npy')
#dic = np.load('/data/ljo31/Lens/J1125/emcee29_diccut.npy')[()]

result = np.load('/data/ljo31/Lens/J1125/emcee29b')
lp= result[0]
...
cov += [0.4,0.4]
trace2[:,:,j] = dic['xoffset'][a1]
j+=1
trace2[:,:,j] = dic['yoffset'][a1]
j+=1
...
trace2[:,:,j] = dic['extShear'][a1]
j+=1
trace2[:,:,j] = dic['extShear PA'][a1]
j+=1
...
for i in range(len(imgs)):
    ...
    galaxy = components[0] + components[1]
    imgs[i] = image - galaxy
...
for jj in range(3):
    S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=5,nwalkers=60,ntemps=4,initialPars=trace2)
    S.sample(10000)
    #S = myEmcee.Emcee(pars+[likelihood],cov=optCov,nwalkers=100,nthreads=6) # should have 100 walkers.
    #S.sample(1000)
    outFile = '/data/ljo31/Lens/J1125/emcee_galsub'+str(X)
    f = open(outFile,'wb')
