...
pars.append(pymc.Uniform('log lens mass j1125',10,11.8,value=11.))
...
#data  = np.array([vi,vis,vk,vks,u_sdss,g_sdss,r_sdss,i_sdss,z_sdss])
#sigma = np.array([dvi,dvis,dvk,dvks,du,dg,dr,di,dz])

data  = np.array([v-i,vs-i_s,v-k,vs-ks,g_sdss-r_sdss,i_sdss-z_sdss])
sigma = np.array([0.2*dvi,0.2*dvis,0.2*dvk,0.2*dvks,dgr,diz])
...
def logP(value=0.,p=pars):
    ...
    model = np.array([model_vi[0],model_vi[1],model_vk[0],model_vk[1],gr,iz])
...
mod_i,mod_z = -2.5*np.log10(Fi),-2.5*np.log10(Fz)
model_vi,model_vk = model_v-model_i, model_v-model_k
model = np.array([model_vi[0],model_vi[1],model_vk[0],model_vk[1],mod_g,mod_r,mod_i,mod_z])
