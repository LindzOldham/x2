...
noisemap = np.where((smooth>0.7*sigma)&(im>0),0.1*im/wht+poisson, poisson)**0.5
...
py.writeto('/data/ljo31/Lens/'+str(name[:5])+'/F606W_poissonnoisemap2.fits',noisemap,clobber=True)
pl.figure()
...
## save
py.writeto('/data/ljo31/Lens/'+str(name[:5])+'/F814W_poissonnoisemap2.fits',noisemap,clobber=True)
pl.figure()
pl.imshow(noisemap,origin='lower',interpolation='nearest')
pl.colorbar()
