...
from scipy import ndimage
im = py.open('/data/ljo31/Lens/J1605/SDSSJ1605+3811_F555W_sci_cutout_med.fits')[0].data.copy()
wht = py.open('/data/ljo31/Lens/J1605/SDSSJ1605+3811_F555W_wht_cutout_med.fits')[0].data.copy()

smooth = ndimage.gaussian_filter(im,0.7)
noisemap = np.where((smooth>0.7*sigma)&(im>0),im/wht+poisson_v, poisson_v)**0.5
#py.writeto('/data/ljo31/Lens/J1605/SDSSJ1605+3811_F555W_noisemap_med.fits',noisemap,clobber=True)
pl.figure()
pl.imshow(noisemap)
pl.colorbar()
'''
...
im = py.open('/data/ljo31/Lens/J1605/SDSSJ1605+3811_F814W_sci_cutout_med.fits')[0].data.copy()
wht = py.open('/data/ljo31/Lens/J1605/SDSSJ1605+3811_F814W_wht_cutout_med.fits')[0].data.copy()

smooth = ndimage.gaussian_filter(im,0.7)
noisemap = np.where((smooth>0.7*sigma)&(im>0),im/(4.*wht)+poisson_i, poisson_i)**0.5
py.writeto('/data/ljo31/Lens/J1605/SDSSJ1605+3811_F814W_noisemap_med_2.fits',noisemap,clobber=True)
pl.figure()
pl.imshow(noisemap,origin='lower',interpolation='nearest',vmin=0,vmax=0.04)
pl.colorbar()
