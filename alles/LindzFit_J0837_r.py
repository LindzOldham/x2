...
X=3 - putting the Kp model onto HST images. Using dustlanenew noisemaps so the nnls isn't thrown off...but see how this performs. We might want to revert to the old noisemaps
'''
...
img1 = py.open('/data/ljo31/Lens/J0837/F606W_sci_cutout.fits')[0].data.copy()[15:-15,15:-15]
sig1 = py.open('/data/ljo31/Lens/J0837/F606W_noisemap_dustlanenew.fits')[0].data.copy()#[15:-15,15:-15]
psf1 = py.open('/data/ljo31/Lens/J0837/F606W_psf1.fits')[0].data.copy()
psf1 = psf1/np.sum(psf1)

img2 = py.open('/data/ljo31/Lens/J0837/F814W_sci_cutout.fits')[0].data.copy()[15:-15,15:-15]
sig2 = py.open('/data/ljo31/Lens/J0837/F814W_noisemap_dustlanenew.fits')[0].data.copy()#[15:-15,15:-15]
psf2 = py.open('/data/ljo31/Lens/J0837/F814W_psf3.fits')[0].data.copy()
psf2 = psf2/np.sum(psf2)


guiFile = '/data/ljo31/Lens/J0837/model4_i'
...
mask = py.open('/data/ljo31/Lens/J0837/mask.fits')[0].data.copy()[15:-15,15:-15] #+ py.open('/data/ljo31/Lens/J0837/mask4.fits')[0].data.copy()[15:-15,15:-15]
...
pars = []
cov = []
pars.append(pymc.Uniform('V-I xoffset',-5.,5.,value=0.1))
pars.append(pymc.Uniform('V-I yoffset',-5.,5.,value=0.1))
cov += [0.2,0.2]
pars.append(pymc.Uniform('Kp-V xoffset',-35.,-20,value=-27))
pars.append(pymc.Uniform('Kp-V yoffset',-35.,-20.,value=-24))
cov += [0.2,0.2]
    ...
    if name == 'Source 1':
        ...
        for key in 'q','n','pa':
           ...
           if key == 'pa':
               ...
               cov.append(s[key]['sdev']*10)
        for key in ['re']:
            lo,hi,val = s[key]['lower'],s[key]['upper'],dic[name+' '+key][a1,a2,a3]*0.2
            pars.append(pymc.Uniform('%s %s'%(name,key),lo,hi,value=val))
            p[key] = pars[-1]
            cov.append(s[key]['sdev']*10)
...
def logP(value=0.,p=pars):
    ...
    for i in range(len(imgs)):
        if i == 0:
            dx,dy = pars[2].value,pars[3].value
        else:
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=15,nwalkers=50,ntemps=3)
print S
S.sample(1000)
...
jj=0
for jj in range(12):
    S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=15,nwalkers=50,ntemps=3,initialPars=trace[a1])
    S.sample(1000)
