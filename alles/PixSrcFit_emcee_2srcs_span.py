...
def lnprob(X,xc,yc,rhs):
    ...
    if np.any(X<0) or n<0.1 or n2<0.1 or n>6 or n2>6 or re>40 or re2>40 or q>1 or q2>1 or pa>200 or pa2>200 or pa<-180 or pa2<-180:
        return -np.inf
...
yc,xc = iT.coords(osrc.shape)*0.2275 # 92/400
...
p0[:,0] = 220*0.2275 +np.random.randn(nwalkers)*5 # x
p0[:,1] = 200.*0.2275 +np.random.randn(nwalkers)*5 # y
p0[:,2] = 150. + np.random.randn(nwalkers)*20 # pa
p0[:,3] = 0.7 + np.random.randn(nwalkers)*0.1 # q
p0[:,4] = 30 + np.random.randn(nwalkers)*2 # Re
p0[:,5] = 4. + np.random.randn(nwalkers)*1 # n
p0[:,6] = 150. + np.random.randn(nwalkers)*20 # pa
p0[:,7] = 0.7 + np.random.randn(nwalkers)*0.1 # q
p0[:,8] = 2. + np.random.randn(nwalkers)*0.5 # Re
...
model = fit[0] * src.pixeval(iT.coords(osrc.shape)[1]*0.2275,iT.coords(osrc.shape)[0]*0.2275) + fit[1] * src2.pixeval(iT.coords(osrc.shape)[1]*0.2275,iT.coords(osrc.shape)[0]*0.2275)
