...
img1 = py.open('/data/ljo31/Lens/J1619/F606W_sci_cutout_big.fits')[0].data.copy()[10:-10,10:-10]
sig1 = py.open('/data/ljo31/Lens/J1619/F606W_noisemap_big.fits')[0].data.copy()[10:-10,10:-10]
psf1 = py.open('/data/ljo31/Lens/J1619/F606W_psf2.fits')[0].data.copy()
psf1 = psf1/np.sum(psf1)
img2 = py.open('/data/ljo31/Lens/J1619/F814W_sci_cutout_big.fits')[0].data.copy()[10:-10,10:-10]
sig2 = py.open('/data/ljo31/Lens/J1619/F814W_noisemap_big.fits')[0].data.copy()[10:-10,10:-10]
psf2 = py.open('/data/ljo31/Lens/J1619/F814W_psf2.fits')[0].data.copy()
psf2 = psf2/np.sum(psf2)

guiFile = '/data/ljo31/Lens/J1619/1both5'
result = np.load('/data/ljo31/Lens/J1619/emcee28')
lp= result[0]
...
oldresult = np.load('/data/ljo31/Lens/J1619/emcee28')
oldlp= result[0]
olda1,olda3 = numpy.unravel_index(oldlp[:,0].argmax(),oldlp[:,0].shape)
olda2=0
oldtrace=oldresult[1]
olddic = oldresult[2]
...
for name in L.keys():
    ...
    for key in 'x','y','q','pa','b','eta':
        lo,hi,val = s[key]['lower'],s[key]['upper'],s[key]['value']
        val = olddic[name+' '+key][olda1,olda2,olda3]
        pars.append(pymc.Uniform('%s %s'%(name,key),lo,hi,value=val))
        cov.append(s[key]['sdev']*1)
        p[key] = pars[-1]
...
p['x'] = lenses[0].pars['x']
p['y'] = lenses[0].pars['y']
pars.append(pymc.Uniform('extShear',-0.3,0.3,value=olddic['extShear'][olda1,olda2,olda3]))
cov.append(1)
p['b'] = pars[-1]
pars.append(pymc.Uniform('extShear PA',-180.,180,value=olddic['extShear PA'][olda1,olda2,olda3]))
cov.append(100.)
p['pa'] = pars[-1]
...
#starter = np.zeros(oldtrace[-1].shape)
#starter[:,:,:12] = trace[-1,:,:,:12]
#starter[:,:,20:] = trace[-1,:,:,12:]
#starter[:,:,12:20] = oldtrace[-1,:,:,12:20]

print len(cov), len(pars)
print trace[a1].shape
import time
start=time.time()
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=22,nwalkers=72,ntemps=4,initialPars=trace[-1])
S.sample(500)
