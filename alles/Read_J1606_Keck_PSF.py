...
kresult = np.load('/data/ljo31/Lens/J1606/KeckPSF_112_ctd_ovrs')
klp= kresult[0]
ka2=0
...
for i in range(len(imgs)):
    ...
    for gal in gals:
        ...
        tmp = xc*0.
        print tmp.shape
        tmp = gal.pixeval(xin,yin,0.6/OVRS,csub=11).reshape(xc.shape) # evaulate on the oversampled grid. OVRS = number of new pixels per old pixel.
    ...
    for src in srcs:
        ...
        tmp = xc*0.
        if src.pars['q']<0.2:
            tmp = src.boxypixeval(x0,y0,0.6,csub=11,c=bar).reshape(xc.shape)
        else:
            tmp = src.pixeval(x0,y0,0.6/OVRS,csub=11).reshape(xc.shape)
