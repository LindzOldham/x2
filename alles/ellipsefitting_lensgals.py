...
name = py.open('/data/ljo31/Lens/LensParams/LensinggalaxyPhot_1src.fits')[1].data['name']
res,masses = np.load('/data/ljo31/Lens/Analysis/ReMassTrace_lensgals_110.npy')
...
for ii in range(name.size):
    ...
    if name[ii]=='J0913'  or name[ii]=='J1605':
        sigmas[ii] = ellipse(x,y,xbins,ybins,smooth=[3,2])
    elif name[ii] == 'J0837' or name[ii]=='J1606':
        sigmas[ii] = ellipse(x,y,xbins,ybins,smooth=[2,1])
    elif name[ii] == 'J1347':
        sigmas[ii] = ellipse(x,y,xbins,ybins,smooth=[0.1,0.1])
    else:
        sigmas[ii] = ellipse(x,y,xbins,ybins)
    
np.save('/data/ljo31/Lens/LensParams/ReMass_lensgals_covariances.npy',sigmas)
...
masses = np.load('/data/ljo31/Lens/Analysis/ReMassTrace_lensgals_200.npy')[:,:110]
res = np.zeros((12,110))
name = py.open('/data/ljo31/Lens/LensParams/LensinggalaxyPhot_1src.fits')[1].data['name']
for ii in range(name.size):
    Res,mags = np.load('/data/ljo31/Lens/Analysis/ReMagPDFs_lensgals_'+str(name[ii])+'.npy')
    rev,rei = Res.T
    res[ii] = rev[:110]
np.save('/data/ljo31/Lens/Analysis/ReMassTrace_lensgals_110',[res,masses])
