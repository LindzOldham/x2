...
result = np.load('/data/ljo31/Lens/J1144/emcee_restart24')
...
p['x'] = lenses[0].pars['x']
p['y'] = lenses[0].pars['y']
p['b'] = olddic['extShear'][olda1,a2,olda3]
p['pa'] = olddic['extShear PA'][olda1,a2,olda3]
...
for name in S.keys():
    ...
    if name == 'Source 1':
        for key in 'q','re','n','pa':
            s[key]['value'] = np.array(dic[name+' '+key][a1,a2,a3])
        for key in 'x','y':
            s[key]['value'] = np.array(dic['Source 2 '+key][a1,a2,a3] + olddic['Lens 1 '+key][olda1,a2,olda3]) 
    elif name == 'Source 2':
        print name
        for key in 'q','re','n','pa':
            s[key]['value'] = np.array(dic[name+' '+key][a1,a2,a3])
...
cPickle.dump([G,L,S,offsets,[guishear,True]],open('/data/ljo31/Lens/J1144/FINAL_2src_24','wb'))
