...
ages,lvs,iis,ks,mlvs,vs,lis,lks,mlbs  = np.load('/data/ljo31/Lens/Analysis/InferredAges_2src_all.npy')
...
ages,logMv,mlv,vi,vk = np.load('/data/ljo31/Lens/LensParams/InferredAges_2src.npy').T
Re,dRe,lumv,dlumv,lumi,dlumi,lumk,dlumk,dv,di,dk = np.load('/data/ljo31/Lens/LensParams/bestmodels_quantities.npy').T
#table = py.open('/data/ljo31/Lens/LensParams/Phot_1src.fits')[1].data
#Re, dlumv, dRe, name = table['Re v'], table['lum v hi'], table['Re v hi'], table['name']
logRe = np.log10(Re)
...
S.sample(4000)
outFile = '/data/ljo31/Lens/Analysis/sizemass_2src_inferredage'
f = open(outFile,'wb')
