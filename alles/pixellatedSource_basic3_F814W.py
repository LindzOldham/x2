...
psf= psf[15:-16,14:-16]
psf /= psf.sum()
img = pyfits.open('/data/ljo31/Lens/J1605/F814W_galsub2.fits')[0].data.copy()
...
outFile = '/data/ljo31/Lens/J1605/pixsrc3'
...
def showRes(x,y,src,psf,img,sig,mask,iflt,vflt,cmat,reg,niter,npix):
    ...
    img[~mask] = numpy.nan
    pylab.title('data')
    pylab.imshow(img,origin='lower',interpolation='nearest',extent=ext,vmin=0,vmax=3.5)
    pylab.colorbar()
    pylab.subplot(222)
    pylab.title('model')
    pylab.imshow(oimg,origin='lower',interpolation='nearest',extent=ext,vmin=0,vmax=3.5)
    pylab.colorbar()
    pylab.subplot(223)
    pylab.title('data-model')
    pylab.imshow((img-oimg)/sig,origin='lower',interpolation='nearest',extent=ext,vmin=-5,vmax=5)
    pylab.colorbar()
    pylab.subplot(224)
    pylab.title('model in source plane')
...
src.update(xl,yl)
osrc = showRes(xl,yl,src,PSFm,img,sig,mask,ifltm,vfltm,cmatm,0.5,10,400)
pylab.show()

np.save('/data/ljo31/Lens/J1605/osrc_F814W',osrc)
'''
reg = 1.
previousResult = None
## now fit
