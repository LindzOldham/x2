...
result = np.load('/data/ljo31/Lens/J1446/emcee3')
...
for i in range(len(imgs)):
    ...
    for gal in gals:
        ...
        if gal.pars['q']<0.25:
            print 'boxy', n
            tmp[mask2] = gal.boxypixeval(xin,yin,1./OVRS,csub=23,c=cc) # evaulate on the oversampled grid. OVRS = number of new pixels per old pixel.
        else:
            tmp[mask2] = gal.boxypixeval(xin,yin,1./OVRS,csub=23)
