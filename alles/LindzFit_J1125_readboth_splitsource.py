...
for name in S.keys():
    ...
    if name == 'Source 2':
        ...
        for key in 'x','y':
            lo,hi,val = s[key]['lower'],s[key]['upper'],s[key]['value']
            val = dic['Source 2 '+key][a1,a2,a3]+0.01
            lo,hi = lo - lenses[0].pars[key].value.item(), hi - lenses[0].pars[key].value.item()
...
# extend trace by two so we can still input it to the sampler
trace2[:,:,:,-2] = dic['Source 2 x'].copy()
trace2[:,:,:,-1] = dic['Source 2 y'].copy()
# add ten extra walkers
trace3 = np.zeros((trace2.shape[0], trace.shape[1], trace2.shape[2]+10, trace2.shape[3]))
trace3[:,:,:60,:] = trace2
trace3[:,:,60:,:] = trace2[:,:,:10,:]
print trace.shape, trace3.shape
...
jj=0
for jj in range(10):
    S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=10,nwalkers=70,ntemps=4,initialPars=trace3[a1])
    S.sample(1000)
