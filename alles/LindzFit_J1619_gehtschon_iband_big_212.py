...
X = 12 # as 11, byt just i band
#X = 13 # as 12, but jjust v band
print X
''' new!!! '''
...
import time
start=time.time()
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=30,nwalkers=60,ntemps=3)
S.sample(1000)
...
jj=0
for jj in range(30):
    S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=30,nwalkers=60,ntemps=3,initialPars=trace[-1])
    S.sample(1000)
