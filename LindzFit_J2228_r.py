...
X=0 - model3 2/1/1
X=1 - model8 2/1/2
X=2 - restarting X=1. Stopping it from hitting the n prior for source 2.
X=3 - restarting X=0.
X=4 - restarting X=2!
X=5 - restarting X=3!
X=6 - restarting X=5!
X=7 - restarting X=4! Prior on galaxy 1 y.
'''
...
guiFile = '/data/ljo31/Lens/J2228/model8'
...
result = np.load('/data/ljo31/Lens/J2228/emcee4')
...
for name in S.keys():
    ...
    if name == 'Source 2':
        ...
        for key in 'q','re','n','pa':
           lo,hi,val = s[key]['lower'],s[key]['upper'],dic[name+' '+key][a1,a2,a3]
           if key == 'n':
                pars.append(pymc.Uniform('%s %s'%(name,key),0.1,12,value=val))
           else:
                pars.append(pymc.Uniform('%s %s'%(name,key),lo,hi,value=val))
           p[key] = pars[-1]
           if key == 'pa':
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=15,nwalkers=66,ntemps=3,initialPars=trace[-1])
S.sample(1000)
outFile = '/data/ljo31/Lens/J2228/emcee'+str(X)
f = open(outFile,'wb')
