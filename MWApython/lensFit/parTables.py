...
class TableEntry:
    def __init__(self,frame,obj):
        ...
        self.f = Tk.Frame(frame)
        self.f.pack(side=Tk.LEFT)

        row0 = Tk.Frame(self.f)
        row0.grid(columnspan=6)
        lname = Tk.Label(row0,text=obj.name)
        lname.pack(side=Tk.LEFT)
        button = Tk.Button(row0,text='Delete',command=obj.delete())
        button.pack(side=Tk.LEFT)
        row = 1
        for key in obj.keys:
            TableRow(self.f,key,row,obj.pars[key])
            row += 1


class TableRow:
    def __init__(self,frame,label,row,obj,parent):
        self.obj = obj
        self.frame = frame
        self.parent = parent
        ...
        self.valueBox = Tk.Entry(self.frame,textvariable=self.valueVar,width=7)
        ...
        self.var1Box = Tk.Entry(self.frame,textvariable=self.var1,width=7)
        self.var2Box = Tk.Entry(self.frame,textvariable=self.var2,width=7)
