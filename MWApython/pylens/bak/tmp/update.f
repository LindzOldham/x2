...
c
      ...
      double precision defl(2),magmx2(2,2),magmx(2,2),
     *     defl2(2),defl3(2)
 ...
 12      write(*,*) 'Enter q,gam,s2'
         ...
         do j=1,100
            write(*,*) 'Enter x1,x2 (both = -100 to exit loop):'
            read(*,*) x1,x2               
            if ((x1+100.d0)*(x1+100.d0)+(x2+100.d0)*(x2+100.d0) 
     *           .lt. 1.d-10) goto 20
            call fastelldefl(x1,x2,q,gam,arat,s2,defl3)
            write(*,*) 'fastell Defl.= ',defl3
            call fastellmag(x1,x2,q,gam,arat,s2,defl,magmx)
            write(*,*) 'fastell mag routine: Defl.= ',defl
            write(*,*) 'and Magmx(11,22,12)= ',magmx(1,1)
            write(*,*) magmx(2,2),magmx(1,2)
            call ellipdefl(x1,x2,q,gam,arat,s2,defl2)
            write(*,*) 'slow Defl.= ',defl2
            call ellipmag(x1,x2,q,gam,arat,s2,defl2,magmx2)
            write(*,*) 'slow mag routine: Defl.= ',defl2
            write(*,*) 'and Magmx(11,22,12)= ',magmx2(1,1)
            write(*,*) magmx2(2,2),magmx2(1,2)
            call ellipphi(x1,x2,q,gam,arat,s2,phi)
            write(*,*) 'slow Phi= ',phi
         enddo
 20   enddo
 30   end
...
c      SUBROUTINE ROMINT ( VAL, ERR, EPS, A, B, N, MAXE, F )
c*********************************************************************72
c
cc ROMINT
c
      IMPLICIT NONE
      double precision A,B,BB,tol,ERR,R,S,S0,S1,T,ans,F,H
      double precision RM(16)
      external F
      integer J,K,K0,K1,K2,KK,KKK,L,N,N0,N1,MAXJ
C
