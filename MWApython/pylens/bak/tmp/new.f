...
c     (arat), we make a slight change:      
     ...
     *        fpe2*d1e2-d1s*mres(2)+e1g1*fpe1/x1)
      ...
      subroutine powerLawDeflections(x,y,reff,eta,s,q,xout,yout,n)
...
!f2py integer intent(hide),depend(x) :: n=len(x)
      double precision x(n),y(n),xout(n),yout(n),defl(2)
      double precision reff,eta,s,q
      double precision q0,gam,arat,s2

      write(*,*) 'blah!',n
      gam = eta*0.5
      arat = q
      s2 = s*s
      q0 = (1.0-gam)*reff**(2.0*gam)/(q**gam)
      do i=1,n
         call fastelldefl(x(i),y(i),q0,gam,arat,s2,defl)
         xout(i) = defl(1)
         yout(i) = defl(2)
      enddo
      end
...
c
      implicit double precision (a-h,o-z)
!f2py double precision intent(out) :: defl(2)
      double precision defl(2),ares(2)
      common /ellfirsttime/ ifirst1,ifirst2
      if ((ifirst1 .eq. 0) .and. (ifirst2 .eq. 0)) then
c         write(*,*) 'Initializing FASTELL routines'
         call fastellprep()
         ifirst1=1
         ifirst2=1
      endif         
