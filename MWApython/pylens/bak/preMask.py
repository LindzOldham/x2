...
def lensFit(inpars,image,sig,gals,lenses,sources,xc,yc,OVRS=1,csub=11,psf=None,mask=None,noResid=False,verbose=False,getModel=False):
    ...
    n = 0
    model = numpy.empty((len(gals)+len(sources),xc.size))
    for gal in gals:
        gal.setPars(inpars)
        gal.amp = 1
        tmp = gal.pixeval(xc,yc,1./OVRS,csub=csub)
    ...
    x0,y0 = pylens.lens_images(lenses,sources,[xc,yc],1./OVRS,getPix=True)
    ...
    rhs = (image/sig).flatten()
    op = (model/sig.ravel()).T
