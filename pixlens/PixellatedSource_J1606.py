...
img1 = py.open('/data/ljo31/Lens/J1606/galsub_0.fits')[0].data.copy()
...
# and a mask
mask = py.open('/data/ljo31/Lens/J1606/maskforpix.fits')[0].data.copy()
mask = mask==1
...
def showRes(x,y,src,psf,img,sig,mask,iflt,vflt,cmat,reg,niter,npix):
    ...
    pylab.subplot(221)
    img[~mask] = numpy.nan
    pylab.imshow(img[150:-177,165:-168],origin='lower',interpolation='nearest',extent=ext,vmin=0,vmax=1,cmap='jet',aspect='auto')
    pylab.colorbar()
    pylab.subplot(222)
    pylab.imshow(oimg[150:-177,165:-168],origin='lower',interpolation='nearest',extent=ext,vmin=0,vmax=1,cmap='jet',aspect='auto')
    pylab.colorbar()
    pylab.subplot(223)
    pylab.imshow((img[150:-177,165:-168]-oimg[150:-177,165:-168])/sig[150:-177,165:-168],origin='lower',interpolation='nearest',extent=ext,vmin=-3,vmax=3,cmap='jet',aspect='auto')
    pylab.colorbar()
    pylab.subplot(224)
    ...
    pylab.colorbar()
    pylab.subplot(122)
    pylab.imshow(osrc[:,:-30],origin='lower',interpolation='nearest',extent=ext2,vmin=0,vmax=2,cmap='jet',aspect='auto')
    pl.gca().xaxis.set_major_locator(pl.NullLocator())
    pl.gca().yaxis.set_major_locator(pl.NullLocator())
    pylab.colorbar()
    return osrc
