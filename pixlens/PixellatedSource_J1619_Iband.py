...
img1 = py.open('/data/ljo31/Lens/J1619/galsub_1.fits')[0].data.copy()
...
# and a mask
mask = py.open('/data/ljo31/Lens/J1619/maskforpix_I.fits')[0].data.copy()
mask = mask==1
...
def showRes(x,y,src,psf,img,sig,mask,iflt,vflt,cmat,reg,niter,npix):
    ...
    pylab.subplot(221)
    img[~mask] = numpy.nan
    pylab.imshow(img[125:-120,118:-123],origin='lower',interpolation='nearest',extent=ext,vmin=0,vmax=2,cmap='jet',aspect='auto')
    pylab.colorbar()
    pylab.subplot(222)
    pylab.imshow(oimg[125:-120,118:-123],origin='lower',interpolation='nearest',extent=ext,vmin=0,vmax=2,cmap='jet',aspect='auto')
    pylab.colorbar()
    pylab.subplot(223)
    pylab.imshow((img[125:-120,118:-123]-oimg[125:-120,118:-123])/sig[125:-120,118:-123],origin='lower',interpolation='nearest',extent=ext,vmin=-3,vmax=3,cmap='jet',aspect='auto')
    pylab.colorbar()
    pylab.subplot(224)
    pylab.imshow(osrc,origin='lower',interpolation='nearest',extent=ext2,vmin=0,vmax=7,cmap='jet',aspect='auto')
    pylab.colorbar()
    ...
    pylab.imshow(oimg[131:-123,121:-128],origin='lower',interpolation='nearest',extent=ext,vmin=0,vmax=2,cmap='jet',aspect='auto')
    #ax = pl.axes()
    #ax.set_xticklabels([])
    #ax.set_yticklabels([])
    ...
    pylab.colorbar()
    pylab.subplot(122)
    pylab.imshow(osrc,origin='lower',interpolation='nearest',extent=ext2,vmin=0,vmax=8,cmap='jet',aspect='auto')
    pl.gca().xaxis.set_major_locator(pl.NullLocator())
    pl.gca().yaxis.set_major_locator(pl.NullLocator())
    pylab.colorbar()
    return osrc
