...
def showRes(x,y,src,psf,img,sig,mask,iflt,vflt,cmat,reg,niter,npix,vmx,band):
    ...
    lmat = psf*src.lmat
    rmat = src.rmat
    res,fit,model,rhs,regg = aT.getModelG(iflt,vflt,lmat,cmat,rmat,2.9,niter=niter)
    res,fit2,model,rhs,regg = aT.getModelG(iflt,vflt,lmat,cmat,rmat,0.9,niter=niter)
    ...
    pl.subplot(132)
    resid = (img-oimg)/sig
    resid[20:28,26:39] = np.random.randn(8,13)
    ...
    pl.imshow(osrc[100:-100,100:-100],origin='lower',interpolation='nearest',extent=ext,vmin=0,vmax=1.5,cmap='jet',aspect='auto')
