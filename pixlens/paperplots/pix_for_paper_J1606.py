...
def showRes(x,y,src,psf,img,sig,mask,iflt,vflt,cmat,reg,niter,npix,vmx,band):
    ...
    lmat = psf*src.lmat
    rmat = src.rmat
    res,fit,model,rhs,regg = aT.getModelG(iflt,vflt,lmat,cmat,rmat,10.8,niter=niter)
    ...
    pl.imshow(oimg,origin='lower',interpolation='nearest',extent=ext,vmin=0,vmax=0.5,cmap='jet',aspect='auto')
...
pix_mask += py.open('/data/ljo31b/EELs/galsub/masks/'+name+'_3.fits')[0].data.copy()[my-YY:my+YY,mx-XX:mx+XX]
