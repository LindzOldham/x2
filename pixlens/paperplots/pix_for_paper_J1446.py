...
def showRes(x,y,src,psf,img,sig,mask,iflt,vflt,cmat,reg,niter,npix,vmx,band):
    ...
    lmat = psf*src.lmat
    rmat = src.rmat
    res,fit,model,rhs,regg = aT.getModelG(iflt,vflt,lmat,cmat,rmat,5.5,niter=niter)
    _,fit2,_,_,_ = aT.getModelG(iflt,vflt,lmat,cmat,rmat,3.5,niter=niter)
    ...
    pl.subplot(132)
    resid = (img-oimg)/sig
    resid[16:23,20:36] = np.random.randn(7,16)
...
pix_mask = py.open('/data/ljo31b/EELs/galsub/masks/'+name+'.fits')[0].data.copy()[my-YY:my+YY,mx-XX:mx+XX]
pix_mask -= py.open('/data/ljo31b/EELs/galsub/masks/'+name+'in.fits')[0].data.copy()[my-YY:my+YY,mx-XX:mx+XX]
