...
def showRes(x,y,src,psf,img,sig,mask,iflt,vflt,cmat,reg,niter,npix,vmx,band):
    ...
    lmat = psf*src.lmat
    rmat = src.rmat
    res,fit,model,rhs,regg = aT.getModelG(iflt,vflt,lmat,cmat,rmat,0.4,niter=niter)
    _,fit2,_,_,_ = aT.getModelG(iflt,vflt,lmat,cmat,rmat,reg,niter=niter)
    oy +=0.7
    ox -=0.7
    ...
    pl.imshow(osrc[100:-100,100:-100],origin='lower',interpolation='nearest',extent=ext,vmin=0,vmax=2.0,cmap='jet',aspect='auto')
