...
def showRes(x,y,src,psf,img,sig,mask,iflt,vflt,cmat,reg,niter,npix,vmx,band):
    ...
    lmat = psf*src.lmat
    rmat = src.rmat
    res,fit,model,rhs,regg = aT.getModelG(iflt,vflt,lmat,cmat,rmat,1.6,niter=niter)
    res,fit2,model,rhs,regg = aT.getModelG(iflt,vflt,lmat,cmat,rmat,1.6,niter=niter)
    ...
    pl.subplot(132)
    resid = (img-oimg)/sig
    resid[26:32,36:44] = np.random.randn(6,8)
