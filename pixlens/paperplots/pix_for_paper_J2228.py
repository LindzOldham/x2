...
def showRes(x,y,src,psf,img,sig,mask,iflt,vflt,cmat,reg,niter,npix,vmx,band):
    ...
    lmat = psf*src.lmat
    rmat = src.rmat
    res,fit,model,rhs,regg = aT.getModelG(iflt,vflt,lmat,cmat,rmat,14.7,niter=niter)
    ox +=2
    osrc = src.eval(ox.ravel(),oy.ravel(),fit).reshape(ox.shape)
    _,fit2,_,_,_ = aT.getModelG(iflt,vflt,lmat,cmat,rmat,0.7,niter=niter)
    ...
    pl.subplot(132)
    resid = (img-oimg)/sig
    resid[20:32,17:25] = np.random.randn(12,8)
    ...
    pl.imshow(osrc[100:-100,100:-100],origin='lower',interpolation='nearest',extent=ext,vmin=0,vmax=1.,cmap='jet',aspect='auto')
