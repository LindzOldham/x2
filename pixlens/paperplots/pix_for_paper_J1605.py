...
def showRes(x,y,src,psf,img,sig,mask,iflt,vflt,cmat,reg,niter,npix,vmx,band):
    ...
    lmat = psf*src.lmat
    rmat = src.rmat
    res,fit,model,rhs,regg = aT.getModelG(iflt,vflt,lmat,cmat,rmat,110.,niter=niter)
    ox +=2
    ...
    pl.imshow(oimg,origin='lower',interpolation='nearest',extent=ext,vmin=0,vmax=0.2,cmap='jet',aspect='auto')
