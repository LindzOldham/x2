...
def showRes(x,y,src,psf,img,sig,mask,iflt,vflt,cmat,reg,niter,npix,vmx,band):
    ...
    lmat = psf*src.lmat
    rmat = src.rmat
    res,fit,model,rhs,regg = aT.getModelG(iflt,vflt,lmat,cmat,rmat,8.5,niter=niter)
    ox -=1
    oy -= 0.5
    ...
    pl.subplot(132)
    resid = (img-oimg)/sig
    resid[16:20,19:22] = np.random.randn(4,3)
    ...
    yy = np.ones(xx.size)*7

    pl.plot(xx,yy,color='white',lw=4)
    pl.imshow(osrc[100:-100,100:-100],origin='lower',interpolation='nearest',extent=ext,vmin=0,vmax=1.0,cmap='jet',aspect='auto')
...
def MakeModel(name,result,Npnts=1):
    ...
    for ii in range(len(imgs)-1):
        ...
        xl,yl = pylens.getDeflections(lenses,coord)
        srcs[ii].update(xl,yl)
        osrc = showRes(xl,yl,srcs[ii],PSFm,img,sig,pix_mask,ifltm,vfltm,cmatm,2,0,400,vmx = vmxs[ii],band=bands[ii])
        pl.show()
...
pix_mask = py.open('/data/ljo31b/EELs/galsub/masks/'+name+'_1.fits')[0].data.copy()[my-XX:my+XX,mx-XX:mx+XX]
