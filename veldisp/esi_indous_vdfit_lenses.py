...
triangle=False
filename = name+'_'+wid+'_'+typ+'_esi_indous_vdfit_LENS'#_BC_6'
...
def run(zl,zs,fit=True,read=False,File=None,mask=None,nfit=6,bg='polynomial',bias=100,restmask=None,wid=1.,typ=None):
    # Load in spectrum
    if typ=='lens':
        print 'lens'
...
if name=='J0837':
    ...
    result = run(lz[name],sz[name],fit=True,read=False,File = filename,typ=typ,wid=wid,nfit=10)
    result = run(lz[name],sz[name],fit=False,read=True,File = filename,typ=typ,wid=wid,nfit=10)
...
# may 2014 dataset
    ...
    result = run(lz[name],sz[name],fit=True,read=False,File = filename,wid=wid,typ=typ,nfit=10)
    result = run(lz[name],sz[name],fit=False,read=True,File = filename,wid=wid,typ=typ,nfit=10)
    ...
    result = run(lz[name[8:]],sz[name[8:]],fit=True,read=False,File = filename,wid=wid,typ=typ,mask=np.array([[7580,7700],[5570,5585],[6860,6900],[7250,7580]]))
    result = run(lz[name[8:]],sz[name[8:]],fit=False,read=True,File = filename,wid=wid,typ=typ,mask=np.array([[7580,7700],[5570,5585],[6860,6900],[7250,7580]]))
    ...
    result = run(lz[name[8:]],sz[name[8:]],fit=True,read=False,File = filename,wid=wid,typ=typ)
    result = run(lz[name[8:]],sz[name[8:]],fit=False,read=True,File = filename,wid=wid,typ=typ)
...
#pl.savefig('/data/ljo31/public_html/Lens/sdss_eels_spectra/texplots/fits_esi_indous/'+filename+'.png')
#pl.show()
