...
lenspec = py.open('/data/ljo31b/EELs/esi/testcode/spec-4005-55325-0039.fits')
sourcespec = py.open('/data/ljo31b/EELs/esi/testcode/spec-4005-55325-0063.fits')
...
np.save('/data/ljo31b/EELs/esi/INDOUSmodels_set2',model)'''
...
def run(zl,zs,fit=True,read=False,File=None,mask=None,lim=5000.,nfit=6.,bg='polynomial',bias=1e8,restmask=None,srclim=6000.,lenslim=5500.):
    # Load in spectrum
    sciwave,scispec,varspec = np.load('/data/ljo31b/EELs/esi/testcode/summedspec_set2_neu_neu.npy')

    # cut nonsense data - nans at edges
    edges = np.where(np.isnan(scispec)==False)[0]
    start = edges[0]
    end = np.where(sciwave>np.log10(8500.))[0][0]#scispec.size
    ...
    t1 = np.load('/data/ljo31b/EELs/esi/INDOUSmodels_set2.npy')    
    tmpwave1 = wave.copy()
...
#result = run(0.466498, 0.64319,fit=True,read=False,File = name,mask=np.array([[5560,5620],[6300,6320],[7225,7280],[7580,7700],[6860,6900]]),nfit=6,bg='polynomial',lim=5500.,bias=1e10,srclim=6100.,lenslim=5700.)
#result = run(0.466498, 0.64319,fit=False,read=True,File = name,mask=np.array([[5560,5620],[6300,6320],[7225,7280],[7580,7700],[6860,6900]]),nfit=6,bg='polynomial',lim=5500.,bias=1e10,srclim=6100.,lenslim=5700.)

#result = run(0.466498, 0.64319,fit=True,read=False,File = name,mask=np.array([[5560,5620],[6300,6320],[7225,7280],[7580,7700],[6860,6900]]),nfit=6,bg='polynomial',lim=4500.,bias=1e10,srclim=6100.,lenslim=5700.)
result = run(0.466498, 0.64319,fit=False,read=True,File = name,mask=np.array([[5560,5620],[6300,6320],[7225,7280],[7580,7700],[6860,6900]]),nfit=6,bg='polynomial',lim=4500.,bias=1e10,srclim=6100.,lenslim=5700.)
pl.title(name[:5])
...
pl.subplot(221)
key = 'lens velocity'
pl.hist(dic[key][200:,lp[300]>-1640].ravel(),30,histtype='stepfilled')
pl.title(key)
pl.subplot(222)
key = 'lens dispersion'
pl.hist(dic[key][200:,lp[300]>-1640].ravel(),30,histtype='stepfilled')
