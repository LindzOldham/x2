...
def getmodel_gen(twave,tspec,tscale,sigsci,sigtmp,smin=5.,smax=501):
    def func(disp,wl):
        sigtmp = 3./10**wl / 2.355 * light
        dispkern = (disp**2.+sigsci**2.-sigtmp**2.)**0.5
        kernel = dispkern/(light*ln10*tscale)
        print kernel
        gauss = np.exp(-0.5*lin**2./kernel**2.)
        return gauss
    match = tspec.copy()
    lin = 10**twave
    disps = np.arange(smin,smax,VGRID)
    ...
    for i in range(disps.size):
        disp = disps[i]
        cube[i] = ndimage.generic_filter1d(match.copy(),func)
    X = disps.tolist()
    tx = np.array([X[0]]+X+[X[-1]])
    Y = twave.tolist()
    ty = np.array([Y[0]]+Y+[Y[-1]])
...
def run(zl,zs,fit=True,read=False,File=None,mask=None,lim=6000.,nfit=6.,bg='polynomial',bias=1e8):
    ...
    for ii in idx[0]:
        ...
        t.append(getmodel(twave,tmpspec,tmpscale,sigsci,3.))
