...
def finddispersion(scispec,varspec,t1,t2,twave1,twave2,outwave,zl,zs,nfit=6,outfile=None,mask=None,bias=1e8,lim=4000.,bg='polynomial',restmask=None,srclim=6000.,lenslim=5500.):
    ...
    def logprob(value=0.,pars=pars):
        ...
        for k in range(ntemps2):
            a = interpolate.bisplev(sL.value,np.log10(twave2),t2[k])
    ...
    def logp(value=0.,lp=logprob):
        return lp
    print outfile
    S = myEmcee.Emcee(pars+[logp],cov=cov,nthreads=8,nwalkers=40)
    S.sample(500)
    ...
    for k in range(ntemps1):
        ...
        oper[k+ntemps1,~cond] = interpolate.splev(outwave[~cond],modb)  
    for k in range(ntemps2):
        a = interpolate.bisplev(sigL,np.log10(twave2),t2[k])
        moda = interpolate.splrep(owL2,a)
        oper[k+2*ntemps1,lenscond] = interpolate.splev(outwave[lenscond],moda)
