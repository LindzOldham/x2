...
def finddispersion(scispec,varspec,tmpspec,tmpwave,outwave,zl,zs,nfit=6,outfile=None,mask=None,bias=1e8,lim=5200.,bg='polynomial',nsrc=6,smooth='polymonial',bias2=1e8):
    ...
    vL = pymc.Uniform('lens velocity',-350.,350.,value=-200.)
    sL = pymc.Uniform('lens dispersion',5.,501.,value=300.)
    vS = pymc.Uniform('source velocity',-350.,350.,value=-200.)
    sS = pymc.Uniform('source dispersion',5.,501.,value=300.)
    pars = [vL,sL,vS,sS]
    ...
    S = myEmcee.Emcee(pars+[logp],cov=cov,nthreads=30,nwalkers=40)
    S.sample(500)
...
def readresults(scispec,varspec,tmpspec,tmpwave,outwave,zl,zs,nfit=6,infile=None,mask=None,bias=1e8,lim=6000.,bg='polynomial',nsrc=6,smooth='polynomial',bias2=1e8):
    outwave,scispec,varspec = outwave[outwave>log10(lim)], scispec[outwave>log10(lim)], varspec[outwave>log10(lim)]
    ...
    #srccont = scipy.dot(oper[2*ntemps:2*ntemps].T,fit[2*ntemps:2*ntemps])
    print outmodel
    pl.figure()
    if mask is not None:
        for M in mask:
