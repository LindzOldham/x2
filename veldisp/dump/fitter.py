...
def finddispersion(scispec,varspec,tmpspec,tmpwave,outwave,zl,zs,nfit=6,outfile=None,mask=None,bias=1e8,lim=6000.,bg='polynomial'):
    ...
    pretend = 10**outwave.copy()
    pretend -= 10**outwave[0]
    pretend /= pretend[-1]
    pretend -= 0.5
    ...
    if mask is not None:
        ...
        for M in mask:
            ...
            ma[cond]=0
        mask=ma==1
        outwave,scispec,varspec,pretend = outwave[mask],scispec[mask],varspec[mask],pretend[mask]
    isig = 1./varspec**0.5
    ntemps = len(tmpspec)
...
def readresults(scispec,varspec,tmpspec,tmpwave,outwave,zl,zs,nfit=6,infile=None,mask=None,bias=1e8,lim=6000.,bg='polynomial'):
    ...
    if mask is not None:
        ...
        for M in mask:
            ...
            ma[cond]=0
        ma=ma==1
        outwave,scispec,varspec,pretend = outwave[ma],scispec[ma],varspec[ma],pretend[ma]
    isig = 1./varspec**0.5
    ntemps = len(tmpspec)
    ...
    if mask is not None:
        # Create the polynomial fit components
        pretend = 10**origwave.copy()
        pretend -= 10**origwave[0]
        pretend /= pretend[-1]
        pretend -= 0.5
