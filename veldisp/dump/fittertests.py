...
def finddispersion(scispec,varspec,tmpspec,tmpwave,datascale,tscale,outwave,zl,zs,nfit=6,outfile=None,mask=None,bias=1e8,lim=6000.,bg='polynomial'):
    ...
    #vL = pymc.Uniform('lens velocity',-350.,350.,value=0)
    #sL = pymc.Uniform('lens dispersion',5.,501.)
    vL,sL = -225.,250.
    vS = pymc.Uniform('source velocity',-350.,350.,value=0)
    ...
    def logprob(value=0.,pars=pars):
        owL,owS = np.log10(tmpwave*(1.+zl+vL/light)), np.log10(tmpwave*(1.+zs+vS.value/light))
        resL,resS = sL/light, sS.value/light
    ...
    S = myEmcee.Emcee(pars+[logp],cov=cov,nthreads=7,nwalkers=40)
    S.sample(400)
...
def readresults(scispec,varspec,tmpspec,tmpwave,datascale,tscale,outwave,zl,zs,nfit=6,infile=None,mask=None,bias=1e8,lim=6000.,bg='polynomial'):
    outwave,scispec,varspec = outwave[outwave>log10(lim)], scispec[outwave>log10(lim)], varspec[outwave>log10(lim)]
