...
if name=='J0837':
    ...
    print sigsci
    filename = name+'_'+wid+'_'+typ+'_9000_sky'
    ...
    sigsci = resolutions[name] * light#* np.log(10.)
    print sigsci
    filename = name+'_'+wid+'_'+typ+'_8500_sky'
    result = run(0.311,0.586,fit=True,read=False,File = filename,mask=np.array([[7580,7700],[6860,6900]]),nfit=6,bg='polynomial',lim=4500,bias=1e2,srclim=5800.,lenslim=5000.,typ=typ,wid=wid)
    result = run(0.311,0.586,fit=False,read=True,File = filename,mask=np.array([[7580,7700],[6860,6900]]),nfit=6,bg='polynomial',lim=4500,bias=1e2,srclim=5800.,lenslim=5000.,typ=typ,wid=wid)
    pl.title(name[:5])
    pl.savefig('/data/ljo31b/EELs/esi/kinematics/plots/apertures/final/'+filename+'.pdf')
