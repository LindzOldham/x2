...
from stitchfitter3 import readresults as readresults3
...
def run_wide(zl,zs,fit=True,read=False,File=None,mask=None,lim=5000.,nfit=6.,bg='polynomial',bias=1e8,restmask=None,srclim=6000.,lenslim=5500.):
    ...
    result = readresults3(scispec,varspec,t1,t2,tmpwave1,tmpwave2,np.log10(sciwave),zl,zs,nfit=nfit,infile=File,mask=mask,lim=lim,bg=bg,restmask=restmask,srclim=srclim,lenslim=lenslim)
    return result
...
names = ['J1248','J1323','J1347','J1446','J1605','J1606','J1619','J2228']
...
for name in names:
    ...
    if name=='J1248':
        ...
        pl.title(name+' small aperture')
        run_wide(lz[name[:5]],sz[name[:5]],fit=False,read=True,File = 'wide/'+name,mask=np.array([[7580,7700],[6860,6900]]),nfit=6,bg='polynomial',lim=4550.,bias=1e8,srclim=5300.,lenslim=4600.)
        pl.title(name[:5])
        pl.savefig('/data/ljo31b/EELs/esi/kinematics/plots/comp_'+name[:5]+'.pdf')
        pl.show()
