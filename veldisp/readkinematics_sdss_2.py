...
names = ['J1605','J1606','J1446','J1619','J1323','J1347','J2228']
...
for name in names:
    ...
    #print zl,zs
    print name, '& $',
    #result = np.load('/data/ljo31b/EELs/esi/kinematics/inference/zv_stitch2_sdss_'+name)
    result = np.load('/data/ljo31b/EELs/esi/kinematics/inference/zv_sdss_'+name)

    lp,trace,dic,_ = result
    #oresult = np.load('/data/ljo31b/EELs/esi/kinematics/inference/notvar_stitch2_sdss_'+name)#np.load('/data/ljo31b/EELs/esi/kinematics/inference/apertures/final/'+name+'_1.00_lens')
    oresult = np.load('/data/ljo31b/EELs/esi/kinematics/inference/apertures/final/'+name+'_1.00_lens_vac')
    olp,otrace,odic,_ = oresult
    
    if name == 'J2228':
        otrace = otrace[:,olp[450]>-10200]
        for key in odic.keys():
            odic[key] = odic[key][:,olp[450]>-10200]
        olp = olp[:,olp[450]>-10200]
    ...
    #np.savetxt('/data/ljo31b/EELs/esi/kinematics/inference/rest_stitch2_sdss_chain_'+name,trace[100:].reshape((trace[100:].shape[0]*trace.shape[1],trace.shape[-1])))
    #np.savetxt('/data/ljo31b/EELs/esi/kinematics/inference/vacesi_chain_'+name+'_9000',otrace[200:].reshape((otrace[200:].shape[0]*otrace.shape[1],otrace.shape[-1])))
    
    i.CornerPlotter(['/data/ljo31b/EELs/esi/kinematics/inference/vacesi_chain_'+name+'_9000,blue','/data/ljo31b/EELs/esi/kinematics/inference/rest_stitch2_sdss_chain_'+name+',red'])
    pl.figtext(0.65,0.65,name,fontsize=30)
    pl.savefig('/data/ljo31/public_html/Lens/sdss_eels_spectra/'+name+'vaccorner.png')
