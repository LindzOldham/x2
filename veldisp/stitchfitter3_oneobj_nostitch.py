...
def finddispersion(scispec,varspec,t1,twave1,outwave,z,nfit=6,outfile=None,mask=None,bias=1e8,lim=4000.,bg='polynomial',restmask=None,lenslim=5500.):
    ...
    operator = scipy.zeros((scispec.size,ntemps1+nfit))
...
def readresults(scispec,varspec,t1,twave1,outwave,z,nfit=6,infile=None,mask=None,bias=1e8,lim=5000.,bg='polynomial',restmask=None,lenslim=5500.):
    outwave,scispec,varspec = outwave[outwave>log10(lim)], scispec[outwave>log10(lim)], varspec[outwave>log10(lim)]
   ...
   # Create the polynomial fit components
    ...
    if mask is not None or restmask is not None:
        operator = scipy.zeros((origsci.size,ntemps1+nfit))
    ...
    cont = scipy.dot(oper[ntemps1:].T,fit[ntemps1:])
