...
if name=='J1248':
    ...
    filename='wide/test_'+name
    sigsci = resolutions[name[:5]] * light
   
    result = run(lz[name[:5]],sz[name[:5]],fit=True,read=False,File = filename,mask=np.array([[7580,7700],[6860,6900]]),nfit=6,bg='polynomial',lim=4750.,bias=1e10,srclim=6000.,lenslim=5200.)
    result = run(lz[name[:5]],sz[name[:5]],fit=False,read=True,File = filename,mask=np.array([[7580,7700],[6860,6900]]),nfit=6,bg='polynomial',lim=4750.,bias=1e10,srclim=6000.,lenslim=5200.)
    pl.title(name[:5])
    pl.savefig('/data/ljo31b/EELs/esi/kinematics/plots/'+name[:5]+'.pdf')
    pl.show()

elif name == 'J1446':
    name = 'J1446_stitch_2_wide'
    sigsci = resolutions[name[:5]] * light
