...
from stitchfitter3_oneobj import finddispersion,readresults
...
#sigsci = 21.
sourcespec = py.open('/data/ljo31b/EELs/esi/testcode/spec-4056-55357-0087.fits')
lenspec = py.open('/data/ljo31b/EELs/esi/testcode/spec-4056-55357-0119.fits')

wdisp = lenspec[1].data['wdisp']
sig = wdisp * 0.0001*np.log(10.)
...
def run(z,fit=True,read=False,File=None,mask=None,lim=5000.,nfit=6.,bg='polynomial',bias=1e8,restmask=None,lenslim=5500.):
    # Load in spectrum
    spec1 = py.open('/data/ljo31b/EELs/esi/testcode/spec-4056-55357-0119.fits')[1].data
    sciwave,scispec,varspec = spec1['loglam'],spec1['flux'],spec1['ivar']**-1
    ...
    pl.plot(sciwave,scispec)
    pl.figure()
    pl.plot(sciwave,scispec/varspec**0.5)
    pl.show()
    t1,t2 = [],[]
    ...
    if fit:
        result = finddispersion(scispec,varspec,t1,t2,tmpwave1,tmpwave2,np.log10(sciwave),z,nfit=nfit,outfile=File,mask=mask,lim=lim,bg=bg,restmask=restmask,lenslim=lenslim)
        return result
    elif read:
        result = readresults(scispec,varspec,t1,t2,tmpwave1,tmpwave2,np.log10(sciwave),z,nfit=nfit,infile=File,mask=mask,lim=lim,bg=bg,restmask=restmask,lenslim=lenslim)
        return result
    else:
        return
...
result = run(0.392347,fit=True,read=False,File = name,mask=np.array([[5560,5620],[6300,6320],[7225,7280],[7580,7700],[6860,6900]]),nfit=10,bg='polynomial',lim=4500.,bias=1e9,lenslim=4610.)
result = run(0.392347,fit=False,read=True,File = name,mask=np.array([[5560,5620],[6300,6320],[7225,7280],[7580,7700],[6860,6900]]),nfit=10,bg='polynomial',lim=4500.,bias=1e9,lenslim=4610.)
