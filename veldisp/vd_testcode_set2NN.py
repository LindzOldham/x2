...
def run(zl,zs,fit=True,read=False,File=None,mask=None,lim=6000.,nfit=6.,bg='polynomial',bias=1e8):
    # Load in spectrum
    sciwave,scispec,varspec = np.load('/data/ljo31b/EELs/esi/testcode/summedspec_set2_neu_neu.npy') # varspec is sigma
...
name='testcodehitrueres3_NEW'#_hires'
...
result = run(0.466498, 0.64319,fit=True,read=False,File = name,mask=np.array([[6300,6320],[7580,7700],[6860,6900]]),nfit=6,bg='polynomial',lim=6100.,bias=1e6)
result = run(0.466498, 0.64319,fit=False,read=True,File = name,mask=np.array([[6300,6320],[7580,7700],[6860,6900]]),nfit=6,bg='polynomial',lim=6100.,bias=1e6)
...
pl.show()
'''
result = np.load('/data/ljo31b/EELs/esi/kinematics/trials/testcodehitrueres3_NEW')
