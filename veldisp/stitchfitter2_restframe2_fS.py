...
def finddispersion(scispec,varspec,t1,t2,twave1,twave2,outwave,zl,zs,nfit=6,outfile=None,mask=None,bias=1e8,lim=4000.,bg='polynomial',restmask=None,srclim=6000.,lenslim=5500.,name=None):
    ...
    vS=dic['source velocity'][a1,a2]
    sS=dic['source dispersion'][a1,a2]
    ...
    def logprob(value=0.,pars=pars):
        zL, zS = np.log10(1.+zl+vL.value/light), np.log10(1.+zs+vS/light)
        for k in range(ntemps1):
            oper[k,~lenscond] = interpolate.bisplev(sL.value,outwave[~lenscond]-zL,t1[k])
            oper[k+ntemps1,~cond] = interpolate.bisplev(sS,outwave[~cond]-zS,t1[k])
        for k in range(ntemps2):
            oper[k+2*ntemps1,lenscond] = interpolate.bisplev(sL.value,outwave[lenscond]-zL,t2[k])
            oper[k+2*ntemps1+ntemps2,cond] = interpolate.bisplev(sS,outwave[cond]-zS,t2[k])  
        op = (oper*isig).T
