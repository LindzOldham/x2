...
def run(zl,zs,fit=True,read=False,File=None,mask=None,lim=5200.,nfit=6.,bg='polynomial',nsrc=5,smooth='polynomial',bias=1e8,bias2=1e8):
    ...
    if fit:
        result = finddispersion(scispec,varspec,t1,t2,tmpwave1,tmpwave2,np.log10(sciwave),zl,zs,nfit=nfit,outfile=File,mask=mask,lim=lim,bg=bg)
        return result
    elif read:
        result = readresults(scispec,varspec,t1,t2,tmpwave1,tmpwave2,np.log10(sciwave),zl,zs,nfit=nfit,infile=File,mask=mask,lim=lim,bg=bg)
        return result
    else:
        return
...
name = '6'
print name
result = run(0.4256,0.6411,fit=True,read=False,File = 'J0837_'+name,mask=np.array([[7580,7700],[6860,6900]]),nfit=6,bg='polynomial',lim=5500,bias=1e8,srclim=6000.)
result = run(0.4246,0.6411,fit=False,read=True,File = 'J0837_'+name,mask=np.array([[7580,7700],[6860,6900]]),nfit=6,bg='polynomial',lim=5500,bias=1e8,srclim=6000.)
