...
def finddispersion(scispec,varspec,t1,t2,twave1,twave2,outwave,zl,zs,nfit=6,outfile=None,mask=None,bias=1e8,lim=5200.,bg='polynomial',srclim=6000.):
    ...
    vL = pymc.Uniform('lens velocity',-3050.,4050.)#,value=2200.)
    sL = pymc.Uniform('lens dispersion',5.,501.)
    vS = pymc.Uniform('source velocity',-550.,550.)
    ...
    def logprob(value=0.,pars=pars):
        owL1,owS1 = np.log10(twave1*(1.+zl+vL.value/light)), np.log10(twave1*(1.+zs+vS.value/light))
        owL2,owS2 = np.log10(twave2*(1.+zl+vL.value/light)), np.log10(twave2*(1.+zs+vS.value/light))
        for k in range(ntemps1):
            a = interpolate.bisplev(sL.value,np.log10(twave1),t1[k])
            moda = interpolate.splrep(owL1,a)
            oper[k] = interpolate.splev(outwave,moda)
            b = interpolate.bisplev(sS.value,np.log10(twave1),t1[k])
            modb = interpolate.splrep(owS1,b)
            oper[k+ntemps1,~cond] = interpolate.splev(outwave[~cond],modb)  
        for k in range(ntemps2):
            b = interpolate.bisplev(sS.value,np.log10(twave2),t2[k])
            modb = interpolate.splrep(owS2,b)
            oper[k+2*ntemps1,cond] = interpolate.splev(outwave[cond],modb)  
        op = (oper*isig).T
    ...
    owL1,owS1 = np.log10(twave1*(1.+zl+velL/light)), np.log10(twave1*(1.+zs+velS/light))
    owL2,owS2 = np.log10(twave2*(1.+zl+velL/light)), np.log10(twave2*(1.+zs+velS/light))
    for k in range(ntemps1):
        a = interpolate.bisplev(sigL,np.log10(twave1),t1[k])
        moda = interpolate.splrep(owL1,a)
        oper[k] = interpolate.splev(outwave,moda)
        b = interpolate.bisplev(sigS,np.log10(twave1),t1[k])
        modb = interpolate.splrep(owS1,b)
        oper[k+ntemps1,~cond] = interpolate.splev(outwave[~cond],modb)  
    for k in range(ntemps2):
        b = interpolate.bisplev(sigS,np.log10(twave2),t2[k])
        modb = interpolate.splrep(owS2,b)
        oper[k+2*ntemps1,cond] = interpolate.splev(outwave[cond],modb)  
    op = (oper*isig).T
    rhs = (scispec+BIAS)*isig
...
def readresults(scispec,varspec,t1,t2,twave1,twave2,outwave,zl,zs,nfit=6,infile=None,mask=None,bias=1e8,lim=5200.,bg='polynomial',srclim=6000.):
    outwave,scispec,varspec = outwave[outwave>log10(lim)], scispec[outwave>log10(lim)], varspec[outwave>log10(lim)]
   ...
   # Create the polynomial fit components
    ...
    fit,chi = optimize.nnls(op,rhs)
    for i in range(nfit):
        fit[2*ntemps1+2*ntemps2+i] -= bias/grid**i
    ...
    if mask is not None:
        ...
        for k in range(ntemps2):
            b = interpolate.bisplev(sigS,np.log10(twave2),t2[k])
            modb = interpolate.splrep(owS2,b)
            oper[k+2*ntemps1,origcond] = interpolate.splev(origwave[origcond],modb)      
        
    outmodel = scipy.dot(oper.T,fit)
    lens = scipy.dot(oper[:ntemps1].T,fit[:ntemps1])
    source = scipy.dot(oper[ntemps1:2*ntemps1+ntemps2].T,fit[ntemps1:2*ntemps1+ntemps2])
    cont = scipy.dot(oper[2*ntemps1+2*ntemps2:].T,fit[2*ntemps1+2*ntemps2:])

    coarsesource = scipy.dot(oper[ntemps1*2:ntemps1*2+ntemps2].T,fit[ntemps1*2:2*ntemps1+ntemps2])
    pl.figure()
    if mask is not None:
        for M in mask:
    ...
    #    print i
    #pl.figure()
    #for i in range(ntemps2):
    #    pl.plot(10**origwave, oper[i+2*ntemps1])
    return result
