...
def run(zl,zs,fit=True,read=False,File=None,mask=None,lim=5000.,nfit=6.,bg='polynomial',bias=1e8,restmask=None):
    # Load in spectrum
    scispec = py.open('/data/ljo31b/EELs/esi/kinematics/wide/J0837_spec.fits')[0].data
    varspec = py.open('/data/ljo31b/EELs/esi/kinematics/wide/J0837_var.fits')[0].data
    sciwave = py.open('/data/ljo31b/EELs/esi/kinematics/wide/J0837_wl.fits')[0].data
...
name = '00' # 10,000
print name
result = run(0.4256,0.6411,fit=True,read=False,File = 'wide_J0837_'+name,mask=np.array([[7580,7700],[6860,6900]]),nfit=6,bg='polynomial',lim=5000,bias=1e8)#,restmask=np.array([[3900,3990]]))
result = run(0.4246,0.6411,fit=False,read=True,File = 'wide_J0837_'+name,mask=np.array([[7580,7700],[6860,6900]]),nfit=6,bg='polynomial',lim=5000,bias=1e8)#,restmask=np.array([[3900,3990]]))
