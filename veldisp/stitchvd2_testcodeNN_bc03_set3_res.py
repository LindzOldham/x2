...
def run(zl,zs,fit=True,read=False,File=None,mask=None,lim=5000.,nfit=6.,bg='polynomial',bias=1e8,restmask=None,srclim=6000.,lenslim=5500.):
    # Load in spectrum
    sciwave,scispec,varspec = np.load('/data/ljo31b/EELs/esi/testcode/summedspec_logSigma.npy')
...
#result = run(0.392347,0.50913,fit=True,read=False,File = name,mask=np.array([[5560,5620],[6300,6320],[7225,7280],[7580,7700],[6860,6900]]),nfit=8,bg='polynomial',lim=4500.,bias=1e6,srclim=5000.,lenslim=4610.)
result = run(0.392347,0.50913,fit=False,read=True,File = name,mask=np.array([[5560,5620],[6300,6320],[7225,7280],[7580,7700],[6860,6900]]),nfit=8,bg='polynomial',lim=4500.,bias=1e6,srclim=5000.,lenslim=4610.)
