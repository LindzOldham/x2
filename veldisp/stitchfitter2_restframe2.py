...
def finddispersion(scispec,varspec,t1,t2,twave1,twave2,outwave,zl,zs,nfit=6,outfile=None,mask=None,bias=1e8,lim=4000.,bg='polynomial',restmask=None,srclim=6000.,lenslim=5500.,name=None):
    ...
    if name is None:
        vL = pymc.Uniform('lens velocity',-3050.,3050.)
        sL = pymc.Uniform('lens dispersion',5.,501.)
        vS = pymc.Uniform('source velocity',-3050.,3050.)
        sS = pymc.Uniform('source dispersion',5.,501.)
    else:
        result = np.load('/data/ljo31b/EELs/esi/kinematics/dump/apertures/final/'+name+'_1.00_lens')
        lp,trace,dic,_=result
        a1,a2=np.unravel_index(lp.argmax(),lp.shape)
        vL = pymc.Uniform('lens velocity',-3050.,3050.,value=dic['lens velocity'][a1,a2])
        sL = pymc.Uniform('lens dispersion',5.,501.,value=dic['lens dispersion'][a1,a2])
        vS = pymc.Uniform('source velocity',-3050.,3050.,value=dic['source velocity'][a1,a2])
    ...
    def logprob(value=0.,pars=pars):
        ...
        for k in range(ntemps2):
            oper[k+2*ntemps1,lenscond] = interpolate.bisplev(sL.value,outwave[lenscond]-zL,t2[k])
    ...
    def logp(value=0.,lp=logprob):
        return lp
    print outfile
    S = myEmcee.Emcee(pars+[logp],cov=cov,nthreads=24,nwalkers=80)
    S.sample(400)
