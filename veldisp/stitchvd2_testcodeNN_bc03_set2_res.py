...
np.save('/data/ljo31b/EELs/esi/BC03models_set2',model)
...
def run(zl,zs,fit=True,read=False,File=None,mask=None,lim=5000.,nfit=6.,bg='polynomial',bias=1e8,restmask=None,srclim=6000.,lenslim=5500.):
    ...
    t1 = np.load('/data/ljo31b/EELs/esi/BC03models_set2.npy')    
    tmpwave1 = wave.copy()
...
result = run(0.466498, 0.64319,fit=True,read=False,File = name,mask=np.array([[5560,5620],[6300,6320],[7225,7280],[7580,7700],[6860,6900]]),nfit=6,bg='polynomial',lim=5000.,bias=1e5,srclim=6100.,lenslim=5700.)
result = run(0.466498, 0.64319,fit=False,read=True,File = name,mask=np.array([[5560,5620],[6300,6320],[7225,7280],[7580,7700],[6860,6900]]),nfit=6,bg='polynomial',lim=5000.,bias=1e5,srclim=6100.,lenslim=5700.)
pl.title(name[:5])
