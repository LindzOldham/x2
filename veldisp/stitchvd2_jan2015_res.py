...
np.save('/data/ljo31b/EELs/esi/INDOUSmodels_EELs',model)
...
if name=='J0837':
    ...
    result = run(0.4246,0.6411,fit=True,read=False,File = filename,mask=np.array([[7580,7700],[5295,5325],[5570,5585],[6860,6900]]),nfit=12,bg='polynomial',lim=5000,bias=1e10,srclim=6000.,lenslim=5500.,typ=typ,wid=wid)
    result = run(0.4246,0.6411,fit=False,read=True,File = filename,mask=np.array([[7580,7700],[5295,5325],[5570,5585],[6860,6900]]),nfit=12,bg='polynomial',lim=5000,bias=1e10,srclim=6000.,lenslim=5500.,typ=typ,wid=wid)
    pl.suptitle(name)
    pl.savefig('/data/ljo31b/EELs/esi/kinematics/plots/apertures/final/'+filename+'.pdf')
    pl.savefig('/data/ljo31/public_html/Lens/sdss_eels_spectra/texplots/J0837_esi_indoUS.pdf')
    pl.show()
