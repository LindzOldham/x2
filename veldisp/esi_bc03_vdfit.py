...
triangle=False
filename = name+'_'+wid+'_'+typ+'_esi_bc03_vdfit'
...
def run(zl,zs,fit=True,read=False,File=None,mask=np.array([[7580,7700],[5570,5585],[6860,6900]]),nfit=12.,bg='polynomial',bias=100,restmask=None,wid=1.,typ=None):
    # Load in spectrum
    if typ=='lens':
        print 'lens'
        try:
    ...
    #maxlim = 8500.
    #lim=4800.
    #lim,srclim,maxlim = 5000.,5400.,7500.
    if srclim<=lim:
        srclim = lim+10.
    ...
    if fit:
        ...
        result = finddispersion(scispec,varspec,t1,t2,np.log10(sciwave),zl,zs,nfit=12,outfile=File,mask=mask,lim=lim,bg=bg,restmask=restmask,srclim=srclim,maxlim=maxlim,nthr=nthreads,bias=bias)
        return result
    elif read:
        result = readresults(scispec,varspec,t1,t2,np.log10(sciwave),zl,zs,nfit=12,infile=File,mask=mask,lim=lim,bg=bg,restmask=restmask,srclim=srclim,maxlim=maxlim,bias=bias)
        return result
    else:
        return

if name=='J0837':
    result = run(lz[name],sz[name],fit=True,read=False,File = filename,typ=typ,wid=wid,bias=100)#1e6)
    result = run(lz[name],sz[name],fit=False,read=True,File = filename,typ=typ,wid=wid,bias=100)#1e6)
    

elif name == 'J0901':
    result = run(lz[name],sz[name],fit=True,read=False,File = filename,typ=typ,wid=wid,bias=100)
    result = run(lz[name],sz[name],fit=False,read=True,File = filename,typ=typ,wid=wid,bias=100)
    ...
    result = run(lz[name],sz[name],fit=True,read=False,File = filename,typ=typ,wid=wid)#,nfit=14,bias=800)#bias=150)
    result = run(lz[name],sz[name],fit=False,read=True,File = filename,typ=typ,wid=wid)#,nfit=14,bias=800)#bias=150)
   
### may 2014 data set

elif name == 'J1323':
    result = run(lz[name],sz[name],fit=True,read=False,File = filename,wid=wid,typ=typ)#,bias=1e6)#nfit=14)
    result = run(lz[name],sz[name],fit=False,read=True,File = filename,wid=wid,typ=typ)#,bias=1e6)#nfit=14)
...
elif name == 'jul2016_J1605':
    result = run(lz[name[8:]],sz[name[8:]],fit=True,read=False,File = filename,wid=wid,typ=typ,mask=np.array([[5570,5585],[5695,5700],[6860,6900]]))
    result = run(lz[name[8:]],sz[name[8:]],fit=False,read=True,File = filename,wid=wid,typ=typ,mask=np.array([[5570,5585],[5695,5700],[6860,6900]]))


elif name == 'J1605':
    #result = run(lz[name],sz[name],fit=True,read=False,File = filename,wid=wid,typ=typ,bias=1e8,mask=np.array([[5570,5585],[5695,5700],[6860,6900]]))
    result = run(lz[name],sz[name],fit=False,read=True,File = filename,wid=wid,typ=typ,bias=1e8,mask=np.array([[5570,5585],[5695,5700],[6860,6900]]))
    ...
    result = run(lz[name],sz[name],fit=True,read=False,File = filename,wid=wid,typ=typ)#,bias=1)
    result = run(lz[name],sz[name],fit=False,read=True,File = filename,wid=wid,typ=typ)#,bias=1)
    
elif name == 'jul2016_J2228':
    result = run(lz[name[8:]],sz[name[8:]],fit=True,read=False,File = filename,wid=wid,typ=typ,bias=1e3)
    result = run(lz[name[8:]],sz[name[8:]],fit=False,read=True,File = filename,wid=wid,typ=typ,bias=1e3)
...
#pl.savefig('/data/ljo31/public_html/Lens/sdss_eels_spectra/texplots/fits_esi_bc03/'+filename+'.png')
pl.show()
