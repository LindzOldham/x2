...
X = 10 - gui_emcee6_both_2sg_b READY TO RUN ASAP! 10b BEST
X = 11 - big1. Made images bigger to catch all flux and stop psf fringing NO GOOD
X = 12 - Big7, with two sources and two galaxies. RUNNING checked bad
X = 13 - Big5 TO RUN NEXT/LAST! checked bad
X = 14 - gui_emcee10b
X = 15 - 14 with noncincentric sources
X = 16 - gui_emcee10b
'''
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=10,nwalkers=100,ntemps=4)
S.sample(1000)
...
jj=0
for jj in range(15):
    S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=10,nwalkers=100,ntemps=4,initialPars=trace[a1])
    S.sample(1000)
