...
_,_,_,logLb,logLv,logLi,logIb,logIv,logIi = np.load('/data/ljo31/Lens/LensParams/BVI_photometry_212.npy').T

#for ii in range(logLv.size):
#    print logIb[ii], logLb[ii] - np.log10(2.*np.pi*re[ii]**2.)
...
#for ii in range(len(names)):
#    print names[ii], '%.2f'%zz[ii], '%.2f'%xx[ii], '%.2f'%yy[ii]



#print names
#xx,sxx = np.delete(xx,-2),np.delete(sxx,-2)
#print zz
#xx,sxx = np.delete(xx,-1),np.delete(sxx,-1)

# check data
#pl.figure()
#pl.scatter(1.54*xx-0.71*yy+0.64,zz,color='k')
#pl.show()
...
#S.sample(5000)
outFile = '/data/ljo31b/EELs/FP/inference/FP_logI_ABSMAGS'
#f = open(outFile,'wb')
