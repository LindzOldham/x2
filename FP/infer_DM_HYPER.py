...
gamma = pymc.Uniform('gamma',0.15,2.5 )
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=cov,nthreads=8,nwalkers=80,ntemps=10)#28)
S.sample(20000)
outFile = '/data/ljo31b/EELs/FP/inference/FP_infer_gNFW_12_HYPER_PT'
f = open(outFile,'wb')
...
for i in range(len(pars)):
    pars[i].value = np.median(trace[2000:,0,:,i])
...
np.save('/data/ljo31b/EELs/FP/inference/models/gNFW_12_sigmas_HYPER',s2)
'''
