...
for i in range(len(names)):
    try:
        ints.append(np.load('/data/ljo31b/EELs/phys_models/models/interpolators/gNFW_12_LO_M_salpeter_'+names[i]+'.npy')[()])
    except:
        ints.append(np.load('/data/ljo31b/EELs/phys_models/models/interpolators/gNFW_LO_M_salpeter_'+names[i]+'.npy')[()])
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=cov,nthreads=8,nwalkers=100,ntemps=6)#28)
S.sample(20000)
...
for i in range(len(pars)):
    pars[i].value = np.median(trace[10000:,:,i])
...
np.save('/data/ljo31b/EELs/FP/inference/models/gNFW_12_sigmas_HYPER_LO_M_salpeter_BIG',s2)'''
