...
pl.figure()
pl.scatter(np.log10(sigma), mag+20, s=40,color='Crimson')
pl.show()

x,y = np.log10(sigma), mag+20
dxx,dyy = dsigma/sigma/np.log(10.), (dmu**2. + (2.*dre/re/np.log(10.))**2.)**0.5
sxx,syy = dxx**2.,dyy**2.
sxy,syx = 0.,0.

x,y,sxx,syy = x[y<2],y[y<2],sxx[y<2],syy[y<2]
#xx,yy,sxx,syy=xx[:-1],yy[:-1],,sxx[:-1],syy[:-1]
...
pars.append(pymc.Uniform('beta',-20,20,value=-10 ))
...
S.sample(1000)
outFile = '/data/ljo31b/EELs/FP/inference/FJ_macs'
f = open(outFile,'wb')
