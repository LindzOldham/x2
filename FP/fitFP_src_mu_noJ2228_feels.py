...
fp = np.load('/data/ljo31b/EELs/esi/kinematics/inference/results_1.50_source_bc03_vdfit.npy')
l,m,u = fp
...
#xx,yy,zz,sxx,syy,szz=xx[:12],yy[:12],zz[:12],sxx[:12],syy[:12],szz[:12]
#syz,szy = rho[:12]*dyy[:12]*dzz[:12],rho[:12]*dyy[:12]*dzz[:12]
print len(xx)
print zz
...
#fake eels
rre,rvd, rmu = np.loadtxt('/data/ljo31b/EELs/phys_models/FP_nuggets_realeels_noDM.dat').T
pl.figure()
pl.scatter(rvd,rre,color='SteelBlue',s=40)
pl.scatter(sigmas*100.,rre,color='Crimson',s=40)
pl.show()
pl.figure()
pl.scatter(sigmas*100,rvd-sigmas*100,color='SteelBlue',s=40)
pl.axhline(0,color='k')
pl.xlabel('$\sigma_{obs}$')
pl.ylabel('$\sigma_{synth} - \sigma_{obs}$')
pl.show()
# covariances
syz,szy = rho*dyy*dzz,rho*dyy*dzz
#np.save('/data/ljo31b/EELs/esi/kinematics/FP_EELs_mu',np.column_stack((xx,yy,zz,dxx,dyy,dzz)))

xx,yy,zz = np.log10(rvd/100.), rmu-20., np.log10(rre)
xx,yy,zz = xx[:-1],yy[:-1],zz[:-1]
#xx,yy,zz,sxx,syy,szz = np.delete(xx,-2),np.delete(yy,-2),np.delete(zz,-2),np.delete(sxx,-2),np.delete(syy,-2),np.delete(szz,-2)'''
...
print 'new!'
S = myEmcee.Emcee(pars+[likelihood],cov=optCov/5.,nthreads=20,nwalkers=28)
#S.sample(2000)
S.sample(4000)
