...
for i in range(f2.shape[0]):
    R = r*0.05*scales[i]
    logR = np.log10(1./R**2)
    nmod = splrep(logR[::-1],f2[i][::-1])
    nnorm = splint(logR[-1],logR[0],nmod)
    model = splrep(r*0.05*scales[i], f2[i]/nnorm)
    sbfluxes[i] = splev(Re[i],model)
    pl.plot(logR,f2[i]/nnorm)
    pl.plot(np.log10(1./Re[i]**2.),sbfluxes[i],'o')
...
def logP(value=0.,p=pars):
    ...
    #logrfunc = (a*logSigma+b*(mag-20.)+2.5*b*np.log10(2.*np.pi) - 5.*b*np.log10(scales) + g)/(1.-5.*b)
    ...
    #print fluxes.shape, norm.shape, arg.shape, s_arg.shape, m_arg.shape, s_norm.shape, m_norm.shape
    prob = np.log(3.*fluxes) - np.log(norm) - 0.5*arg - 0.5*s_arg - 0.5*m_arg - np.log(s_norm) - np.log(m_norm)
    #prob = np.log(fluxes) + np.log(sbfluxes) - np.log(norm) - 0.5*arg - 0.5*s_arg - 0.5*m_arg - np.log(s_norm) - np.log(m_norm)
    lp = prob.sum()
    return lp
