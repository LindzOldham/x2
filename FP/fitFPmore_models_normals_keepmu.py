...
dsigmas = sigmas*0.09
VD = np.loadtxt('/data/ljo31b/EELs/phys_models/models/NFW_normal_newMSTAR.dat')
...
MAG = phot['mag v']
re, dre, dmu,MAG,mu = np.delete(re,-3), np.delete(dre,-3),np.delete(dmu,-3),np.delete(MAG,-3),np.delete(mu,-3)
...
#xx,yy,zz,sxx,syy,szz = np.delete(xx,I),np.delete(yy,I),np.delete(zz,I),np.delete(sxx,I),np.delete(syy,I),np.del#ete(szz,I)
#I=-1 # J1606
#names = np.delete(names,I)
