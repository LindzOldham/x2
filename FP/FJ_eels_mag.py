...
x,y = np.log10(sigmas), mag+20
dxx,dyy = dsigmas/sigmas/np.log(10.), dmag
sxx,syy = dxx**2.,dyy**2.
sxy,syx = 0.,0.
...
II = sys.argv[1]
print II
x,y,sxx,syy = np.delete(x,II),np.delete(y,II),np.delete(sxx,II),np.delete(syy,II)
...
pl.ylim([-2.5,1])
print names[int(II)]
pl.savefig('/data/ljo31/public_html/Lens/esi/FP/FP_ohneJ2228_and'+names[int(II)]+'.png')
pl.show()
