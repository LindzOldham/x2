...
def logP(value=0.,p=pars):
    lp=0.
    a,b,alpha,mux,muy,taux,tauy,rho = pars[0].value, pars[1].value,pars[2].value, pars[3].value,pars[4].value, pars[5].value, pars[6].value,pars[7].value
    tauxy,tauyx = rho*taux*tauy, rho*taux*tauy
    taux2,tauy2,a2,b2 = taux**2.,tauy**2.,a**2.,b**2.
    X = zz - alpha - a*mux - b*muy
    Y = xx - mux
    Z = yy - muy
    Sxx = taux2*a2 + a*b*(tauxy+tauyx) + tauy2*b2 + szz #
    Syy = taux2 + sxx 
    Szz = tauy2 + syy 
...
print 'new!'
#S = myEmcee.Emcee(pars+[likelihood],cov=optCov/2.,nthreads=24,nwalkers=28)
#S.sample(5000)
outFile = '/data/ljo31b/EELs/FP/inference/FP_logI_CHECK_NEW_noS' # NEW referring to nfit=6
#f = open(outFile,'wb')
