...
logL = logI+9 + np.log10(2.*np.pi*re**2.)

Mstar = np.load('/data/ljo31b/EELs/inference/new/huge/masses_212.npy')[3]
#Mstar = np.delete(Mstar,-3)
#re = np.delete(re,-3)


xx,yy,zz = np.log10(sigmas), logL-10., np.log10(re)
...
pars, cov = [], []
pars.append(pymc.Uniform('a',-10.,10,-2 ))
#a=1.5
pars.append(pymc.Uniform('b',-5.,5,1 ))
...
#S.sample(5000)
outFile = '/data/ljo31b/EELs/FP/inference/FP_logL' # NEW referring to nfit=6
#f = open(outFile,'wb')
...
for key in dic.keys():
    lo,hi = np.percentile(dic[key][2000:].ravel(),16), np.percentile(dic[key][2000:].ravel(),84)
    med = dic[key][a1,a2]
...
m = np.linspace(10,12.5,10)
v = np.linspace(2,2.8,10)

pl.figure(figsize=(22,6),tight_layout=False)
pl.subplot(131)
pl.scatter(Mstar,np.log10(sigmas),c='SteelBlue',s=40)
pl.plot(m,-0.86 + 0.286*m)
pl.xlabel('$M_{\star}$')
pl.ylabel('$\sigma$')
pl.subplot(132)
pl.scatter(np.log10(sigmas), np.log10(re),c='SteelBlue',s=40)
pl.plot(v, -1.42 + 0.835*v)
pl.ylabel('$r_e$')
pl.xlabel('$\sigma$')
pl.subplot(133)
pl.scatter(Mstar, np.log10(re),c='SteelBlue',s=40)
pl.plot(m,-4.79 + 0.489*m)
pl.show()

Mstar = np.load('/data/ljo31b/EELs/inference/new/huge/NEWFORFPNEW_masses_212.npy')[3]
Mstar = np.delete(Mstar,-3)
pl.figure(figsize=(22,6),tight_layout=False)
pl.subplot(131)
pl.scatter(Mstar,np.log10(sigmas),c='SteelBlue',s=40)
