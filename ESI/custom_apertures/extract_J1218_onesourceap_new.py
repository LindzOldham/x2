...
def extract(pref,nums,wid=1.,wht=False,regal=5.,centroid=None,rein=None,lens=True,source=False):
    ...
    for numIndx in range(len(nums)):
        ...
        for order in range(1,11):
            ...
            #pl.plot(spec)
            #pl.show()
            if num == 53 and order == 1 and lens is True:
                vspec /= np.median(spec[:-150])**2
                spec /= np.median(spec[:-150])
                print 'THIS ONE'
            
            elif num == 52 and order == 1 and source is True:
                vspec /= np.median(spec[spec!=0.])**2
                spec /= np.median(spec[spec!=0.])
                #pl.figure()
                #pl.plot(spec)
                #pl.show()
                print 'THIS ONE'
    ...
    corr = np.load('/data/ljo31b/EELs/esi/raw/jan2015/run/orderCorr_Feige66.dat')
    ...
    for order in range(1,11): # purpose is to sum exposures -- here, for each order on the echelle
        ...
        try:
            ...
            if order ==1 and source is True:
                print 'here'
                print spec[(w>rb)&(w<rr)]
                print spec[(w>rb)&(w<rr)][:-100]
                right = np.median(spec[(w>rb)&(w<rr)][:-30])
            else:
                print 'or here'
...
ow,s,v = extract('/data/ljo31b/EELs/esi/raw/jan2015/run/EEL_J1218+5648',[51,52,53],wid=1.,regal=regal,centroid=centroids[name],rein=reins[name],lens=True,source=False)
