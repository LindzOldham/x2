...
def extract(pref,nums,wid=1.,wht=False,centroid=None,rein=None,lens=True,source=False,regal=None):
    ...
    for numIndx in range(len(nums)):
        ...
        for order in range(1,11):
            ...
            source2cent = lenscent-1.05/arcsecperpix[order-1]
            print lenscent,source1cent,source2cent

            lensap = np.where(abs(x-lenscent)<wid/arcsecperpix[order-1],1.,0.)
            #sourceap = np.where(((abs(x-source1cent)<wid/arcsecperpix[order-1])|(abs(x-source2cent)<wid/arcsecperpix[order-1]))&(abs(x-lenscent)>regal*0.025/arcsecperpix[order-1]),1.,0.)
            ...
            print 'aperture size is ', wid, 'in arcseconds, hopefully'
    ...
    corr = np.load('/data/ljo31b/EELs/esi/raw/jul2016/run/orderCorr_HZ44.dat')
    right = None
    rb = None
    rr = None
    for order in range(2,9): # purpose is to sum exposures -- here, for each order on the echelle
        w = owave[order]
        s = w*0.
        v = w*0.
        ...
        #pl.plot(10**outwave,outspec[:,order-1])
    ...
    pl.suptitle(name)
    #if lens:
    #    pl.savefig('/data/ljo31/public_html/Lens/esi/apertures/final/jul2016_'+name+'_'+str(wid)+'_lens.png')
    #elif source:
    #    pl.savefig('/data/ljo31/public_html/Lens/esi/apertures/final/jul2016_'+name+'_'+str(wid)+'_source.png')
    #pl.show()
    if lens:
        py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/final/jul2016_'+name+'_ap_'+'%.2f'%wid+'_spec_lens.fits',s,clobber=True)
        py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/final/jul2016_'+name+'_ap_'+'%.2f'%wid+'_var_lens.fits',v,clobber=True)
        py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/final/jul2016_'+name+'_ap_'+'%.2f'%wid+'_wl_lens.fits',ow,clobber=True)
    elif source:
        py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/final/jul2016_'+name+'_ap_'+'%.2f'%wid+'_spec_sourceb.fits',s,clobber=True)
        py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/final/jul2016_'+name+'_ap_'+'%.2f'%wid+'_var_sourceb.fits',v,clobber=True)
        py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/final/jul2016_'+name+'_ap_'+'%.2f'%wid+'_wl_sourceb.fits',ow,clobber=True)
...
ow,s,v = extract('/data/ljo31b/EELs/esi/raw/jul2016/run/EEL_J2228-0018',[70,71],wid=0.30,centroid=centroids[name],rein=reins[name],lens=False,source=True,regal=regal)
pl.suptitle('0.30')

ow,s,v = extract('/data/ljo31b/EELs/esi/raw/jul2016/run/EEL_J2228-0018',[70,71],wid=0.80,centroid=centroids[name],rein=reins[name],lens=False,source=True,regal=regal)
pl.suptitle('0.40')
...
#ow,s,v = extract('/data/ljo31b/EELs/esi/raw/jul2016/run/EEL_J2228-0018',[70,71],wid=1.0,centroid=centroids[name],rein=reins[name],lens=True,source=False,regal=regal)
