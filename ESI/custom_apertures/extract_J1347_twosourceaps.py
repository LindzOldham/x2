...
def extract(pref,nums,wid=1.,wht=False,centroid=None,rein=None,lens=True,source1=False,source2=False):
    ...
    for numIndx in range(len(nums)):
        ...
        for order in range(1,11):
            ...
            source1ap = np.where(abs(x-source1cent)<wid/arcsecperpix[order-1],1.,0.)
            source2ap = np.where(abs(x-source2cent)<wid/arcsecperpix[order-1],1.,0.)
            if lens:
                ap = lensap
            elif source1:
                ap = source1ap
    ...
    #np.save('/data/ljo31b/EELs/esi/kinematics/J1347_wide_apertures',apsize)
    ...
    pl.show()
    if lens:
        py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/'+name+'_ap_'+'%.2f'%wid+'_spec_lens.fits',s,clobber=True)
        py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/'+name+'_ap_'+'%.2f'%wid+'_var_lens.fits',v,clobber=True)
        py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/'+name+'_ap_'+'%.2f'%wid+'_wl_lens.fits',ow,clobber=True)
    elif source:
        py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/'+name+'_ap_'+'%.2f'%wid+'_spec_source1.fits',s,clobber=True)
        py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/'+name+'_ap_'+'%.2f'%wid+'_var_source1.fits',v,clobber=True)
        py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/'+name+'_ap_'+'%.2f'%wid+'_wl_source1.fits',ow,clobber=True)
    elif source2:
        py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/'+name+'_ap_'+'%.2f'%wid+'_spec_source2.fits',s,clobber=True)
        py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/'+name+'_ap_'+'%.2f'%wid+'_var_source2.fits',v,clobber=True)
        py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/'+name+'_ap_'+'%.2f'%wid+'_wl_source2.fits',ow,clobber=True)
...
name = 'J1347'
ow,s,v = extract('/data/ljo31b/EELs/esi/raw/may2014/run/EEL_J1347-0101',[42,43,44],wid=wid,centroid=centroids[name],rein=reins[name],lens=False,source1=True,source2=False)
