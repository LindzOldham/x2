...
''' J1218 -- two slits had a problem in that they struggled to work out where to put down an aperture. In the end, I did this manually (for orders 1 and 3). '''
...
def extract(pref,nums,wid=1.,wht=False,regal=5.,centroid=None,rein=None,lens=True,source=False):
    ...
    for numIndx in range(len(nums)):
        ...
        for order in range(2,11):
            ...
            if fit[2]>120:
                smooth = ndimage.gaussian_filter(slice,1)
                x = np.arange(slice.size)*1. # spatial grid
                fit = np.array([0.,smooth[:120].max(),smooth[:120].argmax(),1.])
                fit = sf.ngaussfit(slice[:120],fit)[0] 

            if fit[2]<15:
                smooth = ndimage.gaussian_filter(slice,1)
                x = np.arange(slice.size)*1. # spatial grid
                fit = np.array([0.,smooth[20:120].max(),smooth[20:120].argmax(),1.])
                fit = sf.ngaussfit(slice[20:120],fit)[0] 

            #if order == 3 and num == 53:
            #    print 'manually setting aperture location and width'
            #    fit[2],fit[3] = 90.,5.
            #if order == 1 and num ==53:
            #    print 'manually setting aperture location and width'
            #    fit[2],fit[3] = 110.,5.
            

            lenscent = fit[2] + centroid*0.05/arcsecperpix[order-1]
            source1cent = lenscent-rein[0]*0.05/arcsecperpix[order-1]
            source2cent = lenscent+rein[1]*0.05/arcsecperpix[order-1]
            print lenscent,source1cent,source2cent

            lensap = np.where(abs(x-lenscent)<wid/arcsecperpix[order-1],1.,0.)
            sourceap = np.where(((abs(x-source1cent)<wid/arcsecperpix[order-1])|(abs(x-source2cent)<wid/arcsecperpix[order-1]))&(abs(x-lenscent)>regal*0.025/arcsecperpix[order-1]),1.,0.) # half the effective radius of fg
            print regal*0.025
            ...
            '''pl.figure()
            pl.imshow(ap,interpolation='nearest',origin='lower')
            pl.show()
            pl.figure()
            pl.imshow(ap[:,:-200],interpolation='nearest',origin='lower')
            pl.show()'''
            ...
            print np.median(spec[spec>0])
            if num == 52 and order == 100:
                vspec /= np.median(spec[:-150])**2
                spec /= np.median(spec[:-150])
                print 'THIS ONE'
            else:
    ...
    for order in range(2,11): # purpose is to sum exposures -- here, for each order on the echelle
        #pl.plot(owave[order], ospex[order],alpha=0.7)
        w = owave[order]
        s = w*0.
        v = w*0.
        ...
        var = var[c]
        #pl.figure()
        #pl.plot(w,spec,alpha=0.7)
        if right is not None: # the point of this section is to get all echelles on the same scale! right = median of bluer echelle; left = median of current echelle. All normalised to be like the first echelle.
        ...
        try:
            ...
            #pl.figure()
            #pl.plot(w,spec)
            #pl.axvline(rb,color='b')
            #pl.axvline(rr,color='r')
            #pl.show()
        except:
            pass
        print 'RIGHT', right
    ...
    '''if lens:
        pl.savefig('/data/ljo31/public_html/Lens/esi/apertures/final/'+name+'_'+str(wid)+'_lens_2ndorder.png')
    elif source:
        pl.savefig('/data/ljo31/public_html/Lens/esi/apertures/final/'+name+'_'+str(wid)+'_source_2ndorder.png')'''
    pl.show()
    '''if lens:
        py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/final/'+name+'_ap_'+'%.2f'%wid+'_spec_len_2ndorders.fits',s,clobber=True)
        py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/final/'+name+'_ap_'+'%.2f'%wid+'_var_lens_2ndorder.fits',v,clobber=True)
        py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/final/'+name+'_ap_'+'%.2f'%wid+'_wl_lens_2ndorder.fits',ow,clobber=True)
    elif source:
        py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/final/'+name+'_ap_'+'%.2f'%wid+'_spec_source_2ndorder.fits',s,clobber=True)
        py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/final/'+name+'_ap_'+'%.2f'%wid+'_var_source_2ndorder.fits',v,clobber=True)
        py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/final/'+name+'_ap_'+'%.2f'%wid+'_wl_source_2ndorder.fits',ow,clobber=True)'''
...
pl.figure()
#ow,s,v = extract('/data/ljo31b/EELs/esi/'+name+'/EEL_J1218+5648',[51,52,53],wid=1.5,regal=regal,centroid=centroids[name],rein=reins[name],lens=False,source=True)

ow,s,v = extract('/data/ljo31b/EELs/esi/'+name+'/EEL_J1218+5648',[51],wid=1.,regal=regal,centroid=centroids[name],rein=reins[name],lens=True,source=False)
