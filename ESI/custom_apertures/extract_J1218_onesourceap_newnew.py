...
def extract(pref,nums,wid=1.,wht=False,regal=5.,centroid=None,rein=None,lens=True,source=False):
    ...
    for numIndx in range(len(nums)):
        ...
        for order in range(2,11):
            ...
            source1cent = lenscent-1./arcsecperpix[order-1]
    ...
    for order in range(2,11): # purpose is to sum exposures -- here, for each order on the echelle
        ...
        try:
            ...
            if order ==2 and source is True:
                ...
                print right
                pl.figure()
                pl.plot(spec[(w>rb)&(w<rr)])
                pl.show()
                print len(spec[(w>rb)&(w<rr)])
                right = 0.1
            else:
                print 'or here'
                right = np.median(spec[(w>rb)&(w<rr)])
...
ow,s,v = extract('/data/ljo31b/EELs/esi/'+name+'/EEL_J1218+5648',[51,52,53],wid=0.31,regal=regal,centroid=centroids[name],rein=reins[name],lens=False,source=True)
