...
def extract(pref,nums,wid=1.,wht=False,regal=5.,centroid=None,rein=None,lens=True,source=False):
    ...
    for numIndx in range(len(nums)):
        ...
        for order in range(1,11):
            ...
            if order == 3:
                smooth = ndimage.gaussian_filter(slice[:-20],1)
            ...
            sourceap = np.where(((abs(x-source1cent)<wid/arcsecperpix[order-1])|(abs(x-source2cent)<wid/arcsecperpix[order-1]))&(abs(x-lenscent)>0.1/arcsecperpix[order-1]),1.,0.) # half the effective radius of fg
            print regal*0.025
    ...
    for order in range(1,11): # purpose is to sum exposures -- here, for each order on the echelle
        ...
        try:
            ...
            #print 'order, left, right', order,left, right
        except:
            pass
...
ow,s,v = extract('/data/ljo31b/EELs/esi/J1125/EEL_J1125+3058',[46,46,47],wid=1.5,regal=regal,centroid=centroids[name],rein=reins[name],lens=False,source=True)
ow,s,v = extract('/data/ljo31b/EELs/esi/J1125/EEL_J1125+3058',[46,46,47],wid=1.0,regal=regal,centroid=centroids[name],rein=reins[name],lens=True,source=False)
