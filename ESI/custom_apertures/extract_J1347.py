...
def extract(pref,nums,wid=1.,wht=False,centroid=None,rein=None,lens=True,source=False):
    ...
    for numIndx in range(len(nums)):
        ...
        for order in range(1,11):
            ...
            sourceap = np.where((abs(x-source1cent)<wid/arcsecperpix[order-1])|(abs(x-source2cent)<wid/arcsecperpix[order-1]),1.,0.)
