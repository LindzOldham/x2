...
def extract(pref,nums,wid=1.,wht=False):
    ...
    for numIndx in range(len(nums)):
        ...
        for order in range(1,11):
            ...
            sourceap = np.where(((abs(x-source1cent)<wid/arcsecperpix[order-1])|(abs(x-source2cent)<wid/arcsecperpix[order-1]))&(abs(x-lenscent)>0.05/arcsecperpix[order-1]),1.,0.)
    ...
    #np.save('/data/ljo31b/EELs/esi/kinematics/J1248_wide_apertures',apsize)
...
name = 'J1248'
ow,s,v = extract('/data/ljo31b/EELs/esi/raw/may2014/run/EEL_J1347-0101',[42,43,44],wid=wid,centroid=centroids[name],rein=reins[name],lens=False,source=True)
