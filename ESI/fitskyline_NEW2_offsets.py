...
def ngaussFit(d,wave,mu,h1,h2,nsteps=500):
    pars = []
    logsigma = pymc.Uniform('sigma',0.05,5)
    ...
    def logl(value=0.,pars=pars):
        lp=0
        sigma = logsigma.value
        shiftmu = 10**mu-offset.value
        #print mu, shiftmu, sigma
        model = np.ones((wave.size,2))
        model[:,1] = np.exp(-0.5*(10**wave-shiftmu)**2./sigma**2.)
    ...
    for numIndx in range(len(nums)):
        ...
        for order in range(4,11):
            ...
            for i in range(len(c)):
                ...
                #print 10**sigma * np.log(10.)
                #print sky[i], sigma, sigma/10**sky[i]
                #print offset
                sigmas.append(offset )#10**sigma * np.log(10.))
        sigmas = np.array(sigmas)
        #print 'clip', order, clip(sigmas)
    allsigmas.append(sigmas)
    np.save('/data/ljo31b//EELs/esi/spectra/offsets_'+name,allsigmas)
...
np.save('/data/ljo31b/EELs/esi/spectra/resolutions_from_skylines_OFFSETS',dict(resolutions))
