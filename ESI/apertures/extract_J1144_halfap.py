...
def extract(pref,nums,wid=1.,wht=False):
    ...
    for numIndx in range(len(nums)):
        ...
        for order in range(1,11):
            ...
            if order==5.:
                pl.figure()
                pl.plot(x*arcsecperpix[order-1],ap[:,0])
                #pl.plot(x*arcsecperpix[order-1],slice)
                #pl.plot(x*arcsecperpix[order-1],smooth)
                pl.show()
            ap = ap/ap.sum(0)
    ...
    np.save('/data/ljo31b/EELs/esi/kinematics/J1144_wide_apertures',apsize)
...
if sys.argv[2] == 'up':
    ...
    up = False
name = 'J1144'
ow,s,v = extract('/data/ljo31b/EELs/esi/'+name+'/EEL_J1144+1540',[48,49,50],wid=wid)
