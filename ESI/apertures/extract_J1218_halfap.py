...
def extract(pref,nums,wid=1.,wht=False):
    ...
    for numIndx in range(len(nums)):
        ...
        for order in range(1,11):
            ...
            if up:
                ap = np.where((abs(x-fit[2])<wid/arcsecperpix[order-1])&(x>fit[2]),1.,0.)
            else:
                ap = np.where((abs(x-fit[2])<wid/arcsecperpix[order-1])&(x<fit[2]),1.,0.)
    ...
    np.save('/data/ljo31b/EELs/esi/kinematics/J1218_wide_apertures',apsize)
    ...
    for order in range(1,11): # purpose is to sum exposures -- here, for each order on the echelle
        ...
        try:
            ...
            #pl.figure()
            #pl.plot(w[(w>rb)&(w<rr)],spec[(w>rb)&(w<rr)])
            #print 'spec region', spec[(w>rb)&(w<rr)], spec[(w>rb)&(w<rr)].shape
            #right1 = np.median(spec[(w>rb)&(w<rr)])
            #if right1>0:
                #right = right1
...
if sys.argv[2] == 'up':
    ...
    up = False
name = 'J1218'
ow,s,v = extract('/data/ljo31b/EELs/esi/'+name+'/EEL_J1218+5648',[51,52,53],wid=wid)
...
pl.suptitle(name)
py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/'+name+'_halfap_'+sys.argv[2]+'_'+str(wid)+'_spec.fits',s,clobber=True)
py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/'+name+'_halfap_'+sys.argv[2]+'_'+str(wid)+'_var.fits',v,clobber=True)
py.writeto('/data/ljo31b/EELs/esi/kinematics/apertures/'+name+'_halfap_'+sys.argv[2]+'_'+str(wid)+'_wl.fits',ow,clobber=True)
pl.show()
