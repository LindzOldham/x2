...
def ngaussFit(data,wl,sky,nsteps=2500):
    ngauss = len(data)
    pars = []
    ...
    def logl(value=0.,pars=pars):
        ...
        for i in range(ngauss):
            ...
            model[:,1] = np.exp(-0.5*(wave-mu-offset)**2./sigma**2.)
...
def extract(pref,nums,plot=False):
    ...
    for numIndx in range(len(nums)):
        ...
        for order in range(1,11):
            ...
            lp,det = ngaussFit(c,cwl,sky)
            off = det[-1]
            print 'in wavelength: ',det[-1]
            if len(c)>0:
                x = np.arange(cwl[0].shape[0])
                model = interpolate.splrep(cwl[0],x)
                x0 = interpolate.splev(sky[0],model)
                x1 = interpolate.splev(sky[0]+off,model)
                pixoffset = x1-x0
                print 'in pixels? ',pixoffset
                print 'appending to offsets list'
                offsets.append(pixoffset)
            if plot:
                pl.figure()
                pl.plot(lp)
                pl.show()
                for i in range(len(c)):
                    ...
                    model = np.ones((wave.size,2))
                    model[:,1] = np.exp(-0.5*(wave-mu-det[-1])**2./sigma**2.)
