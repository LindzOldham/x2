...
X=37 - using gui_11_2src3 again, but editing it slightly in the gui so gui_11_x3 so the source components resemble bulge+dik more closely.
X=38 - Taking 37 and just continuing, as it was still improving and looked kind of interesting. And cutting down to 3 temperatures (this is fine right?)
'''
...
guiFile = '/data/ljo31/Lens/J1619/gui_11_x3'
...
result = np.load('/data/ljo31/Lens/J1619/emcee37')
lp = result[0]
trace=result[1]
dic=result[2]
a2=0
...
import time
start=time.time()
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=20,nwalkers=80,ntemps=3,initalPars=trace[-1,:3])
S.sample(1000)
...
jj=0
for jj in range(25):
    S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=20,nwalkers=80,ntemps=3,initialPars=trace[-1,:3])
    S.sample(1000)
