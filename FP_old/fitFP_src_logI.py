...
fp = np.load('/data/ljo31b/EELs/esi/kinematics/inference/results_1.00_lens_vdfit.npy')
l,m,u = fp
...
for ii in range(len(names)):
    band = bands[names[ii]]
    logI[ii] = solarmag.mu_to_logI(mu[ii],band+'_ACS',sz[names[ii]][0])
    dlogI[ii] = logI[ii] - solarmag.mu_to_logI(mu[ii]+dmu[ii],band+'_ACS',sz[names[ii]][0])
...
S.sample(6000)
outFile = '/data/ljo31b/EELs/FP/inference/FP_src_logI'
f = open(outFile,'wb')
cPickle.dump(S.result(),f,2)
f.close()
result = S.result()
#result = np.load('/data/ljo31b/EELs/FP/inference/FP_src_logI')
