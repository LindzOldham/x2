...
''' trying fitting parallel to log sigma '''

rec,drec,muc,dmuc,sigmac,dsigmac = np.loadtxt('/data/ljo31b/MACSJ0717/data/ComaFP.dat').T

xx,yy,zz = np.log10(rec), muc, np.log10(sigmac)
dzz,dyy,dxx = 1./np.log(10.) * dsigmac/sigmac, dmuc.copy(),1./np.log(10.) * drec/rec
...
pars, cov = [], []
pars.append(pymc.Uniform('a',0.5,5.,1.3 ))
pars.append(pymc.Uniform('b',-10.,10.,-0.3 ))
pars.append(pymc.Uniform('alpha',-20.,20.,-10 ))
pars.append(pymc.Uniform('mu x',0,2,1.))
pars.append(pymc.Uniform('mu y',15,30,23 ))
pars.append(pymc.Uniform('tau x',0,2,0.5 ))
