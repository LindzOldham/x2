...
x,y = np.log10(sigmas[ii]/100.),fDM[ii] # units: 10^11 solar masses for both axes.
sxx, syy = dsigmas[ii]/sigmas[ii]/np.log(10.), y*0.05 # made up error for now - really need to go through chain
...
S.sample(4000)
outFile = '/data/ljo31/Lens/Analysis/fDMSigma'
f = open(outFile,'wb')
cPickle.dump(S.result(),f,2)
f.close()
result = S.result()
result = np.load('/data/ljo31/Lens/Analysis/fDMSigma')
