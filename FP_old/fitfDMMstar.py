...
x,y = logM[ii] + np.log10(2.)-10.,fDM[ii] # units: 10^11 solar masses for both axes.
sxx, syy = dlogM[ii], y*0.05 # made up error for now - really need to go through chain
...
pars.append(pymc.Uniform('mu',-0.5,3.,value=1.5))
...
S.sample(4000)
outFile = '/data/ljo31/Lens/Analysis/fDMMstar'
f = open(outFile,'wb')
cPickle.dump(S.result(),f,2)
f.close()
result = S.result()
result = np.load('/data/ljo31/Lens/Analysis/fDMMstar')
