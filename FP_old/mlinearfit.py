...
def dofit(data,dep,doscatter=False,sample_factor=25):
    ...
    d = []
    d.append(np.log10(sigmac))
    d.append(muc)
    de = []
    de.append(dsigmac/sigmac)
    de.append(dmuc)
    ndim=2
...
#    tau1 = pymc.InverseWishart('tau',nu0,wsqr,value=tausd.mean(0))
    tau1 = pymc.Uniform('tau',nu0/2.,nu0*sig/2.,value=tausd.mean(0))
    usqr = pymc.Uniform('usqr',k0/2.,k0*wsqr/2.,value=usqrsd.mean(0))
