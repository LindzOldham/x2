...
x,y = logM-10., logMvir-10 # units: 10^11 solar masses for both axes.
sxx, syy = dlogM, dlogM/2. # made up error for now - really need to go through chain
...
S.sample(4000)
outFile = '/data/ljo31/Lens/Analysis/MvirMstar'
f = open(outFile,'wb')
cPickle.dump(S.result(),f,2)
f.close()
result = S.result()
result = np.load('/data/ljo31/Lens/Analysis/MvirMstar')
...
pl.ylabel(r'$\log(M_{vir}/10^{10}M_{\odot})$')
