...
S.sample(5000)
outFile = '/data/ljo31b/EELs/FP/inference/FP_src'
f = open(outFile,'wb')
cPickle.dump(S.result(),f,2)
f.close()
result = S.result()'''
result = np.load('/data/ljo31b/EELs/FP/inference/FP_src')
...
pl.figure()
pl.scatter(1.2*xx-0.75*yy+alpha,zz,color='SteelBlue')
Zline = np.linspace(min(zz),max(zz),10)
pl.plot(Zline,Zline)
pl.figtext(0.2,0.8,'a = '+'%.2f'%a)
pl.figtext(0.2,0.75,'b = '+'%.2f'%b)
pl.ylabel(r'$\log R_e$')
pl.xlabel('1.2$\log\sigma$ -0.75$\log I$')
pl.show()
