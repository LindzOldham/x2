...
x,y = np.log10(sigmas/100.),logMvirL # units: 10^11 solar masses for both axes.
sxx, syy = dsigmas/sigmas, dlogMstarL # made up error for now - really need to go through chain
...
S.sample(4000)
outFile = '/data/ljo31/Lens/Analysis/MLvirSigma'
f = open(outFile,'wb')
cPickle.dump(S.result(),f,2)
f.close()
result = S.result()
result = np.load('/data/ljo31/Lens/Analysis/MLvirSigma')
