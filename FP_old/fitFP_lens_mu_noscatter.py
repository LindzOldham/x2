...
dxx,dyy,dzz = dsigmal/sigmal/np.log(10.), dmu, dre/re/np.log(10.)
...
pars, cov = [], []
pars.append(pymc.Uniform('a',-2.,5,1.0 ))
...
def logP(value=0.,p=pars):
    lp=0.
    a,b,alpha,mux,muy,taux,tauy,rho = pars[0].value, pars[1].value,pars[2].value, pars[3].value,pars[4].value, pars[5].value, pars[6].value,pars[7].value#, pars[8].value
...
S = myEmcee.Emcee(pars+[likelihood],cov=optCov,nthreads=5,nwalkers=28)
S.sample(6000)
outFile = '/data/ljo31b/EELs/FP/inference/noscatter_01'#FP_lens_mu_211_huge'
f = open(outFile,'wb')
