...
x,y = logM-10., np.log10(sigmas/100.) # units: 10^11 solar masses, 100 km/s
sxx, syy = dlogM, dsigmas/sigmas
...
pars.append(pymc.Uniform('mu',0.5,2.,value=1.5))
...
S.sample(4000)
outFile = '/data/ljo31/Lens/Analysis/SigmaMstar'
f = open(outFile,'wb')
cPickle.dump(S.result(),f,2)
f.close()
result = S.result()'''
result = np.load('/data/ljo31/Lens/Analysis/SigmaMstar')
...
for j in range(xfit.size):
    lo[j],med[j],up[j] = np.percentile(fits[:,j],[36,50,64],axis=0)
...
pl.xlabel(r'$\log(M_{\star}/10^{10}M_{\odot})$')
pl.ylabel(r'$\log(\sigma/100$kms$^{-1})$')
pl.figtext(0.2,0.8,'$a = '+'%.2f'%a+'\pm'+'%.2f'%da+'$')
pl.figtext(0.2,0.75,'$b = '+'%.2f'%b+'\pm'+'%.2f'%db+'$')
#pl.xlim([10,12])
#pl.ylim([-0.4,1.9])
hb = xfit*0.286 + (-0.86 + 2.86 - 2)
pl.plot(xfit,hb,color='k',ls='--')
pl.figure()
pl.plot(lp)
pl.show()
