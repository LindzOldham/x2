...
x,y = logMvir-10.,logML # units: 10^11 solar masses for both axes.
sxx, syy = 0.03*x, 0.03*y # made up error for now - really need to go through chain
sxx2, syy2 = sxx**2., syy**2.
sxy,syx = 0.9*sxx*syy, 0.9*syy*sxx
...
S.sample(4000)
outFile = '/data/ljo31/Lens/Analysis/MLvirMvir'
f = open(outFile,'wb')
cPickle.dump(S.result(),f,2)
f.close()
result = S.result()
result = np.load('/data/ljo31/Lens/Analysis/MLvirMvir')
...
plot_ellipses(x,y,sxx,syy,0.9*np.ones(sxx.size),'Crimson')
pl.xlabel(r'$\log(M_{vir})$')
pl.ylabel(r'$\log(M_{vir}/L)$')
