...
#result = np.load('/data/ljo31/Lens/J1125/emcee26a')
#trace = result[1]
#dic = result[2]
#lp = result[0]

lp = np.load('/data/ljo31/Lens/J1125/emcee15_lpcut.npy')
trace = np.load('/data/ljo31/Lens/J1125/emcee15_tracecut.npy')
dic = np.load('/data/ljo31/Lens/J1125/emcee15_diccut.npy')[()]
...
#S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=12,nwalkers=60,ntemps=4,initialPars=trace[-1])
#S.sample(20000)
#S = myEmcee.Emcee(pars+[likelihood],cov=optCov,nwalkers=100,nthreads=6) # should have 100 walkers.
#S.sample(1000)
S = AMAOpt(pars,[likelihood],[logP],cov=optCov/2.)
S.set_minprop(len(pars)*2)
S.sample(1000*len(pars)**2)

outFile = '/data/ljo31/Lens/J1125/opt'+str(X)
f = open(outFile,'wb')
cPickle.dump(S.result(),f,2)
f.close()

lp,trace,det = S.result() # log likelihoods; chain (steps * params); det['extShear PA'] = chain in this variable
