...
X=17 - setup7. A 3/2/2 model, but keeping the galaxies totally fixed so that it doesn't take absolutely forever! Could also start doing things on a galaxy-subtracted image if we ever come up with a good enough disk model!!!
X=18 - galsub, setup14.
X=20 - as 18, but masking some disky regions. Not sure this will work when we've already subtracted the disk, but oh well...'''

X=20 # This almost certainly won't work, but what can you do but try...
print X
...
guiFile = '/data/ljo31/Lens/J1248/setup14'
print guiFile
...
# fixed galaxies, for speeeeeeeeed
gals = []
lenses = []
for name in L.keys():
    s = L[name]
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=22,nwalkers=40,ntemps=3)
print 'made it?'
S.sample(1000)
