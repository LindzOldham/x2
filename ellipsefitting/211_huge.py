...
def ellipse(xo,yo,color='CornflowerBlue',fit=True):
    ...
    a = pymc.Uniform('a',0,1,value=0.3)
    b = pymc.Uniform('b',0,0.1,value=0.03)
    alpha = pymc.Uniform('alpha',0.,np.pi/2.)
    pars = [a,b,alpha]
    cov=np.array([0.1,0.01,0.2])
    ...
    S.sample(2000)
    outFile = '/data/ljo31/Lens/ellipse_hugemodels/huge_211_mu_re_'+str(name)
    f = open(outFile,'wb')
...
for kk in range(len(names)):
    name=names[kk]
    print name
    re,mag = np.load('/data/ljo31/Lens/Analysis/ReMagPDFs_twoband_'+str(name)+'.npy')
    rev,rei = re.T
    magv,magi = mag.T
    z = sz[name][0]
    Da = astCalc.da(z)
...
np.save('/data/ljo31/Lens/LensParams/rho_huge_211',rho)
