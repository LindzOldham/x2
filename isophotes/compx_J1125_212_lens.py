...
ls,num = 'lens','211'
plot=True
result = np.load('/data/ljo31/Lens/'+name+'/twoband_212_pte_ctd_ctd')#/LensModels/new/J0913_212')#
...
xc,yc = gals[1].pars['x'],gals[1].pars['y']
dx,dy = gals[0].pars['x']-xc,gals[0].pars['y']-yc
print dx,dy
s1 = SBObjects.Sersic('s1',{'x':dx,'y':dy,'re':gals[0].pars['re'],'n':gals[0].pars['n'],'q':gals[0].pars['q'],'pa':gals[0].pars['pa']})
s2 = SBObjects.Sersic('s2',{'x':0,'y':0,'re':gals[1].pars['re'],'n':gals[1].pars['n'],'q':gals[1].pars['q'],'pa':gals[1].pars['pa']})
...
def ellipse(xo,yo,H,color='CornflowerBlue',fit=True):
    ...
    for i in range(len(y)):
        ...
        alpha = pymc.Uniform('alpha',1.4,3.0)
        pars = [a,b,alpha]
