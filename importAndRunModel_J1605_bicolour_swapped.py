...
psf2 = pyfits.open('/data/ljo31/Lens/J1605/SDSSJ1605+3811_F814W_psf.fits')[0].data.copy()
...
def logP(value=0.,p=pars):
    ...
    for i in range(len(imgs)):
        if i == 1:
            ...
            x0 = -1.*pars[0].value # xoffset
            y0 = -1.*pars[1].value # yoffset
            #print x0,y0
        image = imgs[i]
        sigma = sigs[i]
        psf = PSFs[i]
...
for i in range(len(imgs)):
    ...
    if i == 1:
        ...
        x0,y0 = -1.*det['xoffset'][-1], -1.*det['yoffset'][-1] # xoffset, yoffset #
        print x0,y0
