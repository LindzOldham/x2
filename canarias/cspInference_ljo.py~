...
vkidata = numpy.load('/data/ljo31/Lens/LensParams/VIK_phot_211_dict_huge_new.npy')[()] 
...
ageL = pymc.Uniform('age lens',tstartL,tendL)
...
massS = pymc.Uniform('massS',9.,12.5,value=11.)
...
import myEmcee_blobs as myEmcee, pylab as pl
S = myEmcee.PTEmcee(pars+[loglikelihood],cov=cov,nthreads=10,nwalkers=30,ntemps=3,filename='/data/ljo31b/EELs/inference/new/huge/prep_211_'+name+'.dat')
S.sample(20000)
result = S.result()
...
S = myEmcee.PTEmcee(pars+[loglikelihood],cov=cov,nthreads=10,nwalkers=30,ntemps=3,filename='/data/ljo31b/EELs/inference/new/huge/211_'+name+'.dat')    
S.sample_save(10000)
result = S.result()
lp,trace,dic,_ = result
a1,a2 = np.unravel_index(lp[:,0].argmax(),lp[:,0].shape)
...
outFile = '/data/ljo31b/EELs/inference/new/huge/result_211_'+str(name)
f = open(outFile,'wb')
