...
## identify peak brightness pixels in arcs
peaks = np.where(imgs[0]>np.percentile(imgs[0],99.85)) # ~ 0.5 here 
#peaks = np.where(imgs[1]>np.percentile(imgs[1],99.8))
# these are to traced back to lens plane each time
...
# these are to traced back to lens plane each time
peaks = np.where(imgs[0]>np.percentile(imgs[0],99.85)) # ~ 0.5 here 
xpeak, ypeak = xo[peaks], yo[peaks] # it's important to use the un-OVRSed coordinates for this.
xlp, ylp = pylens.lens_images(lenses,srcs,[xpeak,ypeak],1.,getPix=True)
s = srcs[0].pixeval(xlp,ylp,1.,csub=1) + srcs[1].pixeval(xlp,ylp,1.,csub=1)
pl.figure()
pl.scatter(xlp,ylp,c=s)
...
for i in range(len(imgs)):
    ...
    for lens in lenses:
        lens.setPars()
    x0,y0 = pylens.lens_images(lenses,srcs,[xp,yp],1./OVRS,getPix=True)
    x02,y02 = pylens.lens_images(lenses,srcs,[xop,yop],1.,getPix=True)
    for src in srcs:
        src.setPars()
        tmp = xc*0.
        tmp = src.pixeval(x0,y0,1./OVRS,csub=1)
    ...
    components = (model.T*fit).T.reshape((n,image.shape[0],image.shape[1]))
    #pl.figure()
    #pl.imshow(components[0] + components[1], interpolation='nearest')
    pl.figure()
    pl.scatter(x02,y02,c=components[0] + components[1],edgecolor='none')
    pl.figure()
    pl.scatter(x02,y02,c=xop,edgecolors='none')
    pl.figure()
    pl.scatter(x02,y02,c=yop,edgecolors='none')

# we need to update srcs. ATM, we're just doing everything over again...
pars = []
cov = []
### first parameters need to be the offsets
xoffset = dic['xoffset'][a1,a2]
...
# recalculate sources 
xoffset,yoffset,pars,cov = OFFSETS(dic)
srcs = SRCS()
lenses = LENSES()
...
#S.sample(1500)
S = myEmcee.Emcee(pars+[likelihood],cov=optCov,nwalkers=40,nthreads=1) # should have 100 walkers.
S.sample(10)
