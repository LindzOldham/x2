...
guiFile = '/data/ljo31/Lens/J0913/1src_2'

result = np.load('/data/ljo31/Lens/J0913/emcee10c')
lp= result[0]
a2=0
...
for key in 'x','y','q','pa','b','eta':
    if key == 'x':
        p[key] = dic['Lens 1 '+key][a1,a2,a3]+15
    elif key == 'y':
        p[key] = dic['Lens 1 '+key][a1,a2,a3]+20
    else:
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=15,nwalkers=40,ntemps=3)
S.sample(1000)
outFile = '/data/ljo31/Lens/J0913/emcee_1src'+str(X)
f = open(outFile,'wb')
...
jj=0
for jj in range(15):
    S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=15,nwalkers=40,ntemps=3,initialPars=trace[-1])
    S.sample(1000)

    outFile = '/data/ljo31/Lens/J0913/emcee_1src'+str(X)+str(jj)
    f = open(outFile,'wb')
    cPickle.dump(S.result(),f,2)
    f.close()
