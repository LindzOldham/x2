...
dic = np.load('/data/ljo31/Lens/LensModels/J1605_212_det')[()]
lp = np.load('/data/ljo31/Lens/LensModels/J1605_212_lp')

pars = []
cov = []
### first parameters need to be the offsets
xoffset =  dic['xoffset'][-1]#offsets[0][3]
yoffset = dic['yoffset'][-1]#offsets[1][3]
...
for name in G.keys():
    ...
    if name == 'Galaxy 1':
        for key in 'x','y','q','pa','re','n':
            lo,hi,val = s[key]['lower'],s[key]['upper'],dic[name+' '+key][-1]#s[key]['value']
...
p['x'] = lenses[0].pars['x']
p['y'] = lenses[0].pars['y']
pars.append(pymc.Uniform('extShear',-0.3,0.3,value=dic['extShear'][-1]))#shear[0]['b']['value']))
cov.append(1)
p['b'] = pars[-1]
pars.append(pymc.Uniform('extShear PA',-180.,180,value=dic['extShear PA'][-1]))#shear[0]['pa']['value']))
cov.append(100.)
p['pa'] = pars[-1]
...
for name in ['Source 1','Source 2']:
    ...
    if name == 'Source 1':
        ...
        for key in 'q','re','n','pa':
           lo,hi,val = s[key]['lower'],s[key]['upper'],dic[name+' '+key][-1]#s[key]['value']
           pars.append(pymc.Uniform('%s %s'%(name1,key),lo,hi,value=val))
           p[key] = pars[-1]
           if key == 'pa':
        ...
        for key in 'x','y': # subtract lens potition - to be added back on later in each likelihood iteration!
            ...
            pars.append(pymc.Uniform('%s %s'%(name1,key),lo-1 ,hi+1,value=val ))   # the parameter is the offset between the source centre and the lens (in source plane obvs)
    ...
    srcs.append(SBModels.Sersic(name1,p))
...
S.sample(1000)
outFile = '/data/ljo31/Lens/J1605/emcee_1src'+str(X)
f = open(outFile,'wb')
