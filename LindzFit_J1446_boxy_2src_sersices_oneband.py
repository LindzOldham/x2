...
X=14 - 2/1/2, but all Sersices (these seem to be better at finding solutions maybe? Weiss nicht) (gui_2src_3_uniforms)
X=15 0 as 14, but just img1.
X=16 - as 15, but on I band only.
'''
...
result = np.load('/data/ljo31/Lens/J1446/emcee15')
lp= result[0]
a2=0.
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=22,nwalkers=70,ntemps=3)
S.sample(1000)
...
jj=0
for jj in range(20):
    S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=22,nwalkers=70,ntemps=3,initialPars=trace[-1])
    S.sample(1000)
