...
from scipy import ndimage
im = py.open('/data/ljo31/Lens/SDSSJ1606+2235_F606W_sci_cutout_2.fits')[0].data.copy()
wht = py.open('/data/ljo31/Lens/SDSSJ1606+2235_F606W_wht_cutout_2.fits')[0].data.copy()

smooth = ndimage.gaussian_filter(im,0.7)
noisemap = np.where((smooth>0.7*sigma)&(im>0),im/wht+poisson, poisson_v)**0.5
py.writeto('/data/ljo31/Lens/SDSSJ1606+2235_F606W_noisemap.fits',noisemap,clobber=True)
pl.figure()
pl.imshow(noisemap)
pl.colorbar()
...
from scipy import ndimage
im = py.open('/data/ljo31/Lens/SDSSJ1606+2235_F814W_sci_cutout_2.fits')[0].data.copy()
wht = py.open('/data/ljo31/Lens/SDSSJ1606+2235_F814W_wht_cutout_2.fits')[0].data.copy()

smooth = ndimage.gaussian_filter(im,0.7)
noisemap = np.where((smooth>0.7*sigma)&(im>0),im/wht+poisson, poisson_v)**0.5
py.writeto('/data/ljo31/Lens/SDSSJ1606+2235_F814W_noisemap.fits',noisemap,clobber=True)
pl.figure()
pl.imshow(noisemap)
pl.colorbar()
