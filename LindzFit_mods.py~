...
X =49 - 47, but ffor longer!
X = 50 - 49, but with n>0.1 as the prior on source 1 n
X = 51 - 49, but starting off with source 1 re = 2, source 1 n = 2
X = 52 - 47, but starting off with source 1 re = 2, source 1 n = 2
X = 53 - 50, but with galaxy 2 re <150
'''
    ...
    if name == 'Galaxy 1':
        ...
        for key in 'q','pa','re','n':
            if s[key]['type']=='constant':
                ...
                val = dic[name+' '+key][a1,a2]
                if key == 're':
                    pars.append(pymc.Uniform('%s %s'%(name,key),lo,150,value=val))
                else:
                    pars.append(pymc.Uniform('%s %s'%(name,key),lo,hi,value=val))
                p[key] = pars[-1]
                cov.append(s[key]['sdev'])
