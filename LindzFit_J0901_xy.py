...
# subtract galaxy and identify brightest pixels - these should belong to the source (unless the galaxy model is bad, which is also very possible)
# need to fit the light also!
galsubs = []
for i in range(len(imgs)):
    if i == 0:
        dx,dy = 0,0
    ...
    # galsubs is a list of the galaxy-subtracted images.
    # now identify the peak-brightness pixels and trace them back into the source plane. Can we do a weighted sum, or just pick one or two? And how to avoid noisy pixels - have to be joined up and within the Einstein radius? Too complicated? Choose between pixels within a ring around the galaxy at the Einstein radius!
    
xcent,ycent,b = lenses[0].pars['x'].value, lenses[0].pars['y'].value, lenses[0].pars['b'].value
R = np.sqrt((xo-xcent}**2. + (yo-ycent)**2.)
ii = np.where(abs(R-b)<7.) # elliptical
pl.figure()
pl.imshow(img1,origin='lower')
sort = np.argsort(img1[ii])
pl.scatter(xo[ii][sort[-10:]],yo[ii][sort[-10:]])
# this has now selected the twn brightest pixels in the source. Trace these back to source plane...
ind = sort[-10:]
xind,yind,imgind = xo[ii][ind],yo[ii][ind],img1[ii][ind]
xsind,ysind = pylens.getDeflections(lenses,[xind,yind]) # positions of these pixels in the source plane
xpeak, ypeak = np.sum(imgind*xsind)/np.sum(imgind), np.sum(imgind*ysind)/np.sum(imgind)
print xpeak,ypeak
# but the point is, we want to recalculate this for EACH model
# so do all this once to get the brightest pixel coordinates xind, yind - this doesn't depend on the particulars of the lens model (just the Einstein radius, in a really broadbrush way)
np.save('/data/ljo31/Lens/J0901/peakpixels',np.column_stack((xind,yind,imgind]))
