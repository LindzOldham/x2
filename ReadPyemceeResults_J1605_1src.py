...
def CotSomponents(components,col):
    ...
    pl.suptitle(col)
    pl.subplot(224)
    pl.imshow(components.sum(0),interpolation='nearest',origin='lower',cmap='afmhot',aspect='auto',vmax=np.amax(components.sum(0))*0.7)
    pl.colorbar()
    pl.title('model ')
    pl.suptitle(col)
...
sig1 = py.open('/data/ljo31/Lens/J1605/SDSSJ1605+3811_F555W_noisemap2_masked.fits')[0].data.copy() # masking out the possiblyasecondsource regions!
...
result = np.load('/data/ljo31/Lens/J1605/emcee_1src5')
lp= result[0]
a2=0.
...
yc,xc,yo,xo = yc-15,xc-15,yo-15,xo-15
...
#x1,y1,re1,n1,pa1,q1 = dic['Source 2 x'][a1,a2,a3], dic['Source 2 y'][a1,a2,a3], dic['Source 2 re'][a1,a2,a3], dic['Source 2 n'][a1,a2,a3], dic['Source 2 pa'][a1,a2,a3], dic['Source 2 q'][a1,a2,a3]
...
print 'source 2 ', '&', '%.2f'%x1, '&',  '%.2f'%y1, '&', '%.2f'%n6, '&', '%.2f'%re6, '&', '%.2f'%q6, '&','%.2f'%pa6,  r'\\'
...
###
print 'source 2 & $', '%.2f'%(Ddic['Source 2 x']+Ddic['Lens 1 x']), '_{-', '%.2f'%Ldic['Source 2 x'],'}^{+','%.2f'%Udic['Source 2 x'], '}$ & $', '%.2f'%(Ddic['Source 2 y']+Ddic['Lens 1 x']),'_{-', '%.2f'%Ldic['Source 2 y'],'}^{+', '%.2f'%Udic['Source 2 y'], '}$ & $', '%.2f'%Ddic['Source 2 re'],'_{-', '%.2f'%Ldic['Source 2 re'],'}^{+', '%.2f'%Udic['Source 2 re'], '}$ & $', '%.2f'%Ddic['Source 2 n'],'_{-', '%.2f'%Ldic['Source 2 n'],'}^{+', '%.2f'%Udic['Source 2 n'], '}$ & $','%.2f'%Ddic['Source 2 pa'],'_{-', '%.2f'%Ldic['Source 2 pa'],'}^{+', '%.2f'%Udic['Source 2 pa'], '}$ & $','%.2f'%Ddic['Source 2 q'],'_{-', '%.2f'%Ldic['Source 2 q'],'}^{+', '%.2f'%Udic['Source 2 q'], '}$',r'\\'
###
