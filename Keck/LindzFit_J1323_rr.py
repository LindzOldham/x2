...
kresult = np.load('/data/ljo31/Lens/J1323/Kp_0')
...
cov = []
### 3 PSF components
pars.append(pymc.Uniform('xoffset',-40.,40.,value=kdic['xoffset'][ka1,0,ka3]))
pars.append(pymc.Uniform('yoffset',-40.,40.,value=kdic['yoffset'][ka1,0,ka3]))
cov += [0.4,0.4]
#psf1
pars.append(pymc.Uniform('sigma 1',0,8,value=kdic['sigma 1'][ka1,0,ka3]))
pars.append(pymc.Uniform('q 1',0.1,1,value=kdic['q 1'][ka1,0,ka3]))
pars.append(pymc.Uniform('pa 1',-180,180,value= kdic['pa 1'][ka1,0,ka3]) )
pars.append(pymc.Uniform('amp 1',0,1,value=kdic['amp 1'][ka1,0,ka3]))
cov += [1,0.5,50,0.5]

# psf2
pars.append(pymc.Uniform('sigma 2',0.1,60,value=kdic['sigma 2'][ka1,0,ka3])  )
pars.append(pymc.Uniform('q 2',0.1,1,value=kdic['q 2'][ka1,0,ka3]))
pars.append(pymc.Uniform('pa 2',-180,180,value=kdic['pa 2'][ka1,0,ka3])  )
pars.append(pymc.Uniform('amp 2',0,1,value=kdic['amp 2'][ka1,0,ka3]))
cov += [1,0.5,50,0.5]

# psf3
pars.append(pymc.Uniform('sigma 3',0.1,400,value= kdic['sigma 3'][ka1,0,ka3]) ) 
pars.append(pymc.Uniform('q 3',0.1,1,value=kdic['q 3'][ka1,0,ka3]))
pars.append(pymc.Uniform('pa 3',-180,180,value= kdic['pa 3'][ka1,0,ka3]) )
pars.append(pymc.Uniform('amp 3',0,1,value=kdic['amp 3'][ka1,0,ka3]))
cov += [1,0.5,50,0.5]
...
def logP(value=0.,p=pars):
    ...
    for gal in gals:
        gal.setPars()
        tmp = xc*0.
        tmp[mask2] = gal.pixeval(xin,yin,0.2/OVRS,csub=11) # evaulate on the oversampled grid. OVRS = number of new pixels per old pixel.
    ...
    for src in srcs:
        src.setPars()
        tmp = xc*0.
        tmp[mask2] = src.pixeval(x0,y0,0.2/OVRS,csub=11)
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=20,nwalkers=48,ntemps=3,initialPars=ktrace[-1])
print 'set up sampler'
S.sample(5000)
outFile = '/data/ljo31/Lens/J1323/Kp_'+str(X)
f = open(outFile,'wb')
...
jj=0
for jj in range(50):
    S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=20,nwalkers=48,ntemps=3,initialPars=trace[-1])
    S.sample(5000)
