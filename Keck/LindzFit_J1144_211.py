...
def NotPlicely(image,im,sigma):
    ...
    pl.figure()
    pl.subplot(221)
    pl.imshow(image,origin='lower',interpolation='nearest',extent=ext,cmap='afmhot',aspect='auto',vmin=0,vmax=np.amax(image)*0.25)#,vmax=4) #,vmin=vmin,vmax=vmax)
    pl.colorbar()
    pl.title('data')
    pl.subplot(222)
    pl.imshow(im,origin='lower',interpolation='nearest',extent=ext,cmap='afmhot',aspect='auto',vmin=0,vmax=np.amax(image)*0.25) #,vmin=vmin,vmax=vmax)
    pl.colorbar()
    pl.title('model')
    pl.subplot(223)
...
# psf2
pars.append(pymc.Uniform('sigma 2',0.1,60,value= 2 )) 
...
# psf3
pars.append(pymc.Uniform('sigma 3',0.1,400,value= 25 )) 
pars.append(pymc.Uniform('q 3',0,1,value=0.9))
pars.append(pymc.Uniform('pa 3',-180,180,value= 90 ))
pars.append(pymc.Uniform('amp 3',0,1,value=0.15))
cov += [1,0.5,50,0.5]

# psf4
#pars.append(pymc.Uniform('sigma 4',0.1,400,value= 7 )) 
...
def logP(value=0.,p=pars):
    ...
    for gal in gals:
        gal.setPars()
        tmp = xc*0.
        tmp[mask2] = gal.pixeval(xin,yin,0.2,csub=11) 
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=20,nwalkers=44,ntemps=3)
print 'set up sampler'
S.sample(500)
outFile = '/data/ljo31/Lens/J1144/Kp_211_'+str(X)
f = open(outFile,'wb')
