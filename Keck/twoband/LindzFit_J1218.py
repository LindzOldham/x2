...
def NotPlicely(image,im,sigma):
    ...
    pl.figure()
    pl.subplot(221)
    pl.imshow(image,origin='lower',interpolation='nearest',extent=ext,cmap='afmhot',aspect='auto',vmin=0,vmax=40) #,vmin=vmin,vmax=vmax)
    pl.colorbar()
    pl.title('data')
    pl.subplot(222)
    pl.imshow(im,origin='lower',interpolation='nearest',extent=ext,cmap='afmhot',aspect='auto',vmin=0,vmax=40) #,vmin=vmin,vmax=vmax)
    pl.colorbar()
    pl.title('model')
    pl.subplot(223)
...
cov = []
### 3 PSF components
pars.append(pymc.Uniform('xoffset',-40.,40.,value=4))
pars.append(pymc.Uniform('yoffset',-40.,40.,value=8))
cov += [0.4,0.4]
#psf1
pars.append(pymc.Uniform('sigma 1',0.5,8,value=1))
...
# psf2
pars.append(pymc.Uniform('sigma 2',0.5,60,value= 2 )) 
...
# psf3
pars.append(pymc.Uniform('sigma 3',0.5,400,value= 5 )) 
...
covs = dict([('x',0.5),('y',0.5),('q',0.1),('pa',1.),('re',1.),('n',0.5)])
...
S.sample(500)
outFile = '/data/ljo31/Lens/J1218/twoband_Kp_'+str(X)
f = open(outFile,'wb')
...
for gal in gals:
    gal.setPars()
    tmp = xc*0.
    tmp = gal.pixeval(xp,yp,0.6,csub=11) # evaulate on the oversampled grid. OVRS = number of new pixels per old pixel.
    tmp = iT.resamp(tmp,OVRS,True) # convert it back to original size
    tmp = convolve.convolve(tmp,psf,False)[0]
    model[n] = tmp.ravel()
    n +=1
...
for src in srcs:
    src.setPars()
    tmp = xc*0.
    tmp = src.pixeval(x0,y0,0.6,csub=11)
    tmp = iT.resamp(tmp,OVRS,True)
    tmp = convolve.convolve(tmp,psf,False)[0]
    model[n] = tmp.ravel()
    n +=1
