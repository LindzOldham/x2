...
result = np.load('/data/ljo31/Lens/LensModels/twoband/J2228_212')
lp= result[0]
a2=0
...
xc,xo,yc,yo = xc+8,xo+8,yc+8,yo+8
...
kresult = np.load('/data/ljo31/Lens/J2228/twoband_Kp_212_0')
klp= kresult[0]
ka2=0
...
for i in range(len(imgs)):
    ...
    for gal in gals:
        gal.setPars()
        tmp = xc*0.
        tmp[mask2] = gal.pixeval(xin,yin,0.6,csub=11) # evaulate on the oversampled grid. OVRS = number of new pixels per old pixel.
    ...
    for src in srcs:
        src.setPars()
        tmp = xc*0.
        tmp[mask2] = src.pixeval(x0,y0,0.6,csub=11)
...
###
print 'source 2 & $', '%.2f'%(Ddic['Source 2 x']+lenses[0].pars['x']), '_{-', '%.2f'%Ldic['Source 2 x'],'}^{+','%.2f'%Udic['Source 2 x'], '}$ & $', '%.2f'%(Ddic['Source 2 y']+lenses[0].pars['x']),'_{-', '%.2f'%Ldic['Source 2 y'],'}^{+', '%.2f'%Udic['Source 2 y'], '}$ & $', '%.2f'%Ddic['Source 2 re'],'_{-', '%.2f'%Ldic['Source 2 re'],'}^{+', '%.2f'%Udic['Source 2 re'], '}$ & $', '%.2f'%Ddic['Source 2 n'],'_{-', '%.2f'%Ldic['Source 2 n'],'}^{+', '%.2f'%Udic['Source 2 n'], '}$ & $','%.2f'%Ddic['Source 2 pa'],'_{-', '%.2f'%Ldic['Source 2 pa'],'}^{+', '%.2f'%Udic['Source 2 pa'], '}$ & $','%.2f'%Ddic['Source 2 q'],'_{-', '%.2f'%Ldic['Source 2 q'],'}^{+', '%.2f'%Udic['Source 2 q'], '}$',r'\\'
###
print r'\end{tabular}'
print r'\end{table}'
