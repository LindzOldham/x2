...
def NotPlicely(image,im,sigma):
    ...
    pl.title('model')
    pl.subplot(223)
    pl.imshow(image-im,origin='lower',interpolation='nearest',extent=ext,cmap='afmhot',aspect='auto',vmin=-0.5,vmax=0.5,)
    pl.colorbar()
    pl.title('data-model')
    pl.subplot(224)
...
def SotPleparately(image,im,sigma,col):
    ext = [0,image.shape[1],0,image.shape[0]]
    pl.figure()
    pl.imshow(image,origin='lower',interpolation='nearest',extent=ext,cmap='afmhot',vmin=0,aspect='auto',vmax=np.amax(image)*0.25) #,vmin=vmin,vmax=vmax)
    pl.colorbar()
    pl.title('data - '+str(col))
    pl.figure()
    pl.imshow(im,origin='lower',interpolation='nearest',extent=ext,cmap='afmhot',vmin=0,aspect='auto',vmax=np.amax(image)*0.25) #,vmin=vmin,vmax=vmax)
    pl.colorbar()
...
kresult = np.load('/data/ljo31/Lens/J1606/twoband_Kp_212_0')
klp= kresult[0]
ka2=0
...
#bar = kdic['boxiness'][ka1,ka2,ka3]
#print xc.shape, image.shape, mask.shape
...
for i in range(len(imgs)):
    ...
    for gal in gals:
        gal.setPars()
        tmp = xc*0.
        tmp = gal.pixeval(xin,yin,0.6/OVRS,csub=23).reshape(xc.shape) # evaulate on the oversampled grid. OVRS = number of new pixels per old pixel.
    ...
    for src in srcs:
        ...
        if src.pars['q']<0.0:
            print 'boxy', bar
            tmp = src.boxypixeval(x0,y0,0.6/OVRS,csub=23,c=bar).reshape(xc.shape)
        else:
            tmp = src.pixeval(x0,y0,0.6/OVRS,csub=23).reshape(xc.shape)
    ...
    for i in range(4):
        pl.figure()
        pl.imshow(components[i],interpolation='nearest',origin='lower',aspect='auto')
        pl.colorbar()
    print fit
    print model.shape, components.shape
