...
xoffset,yoffset,sig1,q1,pa1,amp1,sig2,q2,pa2,amp2,sig3,q3,pa3,amp3 = ktrace[ka1,ka3,:14]
...
for name in ['Source 2', 'Source 1']:
    ...
    if name == 'Source 2':
        ...
        for key in 'x','y': # subtract lens potition - to be added back on later in each likelihood iteration!
            p[key] = kdic[name+' '+key][ka1,ka3]#+lenses[0].pars[key]
    elif name == 'Source 1':
        print name
        for key in 'q','re','n','pa':
           p[key] = kdic[name+' '+key][ka1,ka3]
        for key in 'x','y': # subtract lens potition - to be added back on later in each likelihood iteration!
            p[key] = kdic[name+' '+key][ka1,ka3]+lenses[0].pars[key]
...
###
print 'source 2 & $', '%.2f'%(Ddic['Source 2 x']), '_{-', '%.2f'%Ldic['Source 2 x'],'}^{+','%.2f'%Udic['Source 2 x'], '}$ & $', '%.2f'%(Ddic['Source 2 y']),'_{-', '%.2f'%Ldic['Source 2 y'],'}^{+', '%.2f'%Udic['Source 2 y'], '}$ & $', '%.2f'%srcs[0].pars['re'], '$ & $', '%.2f'%srcs[0].pars['n'],'$ & $','%.2f'%srcs[0].pars['pa'],'$ & $','%.2f'%srcs[0].pars['q'], '$',r'\\'
###
print r'\end{tabular}'
print r'\end{table}'
'''
