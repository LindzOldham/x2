...
def SotPleparately(image,im,sigma,col):
    ext = [0,image.shape[0],0,image.shape[1]]
    pl.figure()
    pl.imshow(image,origin='lower',interpolation='nearest',extent=ext,cmap='afmhot',vmin=0,aspect='auto',vmax=5) #,vmin=vmin,vmax=vmax)
    pl.colorbar()
    pl.title('data - '+str(col))
    pl.figure()
    pl.imshow(im,origin='lower',interpolation='nearest',extent=ext,cmap='afmhot',vmin=0,aspect='auto',vmax=5) #,vmin=vmin,vmax=vmax)
    pl.colorbar()
...
result = np.load('/data/ljo31/Lens/LensModels/twoband/J1605_212')
lp= result[0]
a2=0
...
kresult = np.load('/data/ljo31/Lens/J1605/twoband_Kp_212_0')
klp= kresult[0]
ka2=0
