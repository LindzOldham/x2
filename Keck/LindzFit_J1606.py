...
xc,xo,yc,yo = xc+3,xo+3,yc-7,yo-7
#mask = py.open('/data/ljo31/Lens/J1606/mask_Kp.fits')[0].data.copy()[630:830,880:1050]
#mask = mask[:,:-40]
...
pars.append(pymc.Uniform('pa 1',-180,180,value= 0 ))
...
pars.append(pymc.Uniform('pa 2',-180,180,value= 0 ))
...
cov += [1,0.5,20,0.5]
# psf4
#pars.append(pymc.Uniform('sigma 3',0.1,400,value= 10 )) 
#pars.append(pymc.Uniform('q 3',0,1,value=0.9))
#pars.append(pymc.Uniform('pa 3',-180,180,value= 90 ))
#pars.append(pymc.Uniform('amp 3',0,1,value=0.25))
#cov += [1,0.5,20,0.5]
...
his = dict([('q',1.00),('pa',180.),('re',100.),('n',15.)])
...
#pars.append(pymc.Uniform('boxiness',0.5,10,value=dic['boxiness'][a1,a2,a3] ))
#cov += [0.9]
...
def logP(value=0.,p=pars):
    ...
    imin,sigin,xin,yin = image[np.where(mask)], sigma[np.where(mask)],xp[np.where(mask2)],yp[np.where(mask2)]
    n = 0
    model = np.empty(((len(gals) + len(srcs)+1),imin.size))
    for gal in gals:
        gal.setPars()
        tmp = xc*0.
        tmp[np.where(mask2)] = gal.pixeval(xin,yin,0.6/OVRS,csub=23) # evaulate on the oversampled grid. OVRS = number of new pixels per old pixel.
        tmp = iT.resamp(tmp,OVRS,True) # convert it back to original size
        tmp = convolve.convolve(tmp,psf,False)[0]
        model[n] = tmp[np.where(mask)].ravel()
        n +=1
    for lens in lenses:
        lens.setPars()
    ...
    for src in srcs:
        ...
        tmp = xc*0.
        #if src.pars['q']<0.2:
        #    tmp[np.where(mask2)] = src.boxypixeval(x0,y0,0.6/OVRS,csub=23,c=pars[-1].value)
        #else:
        tmp[np.where(mask2)] = src.pixeval(x0,y0,0.6/OVRS,csub=23)
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=20,nwalkers=54,ntemps=3)
print 'set up sampler'
S.sample(500)
outFile = '/data/ljo31/Lens/J1606/Kp_'+str(X)
f = open(outFile,'wb')
...
jj=0
for jj in range(30):
    S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=20,nwalkers=54,ntemps=3,initialPars=trace[-1])
    S.sample(500)
...
for gal in gals:
    gal.setPars()
    tmp = xc*0.
    tmp = gal.pixeval(xp,yp,0.6/OVRS,csub=23) # evaulate on the oversampled grid. OVRS = number of new pixels per old pixel.
    tmp = iT.resamp(tmp,OVRS,True) # convert it back to original size
    tmp = convolve.convolve(tmp,psf,False)[0]
    model[n] = tmp.ravel()
    n +=1
...
for src in srcs:
    ...
    tmp = xc*0.
    if src.pars['q']<0.2:
        tmp = src.boxypixeval(x0,y0,0.6/OVRS,csub=23,c=pars[-1].value)
    else:
