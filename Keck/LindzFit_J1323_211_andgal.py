...
#X=2
X=3
X=4 # trying to keep re, n small...
X=5 # not dividing by 90
X=6 # also refitting the galaxy light - this should be significant because of all the dust
X=7 # same again. Was I not dividing by 90 last time???
print X
# plot things
...
S.sample(1000)
outFile = '/data/ljo31/Lens/J1323/Kp_211_'+str(X)
f = open(outFile,'wb')
...
for gal in gals:
    gal.setPars()
    tmp = xc*0.
    tmp = gal.pixeval(xp,yp,0.2/OVRS,csub=11) # evaulate on the oversampled grid. OVRS = number of new pixels per old pixel.
    tmp = iT.resamp(tmp,OVRS,True) # convert it back to original size
    tmp = convolve.convolve(tmp,psf,False)[0]
    model[n] = tmp.ravel()
    n +=1
...
for src in srcs:
    src.setPars()
    tmp = xc*0.
    tmp = src.pixeval(x0,y0,0.2/OVRS,csub=11)
    tmp = iT.resamp(tmp,OVRS,True)
    tmp = convolve.convolve(tmp,psf,False)[0]
    model[n] = tmp.ravel()
    n +=1
