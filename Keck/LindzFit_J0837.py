...
kresult = np.load('/data/ljo31/Lens/J0837/twoband_Kp_dl_0')
klp,ktrace,kdic,_ = kresult
ka1,ka3 = numpy.unravel_index(klp.argmax(),klp.shape)
...
pars.append(pymc.Uniform('xoffset',-10.,10.,value=kdic['xoffset'][ka1,ka3]))
pars.append(pymc.Uniform('yoffset',-10.,10.,value=kdic['yoffset'][ka1,ka3]))
cov += [0.4,0.4]
#psf1
pars.append(pymc.Uniform('sigma 1',0,8,value=kdic['sigma 1'][ka1,ka3]))
pars.append(pymc.Uniform('q 1',0.05,1,value=kdic['q 1'][ka1,ka3]))
pars.append(pymc.Uniform('pa 1',-180,180,value= kdic['pa 1'][ka1,ka3] ))
pars.append(pymc.Uniform('amp 1',0,1,value=kdic['amp 1'][ka1,ka3]))
cov += [1,0.5,50,0.5]

# psf2
pars.append(pymc.Uniform('sigma 2',0.1,60,value=kdic['sigma 2'][ka1,ka3]  )) 
pars.append(pymc.Uniform('q 2',0.05,1,value=kdic['q 2'][ka1,ka3]))
pars.append(pymc.Uniform('pa 2',-180,180,value= kdic['pa 2'][ka1,ka3] ))
pars.append(pymc.Uniform('amp 2',0,1,value=kdic['amp 2'][ka1,ka3]))
cov += [1,0.5,50,0.5]

# psf3
pars.append(pymc.Uniform('sigma 3',0.1,400,value= kdic['sigma 3'][ka1,ka3] )) 
pars.append(pymc.Uniform('q 3',0.05,1,value=kdic['q 3'][ka1,ka3]))
pars.append(pymc.Uniform('pa 3',-180,180,value=kdic['pa 3'][ka1,ka3]  ))
pars.append(pymc.Uniform('amp 3',0,1,value=kdic['amp 3'][ka1,ka3]))
cov += [1,0.5,50,0.5]
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=22,nwalkers=52,ntemps=3)
S.sample(500)
outFile = '/data/ljo31/Lens/J0837/twoband_Kp_'+str(X)
f = open(outFile,'wb')
...
for gal in gals:
    gal.setPars()
    tmp = xc*0.
    tmp = gal.pixeval(xp,yp,0.2,csub=11) # evaulate on the oversampled grid. OVRS = number of new pixels per old pixel.
    tmp = iT.resamp(tmp,OVRS,True) # convert it back to original size
    tmp = convolve.convolve(tmp,psf,False)[0]
    model[n] = tmp.ravel()
    n +=1
...
for src in srcs:
    src.setPars()
    tmp = xc*0.
    tmp = src.pixeval(x0,y0,0.2,csub=11)
    tmp = iT.resamp(tmp,OVRS,True)
    tmp = convolve.convolve(tmp,psf,False)[0]
    model[n] = tmp.ravel()
    if src.name == 'Source 2':
