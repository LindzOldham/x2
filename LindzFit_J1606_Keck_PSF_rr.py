...
kresult = np.load('/data/ljo31/Lens/J1606/KeckPSF_112')
klp= kresult[0]
ka2=0
...
cov = []
### four PSF components
pars.append(pymc.Uniform('xoffset',-40.,40.,value=xoffset))
pars.append(pymc.Uniform('yoffset',-40.,40.,value=yoffset))
cov += [0.4,0.4]
#psf1
...
def logP(value=0.,p=pars):
    ...
    for gal in gals:
        gal.setPars()
        tmp = xc*0.
        tmp[mask2] = gal.pixeval(xin,yin,0.6/OVRS,csub=11) # evaulate on the oversampled grid. OVRS = number of new pixels per old pixel.
    ...
    for src in srcs:
        ...
        tmp = xc*0.
        if src.pars['q']<0.2:
            tmp[mask2] = src.boxypixeval(x0,y0,0.6/OVRS,csub=11,c=bar)
        else:
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=20,nwalkers=44,ntemps=3,initialPars=ktrace[-1])
print 'set up sampler'
S.sample(500)
outFile = '/data/ljo31/Lens/J1606/KeckPSF_'+str(X)
f = open(outFile,'wb')
...
for src in srcs:
    ...
    tmp = xc*0.
    if src.pars['q']<0.2:
        tmp = src.boxypixeval(x0,y0,0.6/OVRS,csub=11,c=bar)
    else:
