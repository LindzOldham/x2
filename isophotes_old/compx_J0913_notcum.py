...
yo,xo = iT.overSample(np.zeros((xc*4,yc*4)).shape,1)
xo -= srcs[0].pars['x']
yo -= srcs[0].pars['y']

#dx,dy = xo-srcs[0].pars['x'], yo-srcs[0].pars['y']
r = ((xo-xc)**2. + (yo-yc)**2.)**0.5

S1 = fits[0][-3]*srcs[0].pixeval(xo,yo,1,csub=31)* 2.*np.pi*r
#S2 = fits[0][-2]*srcs[1].pixeval(xo,yo,1.,csub=31)* 2.*np.pi*r


# ok. We want to make contours, and fit said contours with ellipses.
fracs = [0.4,0.5]#,0.55,0.6,0.65,0.7,0.75,0.8,0.85,0.9,0.95,0.99,0.999]
...
def ellipse(H,xbins,ybins,smooth=[1,1],color='CornflowerBlue'):
    ...
    for i in range(len(y)):
        ...
        S.sample(2000)
        outFile = '/data/ljo31/Lens/compx_J0901_S1_'+str(fracs[i])
        f = open(outFile,'wb')
