...
result = np.load('/data/ljo31/Lens/J1144/emcee21')
...
oldresult = np.load('/data/ljo31/Lens/J1144/emcee21')
olddic = oldresult[2]
oldlp=oldresult[0]
...
for name in S.keys():
    ...
    if name == 'Source 1':
        for key in 'q','re','n','pa':
            s[key]['value'] = np.array(dic[name+' '+key][a1,a2,a3])
        for key in 'x','y':
            s[key]['value'] = np.array(dic[name+' '+key][a1,a2,a3] + lenses[0].pars[key])
    elif name == 'Source 2':
        print name
        for key in 'q','re','n','pa':
            s[key]['value'] = np.array(dic[name+' '+key][a1,a2,a3])
...
cPickle.dump([G,L,S,offsets,[guishear,True]],open('/data/ljo31/Lens/J1144/UpdatedGuiFile_21','wb'))
