...
def SotPleparately(image,im,sigma,col):
    ext = [0,image.shape[0],0,image.shape[1]]
    pl.figure()
    pl.imshow(image,origin='lower',interpolation='nearest',extent=ext,cmap='afmhot',vmin=0,vmax=np.amax(image)*0.4) #,vmin=vmin,vmax=vmax)
    pl.colorbar()
    pl.title('data - '+str(col))
    pl.figure()
    pl.imshow(im,origin='lower',interpolation='nearest',extent=ext,cmap='afmhot',vmin=0,vmax=np.amax(image)*0.4) #,vmin=vmin,vmax=vmax)
    pl.colorbar()
...
result = np.load('/data/ljo31/Lens/J1606/emcee9')
...
###
print r'\end{tabular}'
print r'\caption{', 'shear = $', '%.2f'%Ddic['extShear'], '_{-', '%.2f'%Ldic['extShear'],'}^{+','%.2f'%Udic['extShear'], '}$ , shear pa = $',  '%.2f'%Ddic['extShear PA'], '_{-', '%.2f'%Ldic['extShear PA'],'}^{+','%.2f'%Udic['extShear PA'], '}$', 'boxiness = $', '%.2f'%Ddic['boxiness'], '_{-', '%.2f'%Ldic['boxiness'],'}^{+','%.2f'%Udic['boxiness'], '}$}'
print r'\end{table}'
