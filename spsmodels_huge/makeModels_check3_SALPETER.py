...
filters1 = ['B_Johnson','I_Cousins','V_Johnson','F555W_ACS']
filters2 = ['B_Johnson','I_Cousins','V_Johnson','F606W_ACS']
szs = np.load('/data/ljo31/Lens/LensParams/SourceRedshiftsUpdated.npy')[()]
for name in szs.keys():
    redshift = szs[name][0]
    print name, '%.2f'%redshift
    if bands[name] == 'F555W':
        filters = filters1
    ...
    outname = '/data/ljo31b/EELs/spsmodels/wide/BIV_F606W_'+name+'_salpBC03.model'
    f = open(outname,'wb')
