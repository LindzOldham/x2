...
import myEmcee_blobs as myEmcee, pylab as pl
S = myEmcee.PTEmcee(pars+[loglikelihood],cov=cov,nthreads=16,nwalkers=30,ntemps=3)
print S.ndim
S.sample(10000)
result = S.result()
...
S = myEmcee.PTEmcee(pars+[loglikelihood],cov=cov,nthreads=16,nwalkers=40,ntemps=3,filename='/data/ljo31b/EELs/inference/new/huge/prep_212_'+name+'.dat')    
S.sample_save(10000)
result = S.result()
lp,trace,dic,_ = result
a1,a2 = np.unravel_index(lp[:,0].argmax(),lp[:,0].shape)
...
outFile = '/data/ljo31b/EELs/inference/new/huge/result_212_'+str(name)
f = open(outFile,'wb')
