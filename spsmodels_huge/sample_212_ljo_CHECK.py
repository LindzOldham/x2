...
data['i source'] = {'mag':i_src,'sigma':di_src}
data['k source'] = {'mag':k_src,'sigma':dk_src}
...
imods = modelS.models[Iband].eval(s)- 2.5*ms
kmods = modelS.models[Kband].eval(s)- 2.5*ms
mod += [vimodl,vkmodl,vimods,vkmods,vmods,imods,kmods]

filtlist = sdssfilts+ ['v-i lens', 'v-k lens','v-i source', 'v-k source','v source','i source','k source']
# ein Tabular machen
...
np.save('/data/ljo31b/EELs/inference/new/huge/NEW_FOR_FP_212_params_'+name,np.array(data))

lo,med,hi=np.load('/data/ljo31b/EELs/inference/new/huge/NEW_FOR_FP_212_params_'+name+'.npy').T
...
filename = '/data/ljo31b/EELs/inference/new/huge/NEWFORFP_212_masses_'+name+'.npy'
f = open(filename, "wb")
np.save(f,array)
f.close()
'''
filename = '/data/ljo31b/EELs/inference/new/huge/NEWFORFP_212_masses.npy'
f = open(filename, "a+b")
np.save(f,array)
f.close()
'''
