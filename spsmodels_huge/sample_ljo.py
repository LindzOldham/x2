...
lp, trace,det,_ = np.load('/data/ljo31b/EELs/inference/new/huge/result_211_'+name)
...
vkidata = np.load('/data/ljo31/Lens/LensParams/VIK_phot_211_dict_huge_new.npy')[()]
...
chain = np.loadtxt('/data/ljo31b/EELs/inference/new/huge/211_'+name+'.dat')
triangle_plot(chain[5000:],cmap='Blues',nbins=20,axis_labels=[r'$\log \tau_{v,l}$',r'$\tau_l$',r'$T_l$',r'$\log Z_l$',r'$\log \tau_{v,s}$',r'$\tau_s$',r'$T_s$',r'$\log Z_s$',r'$\log M_l$', r'$\log M_s$'])
...
pl.savefig('/data/ljo31/Lens/TeXstuff/inference211newHUGE'+name+'.pdf')
...
for quant in det.keys():
    print quant
    lo,med,hi = np.percentile(det[quant][5000:,0].flatten(),[16,50,84],axis=0)
    data.append([lo,med,hi])
np.save('/data/ljo31b/EELs/inference/new/huge/211_params_'+name,np.array(data))
