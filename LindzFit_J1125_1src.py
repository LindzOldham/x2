...
guiFile = '/data/ljo31/Lens/J1125/1src_3'

result = np.load('/data/ljo31/Lens/J1125/emcee357')
lp= result[0]
a2=0
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=3,nwalkers=40,ntemps=3)
S.sample(1000)
outFile = '/data/ljo31/Lens/J1125/emcee_1src'+str(X)
f = open(outFile,'wb')
...
jj=0
for jj in range(15):
    S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=3,nwalkers=40,ntemps=3,initialPars=trace[-1])
    S.sample(1000)

    outFile = '/data/ljo31/Lens/J1125/emcee_1src'+str(X)+str(jj)
    f = open(outFile,'wb')
    cPickle.dump(S.result(),f,2)
    f.close()
