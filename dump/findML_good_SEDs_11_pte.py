...
dvk = np.sqrt(dv**2.+dk**2.)[:-1]
dvi = np.sqrt(dv**2.+di**2.)[:-1]
#vi0,vk0=v-i,v-k
...
grid_v = np.zeros((len(ages),11))
grid_i,grid_k = grid_v*0.,grid_v*0.
for a in range(len(ages)):
    grid_v[a] = table_v[ages[a]][:-1]
    grid_i[a] = table_i[ages[a]][:-1]
    grid_k[a] = table_k[ages[a]][:-1]
...
pars = []
cov = []
pars.append(pymc.Uniform('log age 1',9.2,11,value=10.))
pars.append(pymc.Uniform('log age 2',9.2,11,value=10.))
pars.append(pymc.Uniform('log age 3',9.2,11,value=10.))
pars.append(pymc.Uniform('log age 4',9.2,11,value=10.))
pars.append(pymc.Uniform('log age 5',9.2,11,value=10.))
pars.append(pymc.Uniform('log age 6',9.2,11,value=10.))
pars.append(pymc.Uniform('log age 7',9.2,11,value=10.))
pars.append(pymc.Uniform('log age 8',9.2,11,value=10.))
pars.append(pymc.Uniform('log age 9',9.2,11,value=10.))
pars.append(pymc.Uniform('log age 10',9.2,11,value=10.))
pars.append(pymc.Uniform('log age 11',9.2,11,value=10.))
...
def logP(value=0.,p=pars):
    ...
    vi,vk=v-i,v-k
    #for ii in range(11):
    #    print 'vi: ',model_vi[ii], vi[ii]
    #    print 'vk: ', model_vk[ii],vk[ii]
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=20,nwalkers=80,ntemps=3)
S.sample(10000)
outFile = '/data/ljo31/Lens/Analysis/1src_VKIage_v4_pte'
f = open(outFile,'wb')
...
np.save('/data/ljo31/Lens/LensParams/InferredAges_1src_v4',data)
...
np.save('/data/ljo31/Lens/Analysis/InferredAges_1src_all_v4',np.array(data))
