...
dvk = np.sqrt(dv**2.+dk**2.)[:-3]
dvi = np.sqrt(dv**2.+di**2.)[:-3]
#vi0,vk0=v-i,v-k
...
grid_v = np.zeros((len(ages),9))
grid_i,grid_k = grid_v*0.,grid_v*0.
for a in range(len(ages)):
    grid_v[a] = table_v[ages[a]][:-3]
    grid_i[a] = table_i[ages[a]][:-3]
    grid_k[a] = table_k[ages[a]][:-3]
...
S.sample(10000)
outFile = '/data/ljo31/Lens/Analysis/1src_VKIage_9'
f = open(outFile,'wb')
