...
def Setup(kk,dvi,dvk):
    pars = []
    cov = []
    #pars.append(pymc.Uniform('log age 1',7,9.95,value=8.5))
    ...
    S.sample(5000)
    outFile = '/data/ljo31/Lens/Analysis/1src_VKIage_single'+str(kk)
    #f = open(outFile,'wb')
    ...
    return result,mass,mlv

Uebertrace = np.zeros(12)
Ueberlp = 0
for KK in range(12):
    print KK
    result,mass,mlv = Setup(KK,dvi,dvk)
    lp,trace,dic,_ = result
    a1,a2 = np.unravel_index(lp.argmax(),lp.shape)
    Uebertrace[KK] = trace[a1,a2,0]
    Ueberlp += lp[a1,a2]
    print name[KK],' & ', '%.2f'%Uebertrace[KK], ' & ', '%.2f'%mass, ' & ', '%.2f'%mlv, r'\\'


print Ueberlp
#np.save('/data/ljo31/Lens/Analysis/InferredAges_1src_singly_lessthan9gyr',Uebertrace)

lv=np.zeros(12)
li,lk,model_i,model_v,model_k,mlv,mlb=lv*0.,lv*0.,lv*0.,lv*0.,lv*0.,lv*0.,lv*0.
logT,divT = np.zeros(12),np.zeros(12)
for kk in range(12):
    logT[kk] = Ueberlp[kk]
...
for m in range(len(mass)):
    print name[m],' & ', '%.2f'%Uebertrace[m], ' & ', '%.2f'%mass[m], ' & ', '%.2f'%mlv[m], r'\\'
...
#utrace = np.load('/data/ljo31/Lens/Analysis/InferredAges_1src_singly_lessthan9gyr.npy')
