...
dvk = np.sqrt(dv**2.+dk**2.)[:-M]
dvi = np.sqrt(dv**2.+di**2.)[:-M]
#vi0,vk0=v-i,v-k
...
grid_v = np.zeros((len(ages),N))
grid_i,grid_k = grid_v*0.,grid_v*0.
for a in range(len(ages)):
    grid_v[a] = table_v[ages[a]][:-M]
    grid_i[a] = table_i[ages[a]][:-M]
    grid_k[a] = table_k[ages[a]][:-M]
...
cov = []
for ii in range(N):
    pars.append(pymc.Uniform('log age '+str(N),6,9.92,value=9.)) # age of universe = 8.277 Gyr at z=0.55
    cov += [1.]
optCov=np.array(cov)
...
S.sample(6000)
outFile = '/data/ljo31/Lens/Analysis/1src_VKIage_'+str(N)
f = open(outFile,'wb')
...
for kk in range(N):
    ...
    print '%.2f'%model_v[kk], '%.2f'%model_i[kk], '%.2f'%model_k[kk]
model_vi,model_vk = model_v-model_i, model_v-model_k
v,i,k = -2.5*np.array(lv)+v_sun, -2.5*np.array(li)+i_sun, -2.5*np.array(lk) + k_sun
vi,vk=v-i,v-k
resid = -0.5*(model_vi-vi)**2./dvi**2. -0.5*(model_vk-vk)**2./dvk**2.
