...
cov += [0.1,0.5,0.05]
## now we are also going to try fitting for the Gaussian parent distributrion from which data are drawn. Also need to work out covariances.
pars.append(pymc.Uniform('sigma',0.2,5.,value=1. ))
pars.append(pymc.Uniform('mean',10.,12.,value=11. ))
# we now have additional terms in Sigma because of the prior
cov += [0.1,0.5]
...
def logP(value=0.,p=pars):
    lp=0.
    theta,b,scatter, sigma, mean = pars[0].value, pars[1].value,pars[2].value, pars[3].value,pars[4].value
    st,ct = sin(theta), cos(theta)
    Delta = y*ct - x*st - b*ct
    sxx2 = sxx**2. + sigma**2.
    syy2 = syy**2. + scatter**2. + (b*sigma)**2.
    sxy2 = sxy**2. + b*sigma**2.
    syx2 = syx**2. + b*sigma**2.


    Sigma = sxx2*st**2. + syy2*ct**2. -st*ct*(sxy2+syx2) # project
