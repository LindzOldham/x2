...
los = dict([('q',0.005),('pa',-180.),('re',0.1),('n',0.1)])
...
pars.append(pymc.Uniform('boxiness',1.,4,value=dic['boxiness'][a1,a2,a3]))
cov += [0.4]
    ...
    for i in range(len(imgs)):
        ...
        for src in srcs:
            ...
            if src.name == 'Source 2':
                tmp[mask2] = src.boxypixeval(x0,y0,1./OVRS,csub=31,c=boxiness)
            else:
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=22,nwalkers=58,ntemps=2,initialPars=trace[-1,:2])
S.sample(500)
...
for i in range(len(imgs)):
    ...
    for src in srcs:
        ...
        tmp = xc*0.
        if src.name == 'Source 2':
            tmp = src.boxypixeval(x0,y0,1./OVRS,csub=31,c=boxiness)
        else:
