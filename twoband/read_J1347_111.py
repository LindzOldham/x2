...
result = np.load('/data/ljo31/Lens/J1347/twoband_111_0')
...
lresult = np.load('/data/ljo31/Lens/LensModels/J1347_211')
...
for i in range(len(imgs)):
    ...
    pl.show()
    #for kk in range(len(components)):
    #    pl.figure()
    #    simple.climshow(components[i])
    #    pl.colorbar()

pl.figure()
pl.plot(lp[:,0])

#lresult = np.load('/data/ljo31/Lens/J1347/twoband_1_ctd_9')#LensModels/J1347_211')
#llp,ltrace,ldic,_= lresult
#la2=0
#la1,la3 = numpy.unravel_index(llp[:,0].argmax(),llp[:,0].shape)


for key in ['Source 1 re', 'Source 1 n', 'Galaxy 1 re', 'Galaxy 1 n']:
    pl.figure()
    pl.subplot(211)
