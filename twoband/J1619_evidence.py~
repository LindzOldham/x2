...
yc,yo=yc-100,yo-100
xc,xo=xc-100,xo-100
mask = np.zeros(img1.shape)# py.open('/data/ljo31/Lens/J1619/mask_huge.fits')[0].data
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=32,nwalkers=52,ntemps=3,initialPars=trace[-1])
S.sample(500)
outFile = '/data/ljo31/Lens/J1619/twoband_'+str(X)
f = open(outFile,'wb')
...
jj=0
for jj in range(12):
    S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=32,nwalkers=52,ntemps=3,initialPars=trace[-1]) #too complicated!
    print 'sampling'
    S.sample(1000)
