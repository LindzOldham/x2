...
result = np.load('/data/ljo31/Lens/J0913/twoband_1_emcee')
lp,trace,dic,_= result
a2=0
a1,a3 = numpy.unravel_index(lp.argmax(),lp.shape)
lresult = np.load('/data/ljo31/Lens/LensModels/J0913_211')
...
presult = np.load('/data/ljo31/Lens/LensModels/new/J0913_211')
_,ptrace,pdic,_= presult
lresult = np.load('/data/ljo31/Lens/J0913/twoband_1')
_,ltrace,ldic,_= lresult
tresult = np.load('/data/ljo31/Lens/J0913/twoband_3')
_,ttrace,tdic,_= tresult

for key in ['Source 1 re', 'Source 1 n', 'Galaxy 1 re', 'Galaxy 1 n', 'Galaxy 2 re', 'Galaxy 2 n']:
    pl.figure()
    pl.subplot(211)
    pl.hist(dic[key].ravel(),30,normed=True,histtype='stepfilled',alpha=0.5,label='current')
    pl.hist(ldic[key][:,0].ravel(),30,normed=True,histtype='stepfilled',alpha=0.5,label='same box size')
    #pl.hist(pdic[key][:,0].ravel(),30,normed=True,histtype='stepfilled',alpha=0.5,label='old box')
    #pl.hist(tdic[key][:,0].ravel(),30,normed=True,histtype='stepfilled',alpha=0.5,label='bigger box')
