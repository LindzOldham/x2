...
N=12
print names[:N]
xx,yy,zz = np.log10(sigmas)[:N], mu.copy()[:N], np.log10(re)[:N]
dxx,dyy,dzz = dsigmas[:N]/sigmas[:N]/np.log(10.), dmu[:N]/mu[:N]/np.log(10.), dre[:N]/re[:N]/np.log(10.)
...
syz,szy=0,0
# covariances
syz,szy = rhos[:N]*dyy*dzz,rhos[:N]*dyy*dzz
