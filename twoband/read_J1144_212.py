...
result = np.load('/data/ljo31/Lens/J1144/twoband_212_ctd')
lp,trace,dic,_= result
a2=0
a1,a3 = numpy.unravel_index(lp[:,0].argmax(),lp[:,0].shape)
lresult = np.load('/data/ljo31/Lens/LensModels/J1144_212_allparams')
llp,ltrace,ldic,_= lresult
la2=0
la1,la3 = numpy.unravel_index(llp.argmax(),llp.shape)
...
for name in ['Source 2','Source 1']:
    ...
    if name+' pa' in dic.keys():
        p['pa'] = dic[name+' pa'][a1,a2,a3]
    else:
        p['pa'] = srcs[0].pars['pa']
...
for key in ['Source 1 re', 'Source 1 n', 'Source 2 re', 'Source 2 n']:
    pl.figure()
    pl.subplot(211)
    pl.hist(dic[key][:,0].ravel(),30,normed=True,histtype='stepfilled',alpha=0.5)
    pl.hist(ldic[key].ravel(),30,normed=True,histtype='stepfilled',alpha=0.5)
    pl.subplot(212)
    pl.plot(dic[key][:,0])
    pl.suptitle(key)
