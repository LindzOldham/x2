...
fp = np.load('/data/ljo31b/EELs/esi/kinematics/inference/results_1.50_source.npy')
l,m,u = fp
...
rhos = np.load('/data/ljo31/Lens/LensParams/rho_huge_211.npy')[:-1]

#xx,yy,zz = np.log10(sigmas), mu.copy(), np.log10(re)
#dxx,dyy,dzz = dsigmas/sigmas/np.log(10.), dmu, dre/re/np.log(10.)

xx,yy,zz = np.log10(sigmas)[:-1], mu[:-1].copy(), np.log10(re)[:-1]
dxx,dyy,dzz = dsigmas[:-1]/sigmas[:-1]/np.log(10.), dmu[:-1], dre[:-1]/re[:-1]/np.log(10.)
...
pl.figure()
pl.scatter(0.7*xx+0.3*yy,zz,color='SteelBlue',s=40)
pl.show()
...
S.sample(6000)
outFile = '/data/ljo31b/EELs/inference/FP/sourceFP_with_lensdata'
f = open(outFile,'wb')
...
pl.title('EELs')
pl.savefig('/data/ljo31b/EELs/inference/FP/src212old.pdf')
pl.show()
