...
X='212_pa_2' # with everything - lens on, big image, ovrs=2!!!
print X
...
# made these smaller for faster convergence!!!
covs = dict([('x',0.1),('y',0.1),('q',0.01),('pa',5.),('re',0.5),('n',0.2)])
covlens = dict([('x',0.1),('y',0.1),('q',0.01),('pa',5.),('b',0.1),('eta',0.1)])
...
for name in ['Source 2','Source 1']:
    ...
    if name == 'Source 2':
        key = 'pa'
        val = dic['Source 1 '+key][a1,a3]
        print 'source 2, pa, ', val
        lo,hi= los[key],his[key]
        pars.append(pymc.Uniform('%s %s'%(name,key),lo ,hi,value=val ))
        p[key] = pars[-1]
        cov.append(covs[key])
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=32,nwalkers=60,ntemps=2)#,ntemps=3)
S.sample(500)
...
for jj in range(12):
    ...
    outFile = '/data/ljo31/Lens/J0913/twoband_'+str(X)#+'_'+str(jj)
    f = open(outFile,'wb')
    cPickle.dump(S.result(),f,2)
    f.close()
