...
'''
S = myEmcee.Emcee(pars+[logP,likelihood],cov=optCov,nthreads=22,nwalkers=64)#,ntemps=3)
S.sample_blobs(500)
...
for jj in range(12):
    S.p0 = trace[-1]
    #S = myEmcee.Emcee(pars+[logP,likelihood],cov=optCov,nthreads=22,nwalkers=64,initialPars=trace[-1]) #too complicated!
    print 'sampling'
    S.sample_blobs(1000)
    ...
    outFile = '/data/ljo31/Lens/J0913/twoband_blobs_'+str(X)#+'_'+str(jj)
    f = open(outFile,'wb')
    cPickle.dump(S.blobs,f,2)
    f.close()
