...
from math import pi
from imageSim import SBProfiles
...
class SBModel:
    def __init__(self,name,pars,convolve=0):
        ...
        if self.keys not in self._SBkeys:
            import sys
            print 'Not all (or too many) parameters were defined!',self.keys
            sys.exit()
...
class Sersic(SBModel,SBBProfiles.Sersic):
    _baseProfile = SBBProfiles.Sersic
...
class Moffat(SBModel,SBProfiles.Moffat):
...
class Disk(SBModel,SBBProfiles.Disk):
    _baseProfile = SBBProfiles.Disk
    _SBkeys = [['amp','pa','x0','y0','x','y','xmax']]
...
class ExpDisk(SBModel,SBBProfiles.ExpDisk):
    _baseProfile = SBBProfiles.ExpDisk
    _SBkeys = [['amp','pa','x0','y0','x','y','xmax','ymax']]

    def __init__(self,name,pars,convolve=0):
        if 'amp' not in pars.keys() and 'logamp' not in pars.keys():
            pars['amp'] = 1.
        if 'ymax' not in pars.keys():
            pars['ymax'] = None
        self.keys = pars.keys()
        self.keys.sort()
        self._baseProfile.__init__(self)
...
class InclinedDisk(SBModel,SBBProfiles.InclinedDisk):
    _baseProfile = SBBProfiles.InclinedDisk
    _SBkeys = [['amp','pa','x0','y0','x','y','i']]
