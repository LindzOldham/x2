...
def SotPleparately(image,im,sigma,col):
    ext = [0,image.shape[0],0,image.shape[1]]
    pl.figure()
    pl.imshow(image,origin='lower',interpolation='nearest',extent=ext,cmap='afmhot',vmin=0,vmax=0.3*np.amax(image)) #,vmin=vmin,vmax=vmax)
    pl.colorbar()
    pl.title('data - '+str(col))
    pl.figure()
    pl.imshow(im,origin='lower',interpolation='nearest',extent=ext,cmap='afmhot',vmin=0,vmax=np.amax(image)*0.3) #,vmin=vmin,vmax=vmax)
    pl.colorbar()
...
result = np.load('/data/ljo31/Lens/J1606/emcee_1src2')
...
guiFile = '/data/ljo31/Lens/J1606/1src_1' 
G,L,S,offsets,shear = numpy.load(guiFile)
lenses = []
p = {}
for key in 'x','y','q','pa','b','eta':
    p[key] = L['Lens 1'][key]['value'] #dic['Lens 1 '+key][a1,a2,a3]
