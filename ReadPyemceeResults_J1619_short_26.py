...
for i in range(len(imgs)):
    ...
    print xin.shape, xc.shape,xc[mask2].shape
    n = 0
    ...
    for gal in gals:
        ...
        gal.setPars()
        tmp = xc*0.
        tmp = gal.pixeval(xin,yin,1./OVRS,csub=1) # evaulate on the oversampled grid. OVRS = number of new pixels per old pixel.
        tmp = tmp.reshape(xc.shape)
...
re6,n6,pa6,q6 = dic['Galaxy 3 re'][a1,a2,a3], dic['Galaxy 3 n'][a1,a2,a3], dic['Galaxy 3 pa'][a1,a2,a3], dic['Galaxy 3 q'][a1,a2,a3]
...
print 'galaxy 3 ', '&', '%.2f'%x3, '&',  '%.2f'%y3, '&', '%.2f'%n6, '&', '%.2f'%re6, '&', '%.2f'%q6, '&','%.2f'%pa6,  r'\\'
...
for i in range(len(imgs)):
    ...
    comps = False
    if comps == True:
        for kk in range(len(gals)+len(srcs)):
            pl.figure()
            pl.imshow(components[kk],interpolation='nearest',origin='lower',cmap='afmhot')
            pl.colorbar()
    fits.append(fit)
