...
'''
X = 0 - basic test with 5 samples
X = 1 - changed covariances of source and shear PAs (100!) and ran for 500 samples
X = 2 - as X = 1, with 1000 samples (3 temps, 60 walkers, 2 threads)
X = 3 - 3 threads, 60 qwalkers, 5 temps, 2000 samples
X = 4 - 500 again. emcee_2, with inflated covariances on everything (as opposed to just the pas of the sources and shear). (Running for 100, 500 and 1000 samples, all under X = 4. Kind of like incrementally saving progress. This doesn't step very often/well. ABort!
X = 5 - emcee_2 (from the gui), with inflated covariances only on the angles of sources and shear!
X = 6 - as X = 5, but with 10*cov for angles. Should sample better?
X = 7 - as in X = 5 & 6, only with source angles initialised at 90 degrees and ellipticities at about 0.7.
X = 8 - det8_alluniform. Ocersample = 8 ! 500 samples, 3 temps, 2 threads.
X = 9 - a long, many-temperatured version of X = 8. 1500 samples, 5 temperatures.
'''
...
guiFile = '/data/ljo31/Lens/J1347/det8_alluniform'
...
cov.append(0.05)
p['b'] = pars[-1]
pars.append(pymc.Uniform('extShear PA',-180.,180.))
cov.append(10.)
p['pa'] = pars[-1]
...
S = myEmcee.PTEmcee(pars+[likelihood],cov=optCov,nthreads=32,nwalkers=60,ntemps=5)
S.sample(1500)
...
np.save('/data/ljo31/Lens/J1347/emcee_'+str(X), result)
...
np.save('/data/ljo31/Lens/J1347/emcee_'+str(X)+'b', result)
