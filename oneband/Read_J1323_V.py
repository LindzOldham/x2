...
def SotPleparately(image,im,sigma,col):
    ext = [0,image.shape[0],0,image.shape[1]]
    pl.figure()
    pl.imshow(image,origin='lower',interpolation='nearest',extent=ext,cmap='afmhot',vmin=0,aspect='auto',vmax=0.5*np.amax(image)) #,vmin=vmin,vmax=vmax)
    pl.colorbar()
...
kresult = np.load('/data/ljo31/Lens/J1323/V_0')
klp= kresult[0]
ka2=0
...
pl.figure()
pl.hist(kdic['Source 1 n'][:,0].flatten(),30)
pl.figure()
pl.hist(kdic['Source 1 re'][:,0].flatten(),30)
pl.figure()
pl.hist(kdic['Source 2 n'][:,0].flatten(),30)
pl.figure()
pl.hist(kdic['Source 2 re'][:,0].flatten(),30)

np.save('/data/ljo31/Lens/radgrads/J1323_212_V',srcs)
